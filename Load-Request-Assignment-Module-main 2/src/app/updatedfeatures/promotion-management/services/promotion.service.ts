...
class PromotionService {
  ...
  private async makeRequest(
    ...
    try {
      ...
      if (
        ...
        const cachedData = this.getCachedData(cacheKey);
        if (cachedData) {
          console.log(`Cache hit for POST: ${url}`);
          return cachedData;
        }
      }
      ...
      if (
        ...
        const cacheKey = this.getCacheKey(method, url, data);
        this.setCachedData(cacheKey, result);
        console.log(`Cached POST response for: ${url}`);
      }
      ...
      if ((method === "PUT" || method === "DELETE") && result.success) {
        ...
        if (url.includes("/Promotion")) {
          ...
          console.log(
            `Cleared ${keysToDelete.length} cache entries after ${method} operation`
          );
        }
      }
  ...
  async getPromotionDetails(
    ...
    const response = await this.makeRequest(
      "POST",
      "/api/Promotion/GetPromotionDetails",
      data
    );
  ...
  async getPromotionByUID(promotionUID: string): Promise<any> {
    const result = await this.makeRequest(
      "GET",
      "/api/Promotion/GetPromotionDetailsByUID",
      null,
      { promotionUID }
    );
  ...
  async createUpdatePromotion(promotionData: any): Promise<any> {
    ...
    return this.makeRequest("POST", "/api/Promotion/CUDPromotionMaster", data);
  }

  async deletePromotion(promotionUID: string): Promise<any> {
    return this.makeRequest(
      "DELETE",
      "/api/Promotion/DeletePromotionDetailsbyUID",
      null,
      { PromotionUID: promotionUID }
    );
  }
  ...
  async activatePromotion(promotionUID: string): Promise<IApiResponse<any>> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/Promotion/ActivatePromotion`,
        {
          method: "PUT",
          headers: this.getAuthHeaders(),
  ...
  async deactivatePromotion(promotionUID: string): Promise<IApiResponse<any>> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/Promotion/DeactivatePromotion`,
        {
          method: "PUT",
          headers: this.getAuthHeaders(),
          body: JSON.stringify({ PromotionUID: promotionUID })
  ...
  async getSKUDetails(pageNumber: number = 1, pageSize?: number): Promise<any> {
    ...
    return this.makeRequest("POST", "/api/SKU/SelectAllSKUDetails", data);
  }
  ...
  async getAllSKUMasterData(
    ...
    return this.makeRequest("POST", "/api/SKU/GetAllSKUMasterData", data);
  }
  ...
  async getAllStores(pageNumber: number = 1, pageSize?: number): Promise<any> {
    ...
    return this.makeRequest("POST", "/api/Store/SelectAllStore", data);
  }
  ...
  async getOrganizationDetails(
    ...
    return this.makeRequest("POST", "/api/Org/GetOrgDetails", data);
  }
  ...
  async getEmployeeDetails(
    ...
    return this.makeRequest("POST", "/api/Emp/GetEmpDetails", data);
  }
