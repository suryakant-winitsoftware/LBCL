...
const EditRoute: React.FC = () => {
  ...
  const [originalScheduleDaywise, setOriginalScheduleDaywise] = useState<any>(null);
  const [originalScheduleFortnight, setOriginalScheduleFortnight] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const loadRouteData = async () => {
      ...
      try {
        ...
        // Use the same API call as the list to get route data
        ...
        if (response.IsSuccess && response.Data) {
          ...
          // Determine schedule configuration first
          const scheduleType = routeSchedule?.Type || "Daily";
          
          // Logic: If we have specific day configuration, enable the appropriate frequency type
          const hasSpecificDayConfig = routeScheduleDaywise && 
            (routeScheduleDaywise.Monday === 1 || routeScheduleDaywise.Tuesday === 1 || 
             routeScheduleDaywise.Wednesday === 1 || routeScheduleDaywise.Thursday === 1 || 
             routeScheduleDaywise.Friday === 1 || routeScheduleDaywise.Saturday === 1 || 
             routeScheduleDaywise.Sunday === 1);
          
          const enableDailyWeekly = hasSpecificDayConfig && scheduleType !== "Fortnightly";
          const enableFortnightly = scheduleType === "Fortnightly";
          ...
          console.log("Schedule Configuration:", {
            scheduleType,
            hasSpecificDayConfig,
            enableDailyWeekly,
            enableFortnightly,
            dailyWeeklyType: scheduleType === "Weekly" ? "Weekly" : scheduleType === "MultiplePerWeeks" ? "MultiplePerWeeks" : "Daily"
          });
          ...
          // Extract day-wise schedule if available
          ...
          const formData = {
            ...
            validFrom: route?.ValidFrom ? moment(route.ValidFrom).format("YYYY-MM-DD") : moment().format("YYYY-MM-DD"),
            validUpto: route?.ValidUpto ? moment(route.ValidUpto).format("YYYY-MM-DD") : moment().add(1, "year").format("YYYY-MM-DD"),
            visitDuration: route?.VisitDuration || 30,
            travelTime: route?.TravelTime || 15,
            totalCustomers: route?.TotalCustomers || 0,
            allowMultipleBeatsPerDay: routeSchedule?.AllowMultipleBeatsPerDay || false,
            selectedCustomers: routeCustomersList.map((customer: any) => customer.StoreUID),
            visitTime: route?.VisitTime !== "00:00:00" ? route?.VisitTime?.substring(0, 5) : "",
            endTime: route?.EndTime !== "00:00:00" ? route?.EndTime?.substring(0, 5) : "",
            // Schedule configuration
            enableDailyWeekly: enableDailyWeekly,
            enableFortnightly: enableFortnightly,
            dailyWeeklyType: scheduleType === "Weekly" ? "Weekly" : scheduleType === "MultiplePerWeeks" ? "MultiplePerWeeks" : "Daily",
            dailyWeeklyDays: dailyWeeklyDays,
            fortnightlyDays: dailyWeeklyDays, // Use same for fortnightly
            // Print settings
  ...
  useEffect(() => {
    if (pendingOrgUID && organizations.length > 0 && orgTypes.length > 0) {
      console.log("Organization data loaded, reconstructing hierarchy for:", pendingOrgUID);
  ...
  const reconstructOrganizationHierarchy = useCallback((targetOrgUID: string) => {
    ...
    const buildPath = (org: Organization, path: Organization[] = []): Organization[] => {
    ...
    console.log("Organization path:", orgPath.map(o => `${o.Name} (${o.OrgTypeUID})`));

    // Initialize hierarchy starting from root
    const initialLevels = initializeOrganizationHierarchy(organizations, orgTypes);
  ...
  const loadCustomers = useCallback(async (orgUID: string) => {
    ...
    try {
      const data = await api.store.selectAll({
        pageNumber: 0,
        pageSize: 500,
        isCountRequired: false,
        sortCriterias: [],
      ...
      if (data.IsSuccess && data.Data?.PagedData) {
        setDropdowns((prev) => ({
          ...prev,
          customers: data.Data.PagedData.map((store: any) => ({
            value: store.UID,
    ...
    try {
      ...
      const routeMasterData = {
        ...
        RouteSchedule: {
          ...
          CreatedBy: originalSchedule?.CreatedBy || originalRouteData?.CreatedBy || currentUserUID,
          CreatedTime: originalSchedule?.CreatedTime || originalRouteData?.CreatedTime || currentTimestamp,
          ModifiedBy: currentUserUID,
          ModifiedTime: currentTimestamp,
          ServerAddTime: originalSchedule?.ServerAddTime || originalRouteData?.ServerAddTime,
        ...
        RouteScheduleDaywise: values.enableDailyWeekly
          ? {
              ...
              CreatedBy: originalScheduleDaywise?.CreatedBy || originalRouteData?.CreatedBy || currentUserUID,
              CreatedTime: originalScheduleDaywise?.CreatedTime || originalRouteData?.CreatedTime || currentTimestamp,
              ModifiedBy: currentUserUID,
              ModifiedTime: currentTimestamp,
              ServerAddTime: originalScheduleDaywise?.ServerAddTime || originalRouteData?.ServerAddTime,
              ServerModifiedTime: currentTimestamp
            }
          : null,
        ...
        RouteCustomersList: (() => {
          ...
          originalCustomersList.forEach((originalCustomer) => {
            const isStillSelected = values.selectedCustomers.includes(originalCustomer.StoreUID);
            
            if (isStillSelected) {
              // Customer is still selected - update it
              const index = values.selectedCustomers.indexOf(originalCustomer.StoreUID);
              ...
              customersList.push({
                ...
                VisitTime: storeSchedule.visitTime || originalCustomer.VisitTime || "00:00:00",
                VisitDuration: storeSchedule.visitDuration || originalCustomer.VisitDuration || 30,
                EndTime: storeSchedule.endTime || originalCustomer.EndTime || "00:00:00",
                IsDeleted: false,
                IsBillToCustomer: values.billToCustomer === originalCustomer.StoreUID,
                TravelTime: storeSchedule.travelTime || originalCustomer.TravelTime || 15,
                ActionType: "Update",
          ...
          values.selectedCustomers.forEach((customerUID, index) => {
            ...
            if (isNewCustomer) {
              ...
              customersList.push({
                ...
                IsDeleted: false,
                IsBillToCustomer: values.billToCustomer === customerUID,
        ...
        RouteUserList: (() => {
          ...
          originalUsersList
            .filter(user => user.JobPositionUID !== originalRouteData?.JobPositionUID)
            .forEach((originalUser) => {
              const isStillSelected = values.selectedUsers.includes(originalUser.JobPositionUID);
          ...
          values.selectedUsers
            .filter(userUID => !originalUsersList.some(orig => orig.JobPositionUID === userUID))
      ...
      console.log("Customer Update Details:", {
        ...
        removedCustomers: originalCustomersList.filter(
          orig => !values.selectedCustomers.includes(orig.StoreUID)
        ).map(c => c.StoreUID),
        updatedCustomers: originalCustomersList.filter(
          orig => values.selectedCustomers.includes(orig.StoreUID)
        ).map(c => c.StoreUID)
      });
      console.log("User Update Details:", {
        ...
        removedUsers: originalUsersList
          .filter(orig => orig.JobPositionUID !== originalRouteData?.JobPositionUID)
      ...
      const response = await apiService.put('/Route/UpdateRouteMaster', routeMasterData);
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Configure visit frequency. If not set, defaults to Daily
                      visits on all weekdays.
                    </p>
                  </div>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-gray-900">Loading Route...</p>
