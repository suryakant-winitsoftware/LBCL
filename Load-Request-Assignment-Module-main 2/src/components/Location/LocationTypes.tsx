...
import { Plus, Edit, Trash2, ArrowUp, ArrowDown, Save, X, MapPin, MoreHorizontal, RefreshCw, Download, Eye, Network, Globe, Building, MapPinned } from "lucide-react"
...
export function LocationTypes() {
  ...
  const handleMoveUp = async (type: LocationType) => {
    const index = locationTypes.findIndex(t => t.UID === type.UID)
    if (index > 0) {
      const prevType = locationTypes[index - 1]
      
      try {
        // Swap level numbers
        await locationService.updateLocationType(type.UID, { LevelNo: prevType.LevelNo })
        await locationService.updateLocationType(prevType.UID, { LevelNo: type.LevelNo })
        
        fetchLocationTypes()
        toast({
          title: "Success",
          description: "Location type order updated"
        })
      } catch (error) {
        toast({
          title: "Error",
  ...
  const handleMoveDown = async (type: LocationType) => {
    const index = locationTypes.findIndex(t => t.UID === type.UID)
    if (index < locationTypes.length - 1) {
      const nextType = locationTypes[index + 1]
      
      try {
        // Swap level numbers
        await locationService.updateLocationType(type.UID, { LevelNo: nextType.LevelNo })
        await locationService.updateLocationType(nextType.UID, { LevelNo: type.LevelNo })
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button 
            variant="outline" 
            onClick={() => {
              const data = JSON.stringify(locationTypes, null, 2)
              const blob = new Blob([data], { type: 'application/json' })
              const url = URL.createObjectURL(blob)
              const a = document.createElement('a')
              a.href = url
              a.download = 'location-types.json'
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              disabled={index === locationTypes.length - 1}
                              title="Move down"
                              className="h-8 w-8 p-0"
                            >
                          ...
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                <DropdownMenuItem onClick={() => {/* View locations of this type */}}>
