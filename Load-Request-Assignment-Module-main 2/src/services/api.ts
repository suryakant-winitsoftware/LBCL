...
class ApiService {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            if (errorDetails.Message || errorDetails.message || errorDetails.ErrorMessage) {
              errorMessage = errorDetails.Message || errorDetails.message || errorDetails.ErrorMessage;
            }
          } else {
      ...
      console.error(`API Error: ${method} ${endpoint}`, ErrorHandler.handleApiError(error));
      throw error;
    } finally {
...
export const api = {
  ...
  location: {
    ...
    getByUID: (uid: string) => apiService.get(`/Location/GetLocationByUID?UID=${uid}`),
