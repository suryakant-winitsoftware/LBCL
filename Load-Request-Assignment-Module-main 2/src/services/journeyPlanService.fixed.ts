...
export const journeyPlanServiceFixed = {
  ...
  async getTodayJourneyPlanDetails(params: {
    ...
    try {
      const response = await apiService.post('/api/UserJourney/SelectTodayJourneyPlanDetails', {
        visitDate: typeof params.visitDate === 'string' 
          ? params.visitDate 
  ...
  async getAllUserJourneys(params: {
    ...
    try {
      const response = await apiService.post('/api/UserJourney/SelectAlUserJourneyDetails', {
        pageNumber: params.pageNumber || 1,
        pageSize: params.pageSize || 50,
        isCountRequired: true,
  ...
  async getJourneyDetailsByUID(uid: string): Promise<UserJourney | null> {
    try {
      const response = await apiService.get(`/api/UserJourney/GetUserJourneyDetailsByUID?UID=${uid}`);
  ...
  async getStoreVisitDetails(beatHistoryUID: string): Promise<StoreVisit[]> {
    try {
      const response = await apiService.post('/api/UserJourney/SelecteatHistoryInnerGridDetails', {
        beatHistoryUID
      });
  ...
  async reassignJourneyPlan(params: {
    ...
    try {
      ...
      const response = await apiService.post('/api/UserJourney/ReassignJourneyPlan', {
      ...
      if (error?.response?.status === 404) {
        throw new Error('Reassign functionality not yet implemented in backend');
      }
      throw error;
    }
  },
  ...
  async cancelJourneyPlan(params: {
    ...
    try {
      ...
      const response = await apiService.post('/api/UserJourney/CancelJourneyPlan', {
        beatHistoryUID: params.beatHistoryUID,
  ...
  async getJourneyPlanStats(params: {
    ...
    try {
      const response = await apiService.post('/api/UserJourney/GetJourneyPlanStatistics', {
        orgUID: params.orgUID,
  ...
  async getAvailableEmployees(params: {
    ...
    try {
      ...
      const response = await apiService.post('/api/Employee/GetAvailableEmployeesForDate', {
        orgUID: params.orgUID,
      ...
      try {
        const allEmployees = await apiService.get(`/api/Employee/GetEmployeesByOrg?orgUID=${params.orgUID}`);
  ...
  async getRoutJourneyHistory(params: {
    ...
    try {
      const response = await apiService.post('/api/UserJourney/GetRouteJourneyHistory', {
  ...
  async getUpcomingJourneyPlans(params: {
    ...
    try {
      ...
      const response = await apiService.post('/api/UserJourney/GetUpcomingJourneyPlans', {
        orgUID: params.orgUID,
  ...
  async checkJourneyPlanExists(params: {
    ...
    try {
      const beatHistoryUID = `${params.routeUID}_${params.date.replace(/-/g, '_')}`;
      const response = await apiService.get(`/api/UserJourney/CheckJourneyPlanExists?beatHistoryUID=${beatHistoryUID}`);
  ...
  async getMyTodayJourneyPlan(empUID: string): Promise<JourneyPlan | null> {
    try {
      ...
      const response = await apiService.post('/api/UserJourney/GetMyTodayJourneyPlan', {
        empUID,
        visitDate: today
      });
      return response?.data || null;
   ...
   * - Start Journey (POST /api/UserJourney/StartJourney)
   * - Store Check-In (POST /api/StoreHistory/CheckIn)
   * - Store Check-Out (POST /api/StoreHistory/CheckOut)
   * - End Journey (POST /api/UserJourney/EndJourney)
   * - Skip Store (POST /api/StoreHistory/SkipStore)
   * 
