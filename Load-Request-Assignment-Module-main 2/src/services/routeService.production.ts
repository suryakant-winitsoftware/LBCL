...
export class RouteServiceProduction {
  ...
  private buildRouteMaster(data: RouteCreationData, existingUID?: string): RouteMaster {
    ...
    const includeDaywise = ['Daily', 'MultiplePerWeeks'].includes(scheduleType);
    const includeFortnight = scheduleType === 'Fortnight';
    ...
    // Add fortnight schedule if type is Fortnight (backend expects 'Fortnight' not 'Fortnightly')
    if (data.scheduleType === 'Fortnightly' || data.scheduleType === 'Fortnight') {
  ...
  private mapScheduleType(type: string | undefined): string {
    switch(type) {
      case 'Daily':
        return 'Daily';
      case 'Weekly':
        return 'MultiplePerWeeks'; // Backend expects this for weekly
      case 'Fortnightly':
      case 'Fortnight':
        return 'Fortnight';
      case 'Monthly':
