...
class UOMTypesService {
  ...
  async getAllUOMTypes(): Promise<UOMType[]> {
    ...
    try {
      ...
      // Fallback to hardcoded common UOM types if API fails
      const fallbackTypes: UOMType[] = [
        { Id: 1, UID: "EA", Name: "Each", Label: "EA" },
        { Id: 2, UID: "KG", Name: "Kilogram", Label: "KG" },
        { Id: 3, UID: "G", Name: "Gram", Label: "G" },
        { Id: 4, UID: "L", Name: "Liter", Label: "L" },
        { Id: 5, UID: "ML", Name: "Milliliter", Label: "ML" },
        { Id: 6, UID: "PCS", Name: "Pieces", Label: "PCS" },
        { Id: 7, UID: "BOX", Name: "Box", Label: "BOX" },
        { Id: 8, UID: "CASE", Name: "Case", Label: "CASE" },
        { Id: 9, UID: "CTN", Name: "Carton", Label: "CTN" },
        { Id: 10, UID: "PALLET", Name: "Pallet", Label: "PALLET" }
      ];

      console.log(
        `⚠️ Using fallback UOM types (${fallbackTypes.length} types)`
      );
  ...
  async getBaseUOMTypes(): Promise<UOMType[]> {
    ...
    // Filter for common base unit types
  ...
  async getOuterUOMTypes(): Promise<UOMType[]> {
    ...
    // Filter for common packaging unit types
