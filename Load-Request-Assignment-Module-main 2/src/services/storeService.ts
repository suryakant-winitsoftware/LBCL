...
class StoreService {
   ...
   */
  async getAllStores(request: StoreListRequest): Promise<PagedResponse<IStore>> {
    try {
      ...
      if (response && typeof response === 'object') {
        ...
        if (response.Data && response.Data.PagedData !== undefined) {
          // Check if we got empty data from Redis cache issue
          if (response.Data.PagedData.length === 0 && response.Data.TotalCount === 0) {
            console.warn('Store API returned empty data - Redis cache may be empty');
            // Try to get at least one store to show the UI is working
            try {
              const sampleStore = await this.getStoreByUID('G54');
              if (sampleStore) {
                return {
                  pagedData: [sampleStore],
                  totalCount: 1
                };
              }
            } catch (e) {
              console.error('Failed to fetch sample store:', e);
            }
          }
          return {
  ...
  async getStoreByUID(uid: string): Promise<IStore> {
    try {
      console.log(`🔍 StoreService.getStoreByUID: Fetching store with UID=${uid}`);
      
      // Use SelectAllStore WITHOUT filters to avoid SQL bug, then filter client-side
      ...
      if (response.pagedData && response.pagedData.length > 0) {
        ...
        const store = response.pagedData.find(s => 
          (s.UID === uid) || (s.uid === uid) || (s.Code === uid) || (s.code === uid)
        );
        
        if (store) {
          console.log('✅ Found store:', store.Code, '-', store.Name);
          return store;
        } else {
          // If not found in first page, try getting more pages
          console.log('🔄 Store not found in first page, trying larger page size...');
          ...
          if (storeFromLargerSet) {
            console.log('✅ Found store in expanded search:', storeFromLargerSet.Code, '-', storeFromLargerSet.Name);
   ...
   * Uses authenticated API service with Bearer token
   */
  async updateStoreStatus(uid: string, isActive?: boolean, isBlocked?: boolean): Promise<number> {
    try {
      console.log(`🔄 Updating store ${uid} status: active=${isActive}, blocked=${isBlocked}`);
      ...
      try {
        ...
        console.warn('Could not fetch current store data, using minimal data:', error);
      }
      
      // Create minimal Store object with required fields
      const storeData = {
        UID: uid,
        IsActive: isActive !== undefined ? isActive : (currentStore.IsActive ?? currentStore.is_active ?? true),
        IsBlocked: isBlocked !== undefined ? isBlocked : (currentStore.IsBlocked ?? currentStore.is_blocked ?? false),
      ...
      const response = await apiService.put('/Store/UpdateStoreStatus', storeApprovalDTO);
  ...
  async createStoreMaster(storeMaster: IStoreMaster): Promise<number> {
    try {
      ...
      const response = await apiService.post('/Store/CreateStoreMaster', storeMaster);
      return response.data || response;
    } catch (error) {
  ...
  async getStoreMasterByUID(uid: string): Promise<IStoreMaster> {
    try {
      ...
      const response = await apiService.get(`/Store/SelectStoreMasterByUID?UID=${uid}`);
      console.log('🏠 StoreMaster API response:', response);
      ...
      if (storeMaster) {
        console.log('✅ StoreMaster retrieved successfully:', {
          storeUID: storeMaster.store?.UID || storeMaster.Store?.UID,
          contactsCount: storeMaster.contacts?.length || storeMaster.Contacts?.length || 0,
          addressesCount: storeMaster.addresses?.length || storeMaster.Addresses?.length || 0
        });
        ...
        return {
          ...
          storeCredits: storeMaster.storeCredits || storeMaster.StoreCredits || [],
          storeAdditionalInfo: storeMaster.storeAdditionalInfo || storeMaster.StoreAdditionalInfo
        };
      }
      
      throw new Error(`StoreMaster not found for UID: ${uid}`);
    } catch (error) {
  ...
  async updateStoreMaster(storeMaster: IStoreMaster): Promise<number> {
    try {
      ...
      const response = await apiService.put('/Store/UpdateStoreMaster', storeMaster);
      return response.data || response;
    } catch (error) {
  ...
  async getStoresByRouteUID(routeUID: string): Promise<IStoreCustomer[]> {
    try {
      const response = await apiService.get(`/Store/GetStoreCustomersByRouteUID?routeUID=${routeUID}`);
      return response.data || response;
    } catch (error) {
   ...
   * Uses authenticated API service with Bearer token
   */
  async getAllStoreItems(request: StoreListRequest, orgUID?: string): Promise<PagedResponse<IStoreCustomer>> {
    try {
      const url = orgUID ? `/Store/GetAllStoreItems?OrgUID=${orgUID}` : '/Store/GetAllStoreItems';
   ...
   * Uses authenticated API service with Bearer token
   */
  async getAllStoreAsSelectionItems(request: StoreListRequest, orgUID?: string): Promise<any> {
    try {
      const url = orgUID ? `/Store/GetAllStoreAsSelectionItems?OrgUID=${orgUID}` : '/Store/GetAllStoreAsSelectionItems';
      const response = await apiService.post(url, request);
      return response.data || response;
    } catch (error) {
  ...
  async exportStores(filters?: StoreSearchParams): Promise<Blob> {
    try {
      ...
      // Convert to CSV
      const headers = ['Code', 'Name', 'Type', 'Classification', 'Status', 'Active', 'Country', 'Region', 'City'];
   ...
   */
  async importStores(file: File): Promise<{ success: number; failed: number; errors: string[] }> {
    try {
   ...
   * Validate store data
   */
  validateStore(store: Partial<IStore>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  async getStoreContacts(storeUID: string): Promise<any[]> {
    try {
      ...
      const response = await apiService.post('/Contact/SelectAllContactDetails', request);
      ...
      for (let i = 0; i < filterStrategies.length; i++) {
        ...
        if (matches.length > 0) {
          ...
          console.log(`✅ Found ${matches.length} contacts using ${usedStrategy}`);
          break;
        }
      }
      ...
      if (storeContacts.length === 0) {
        console.log(`❌ NO CONTACTS found for store ${storeUID} using any strategy`);
        console.log('Available LinkedItemUIDs in first 20 contacts:', 
          allContacts.slice(0, 20).map(c => c.LinkedItemUID || c.linked_item_uid).filter(Boolean)
        );
      }
  ...
  async getStoreAddresses(storeUID: string): Promise<any[]> {
    try {
      ...
      const response = await apiService.post('/Address/SelectAllAddressDetails', request);
      ...
      for (let i = 0; i < addressFilterStrategies.length; i++) {
        ...
        if (matches.length > 0) {
          ...
          console.log(`✅ Found ${matches.length} addresses using ${usedAddressStrategy}`);
          break;
        }
      }
      ...
      if (storeAddresses.length === 0) {
        console.log(`❌ NO ADDRESSES found for store ${storeUID} using any strategy`);
        console.log('Available LinkedItemUIDs in first 20 addresses:', 
          allAddresses.slice(0, 20).map(a => a.LinkedItemUID || a.linked_item_uid).filter(Boolean)
        );
        console.log('Available UIDs in first 20 addresses:', 
          allAddresses.slice(0, 20).map(a => a.UID).filter(Boolean)
        );
      }
