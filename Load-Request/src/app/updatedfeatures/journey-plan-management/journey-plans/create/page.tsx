...
const progressSteps: StepConfig[] = [
  { num: 1, label: "Basic Setup", mobileLabel: "Setup", icon: Building2 },
  {
    num: 2,
...
export default function CreateJourneyPlanPage() {
  ...
  const [showAvailableRoutesOnly, setShowAvailableRoutesOnly] = useState(true); // Default to available only
  ...
  useEffect(() => {
    ...
    // Only filter routes if we're on step 1 (route selection)
    // Skip filtering on step 2 to avoid duplicate calls
    if (currentStep !== 1) {
      console.log("[DateChange] Not on step 1, skipping route filter");
      return;
    }
    ...
    return new Promise<void>((resolve) => {
      filterDebounceTimer.current = setTimeout(async () => {
        ...
        try {
          ...
          const existingPlansResponse = await api.beatHistory.selectAll({
            pageNumber: 1,
          ...
          if (
            existingPlansResponse?.IsSuccess &&
            existingPlansResponse?.Data?.PagedData
          ) {
            routesWithExistingPlans = existingPlansResponse.Data.PagedData.map(
              (plan: any) => plan.RouteUID
            );
            console.log(
  ...
  const loadRouteCustomers = async (routeUID: string) => {
    // Prevent duplicate calls for the same route
    if (lastLoadedCustomersRoute.current === routeUID) {
      console.log(
        "[Customers] Skipping duplicate load for routeUID:",
        routeUID
      );
      return;
    }

    lastLoadedCustomersRoute.current = routeUID;
    setDropdownsLoading((prev) => ({ ...prev, customers: true }));
    try {
      console.log("[Customers] Loading for routeUID:", routeUID);
      const startTime = Date.now();

      // Use optimized loader with caching
      const result = await journeyPlanDataLoader.loadRouteCustomers(routeUID);

      console.log(`Customers loaded in ${result.loadTime}ms`);
      ...
      // Show quick feedback if loaded from cache
      if (result.loadTime < 100) {
        // Data was instant from cache, no need for loading feedback
      } else if (result.loadTime > 1500) {
        toast({
          title: "Customers loaded",
          description: `${result.data.length} customers found`,
          duration: 1500
        });
      }
    } catch (error) {
      ...
      if (isMultipleEmployees) {
          ...
          try {
            ...
            if (response?.IsSuccess) {
              ...
              if (plan.PlannedStores?.length > 0) {
                ...
                await api.beatHistory.createBulkStoreHistories({
                  storeHistories
                });
              }
            } else {
        ...
        try {
          ...
          if (values.selectedCustomersWithTimes?.length > 0) {
            ...
            console.log(
              `[Submit] Creating ${storeHistories.length} store histories`
            );
            const storeResponse =
              await api.beatHistory.createBulkStoreHistories({
                storeHistories
            ...
            if (!storeResponse?.IsSuccess) {
              console.warn("Failed to create store histories:", storeResponse);
            }
          }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold">Create Journey Plan</h1>
