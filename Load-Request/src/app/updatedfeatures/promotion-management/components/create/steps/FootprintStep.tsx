...
export default function FootprintStep({
  ...
  const loadStores = useCallback(async (orgUID: string, append: boolean = false, currentCount: number = 0) => {
    ...
    try {
      ...
      const data: any = await api.store.selectAll(requestBody);
      console.log(`Store API response for org ${orgUID} (page ${requestBody.pageNumber}):`, data);
      
      if (data.IsSuccess && data.Data?.PagedData) {
        console.log('First store in chunk:', data.Data.PagedData[0]);
        const stores: Store[] = data.Data.PagedData.map((store: {
          UID?: string;
          uid?: string;
          Uid?: string;
          Name?: string;
        ...
        const totalCount = data.Data?.TotalCount || stores.length;
        ...
        startTransition(() => {
          ...
          setStoreError(data.Message || "No stores found for this organization");
        });
      }
    } catch (error) {
      setAvailableStores([]);
  ...
  const loadAllStores = useCallback(async () => {
    ...
    try {
      ...
      if (data.IsSuccess && data.Data?.PagedData) {
        ...
        setAvailableStores([]);
        setStoreError(data.Message || "No stores found");
      }
    } catch (error) {
      setAvailableStores([]);
      setStoreError(`Error loading stores: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    const prefetchStores = async () => {
      ...
      if (!cached || Date.now() - cached.timestamp >= CACHE_DURATION) {
        try {
          ...
          if (data.IsSuccess && data.Data?.PagedData) {
            const stores: Store[] = data.Data.PagedData.map((store: any) => ({
              uid: store.UID || store.uid || store.Uid,
              name: store.Name || store.name,
              code: store.Code || store.code,
              address: store.Address || store.address,
              ...
              >
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-gray-900">All Organizations</span>
                  </div>
        ...
        <div className="border border-gray-200 rounded-lg p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-gray-900 mb-1">All Organizations Coverage Selected</h4>
