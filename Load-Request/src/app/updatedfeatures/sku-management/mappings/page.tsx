...
import { Plus, Edit, Trash2, Filter, Download, Upload, Link2, Search } from 'lucide-react'
...
export default function SKUMappingsPage() {
  ...
  const mappingStats = groupTypes.map(gt => ({
    ...gt,
    count: mappings.filter(m => {
      const group = groups.find(g => g.UID === m.SKUGroupUID)
      return group?.SkuGroupTypeUID === gt.UID
    }).length
  }))
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">SKU to Group Mappings</h1>
          <p className="text-muted-foreground">
            Manage relationships between SKUs and groups for classification
          </p>
        </div>
        <div className="flex gap-2">
      ...
      <Card>
        ...
        <CardContent className="flex gap-4">
          <div className="flex-1">
            ...
            <Input
              id="search"
              placeholder="Search by SKU or group name..."
              value={searchTerm}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? 'Edit SKU Mapping' : 'Create SKU Mapping'}
            </DialogTitle>
            <DialogDescription>
              {editingItem ? 'Update the mapping details' : 'Map a SKU to a group for classification'}
            </DialogDescription>
          </DialogHeader>
