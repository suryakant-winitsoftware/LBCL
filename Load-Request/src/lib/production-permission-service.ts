...
class ProductionPermissionService {
  ...
  private hasLoggedFirstModule: boolean = false;
  ...
  private normalizeModule = (module: unknown): Module => {
    ...
    // Debug: Log only first module to avoid clutter
    if (!this.hasLoggedFirstModule) {
      console.log("🔍 Raw module data:", mod);
      console.log("🔍 showInMenu raw values:", {
        showInMenu: mod.showInMenu,
        ShowInMenu: mod.ShowInMenu,
        show_in_menu: mod.show_in_menu
      });
      this.hasLoggedFirstModule = true;
    }
    return {
  ...
  buildMenuHierarchy(
    ...
    try {
      ...
      console.log("🔍 Building menu hierarchy:", {
        totalModules: modules.length,
        totalSubModules: subModules.length,
        totalSubSubModules: subSubModules.length,
        totalPermissions: permissions.length,
        samplePermission: permissions[0]
      });

      // Log first few modules to see their showInMenu values
      if (modules.length > 0) {
        console.log(
          "🔍 First 3 modules:",
      ...
      console.log(
        ...
        [...permissionMap.entries()].slice(0, 3)
      );
      ...
      const menuModules: Module[] = modules
        .filter((m) => {
          ...
          console.log(
            `Module ${m?.uid}: showInMenu=${m?.showInMenu}, included=${show}`
          );
          return show;
        })
        ...
        .map((module) => {
          try {
            ...
            const moduleSubModules = subModules
              ...
              .map((subModule) => {
                try {
                  ...
                  // console.log(`🔍 SubModule ${subModule.uid} - Processing ${filteredPages.length} pages`)
                  ...
                  const subModulePages = filteredPages.filter((ssm) => {
                    ...
                    if (!permission) {
                      ...
                      console.log(`❌ No permission for page: ${ssm.uid}`);
                      return false;
                    }
                    ...
                    console.log(
                      `Page ${ssm.uid}: hasAccess=${hasAccess}, permission:`,
                      {
                  ...
                  // console.log(`🔍 SubModule ${subModule.uid} - ${subModulePages.length} pages with access`)
            ...
            console.log(
              `Module ${module.uid}: subModules=${moduleSubModules.length}, permissions=${modulePermissions.length}`
            );

            // Only show module if it has accessible sub-modules with permissions
            if (moduleSubModules.length > 0) {
              console.log(
                `✅ Including module ${module.uid} with ${moduleSubModules.length} sub-modules`
              );
              return {
                ...module,
              ...
              console.log(
                `✅ Including module ${module.uid} with direct access`
              );
              return {
                ...module,
                children: [],
              ...
              console.log(
                `❌ Hiding module ${module.uid} - no accessible content`
              );
              return null;
            }
          } catch (error) {
