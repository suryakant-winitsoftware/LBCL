...
class EnterpriseSkuService {
   ...
   * Uses ISKUV1 model and integrates all enterprise systems
   */
  async createEnterpriseSKU(
    ...
    const iskuV1Payload: ISKUV1 = {
      ...
      // Hierarchy levels from attributes (L1-L6)
      L1:
        request.attributes.length > 0 ? request.attributes[0]?.code || "" : "",
      L2:
        request.attributes.length > 1 ? request.attributes[1]?.code || "" : "",
      L3:
        request.attributes.length > 2 ? request.attributes[2]?.code || "" : "",
      L4:
        request.attributes.length > 3 ? request.attributes[3]?.code || "" : "",
      L5:
        request.attributes.length > 4 ? request.attributes[4]?.code || "" : "",
      L6:
        request.attributes.length > 5 ? request.attributes[5]?.code || "" : "",

      // Parent relationship
      ParentUID: finalCascadeSelection?.code || "",

      // Extended data fields
      Category:
        request.attributes.find((attr) => attr.type === "Category")?.value ||
        "",
      Year: request.skuData.Year || new Date().getFullYear(),
      ProductCategoryId: request.skuData.ProductCategoryId || 0,
      IsAvailableInApMaster: false,
      FilterKeys: request.skuData.FilterKeys || []
    };
