...
class FullyDynamicUOMService {
  ...
  private _schemaCache: Map<string, DynamicTableSchema> = new Map();
  private _dataCache: Map<string, any[]> = new Map();
  private _metadataCache: Map<string, Record<string, any>> = new Map();
  ...
  async getTableSchema(tableName: string): Promise<DynamicTableSchema> {
    ...
    if (
      this._schemaCache.has(cacheKey) &&
  ...
  private async inferSchemaFromSampleData(
    ...
    // For SKUUOM, use the specific API endpoint we know works
    let sampleData: any[] = [];
    if (tableName.toUpperCase() === "SKUUOM") {
      sampleData = await this.getAllUOMData();
  ...
  private async getRawTableData(
    ...
    const cacheKey = `${tableName}_${pageNumber}_${pageSize}`;
    ...
    for (const endpoint of endpointPatterns) {
      try {
        ...
        if (response) {
          this._dataCache.set(cacheKey, response);
          return response;
        }
      } catch (error) {
  ...
  private clearCacheForTable(tableName: string): void {
    const keysToDelete = Array.from(this._dataCache.keys()).filter((key) =>
      key.startsWith(tableName)
    );
    keysToDelete.forEach((key) => this._dataCache.delete(key));

    this._schemaCache.delete(tableName.toLowerCase());
  }
  ...
  async getSystemInfo(): Promise<Record<string, any>> {
    return {
      tablesDiscovered: Array.from(this._schemaCache.keys()),
      cacheSize: this._dataCache.size,
      lastUpdate: new Date(this._cacheTimestamp).toISOString(),
      systemCapabilities: {
