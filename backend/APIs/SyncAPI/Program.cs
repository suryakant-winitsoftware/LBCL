...
builder.Services.AddSwaggerGen(c =>
{
    ...
    {
        Title = "SalesOrderSync API",                     // API title displayed in Swagger UI.
...
builder.Services.AddMassTransit(x =>
{
    ...
    x.UsingRabbitMq((context, cfg) =>
    {
        ...
        // For Topic exchange
        cfg.Message<IAppRequest>(config =>
        {
            config.SetEntityName(ExchangeNames.AppRequestExchange); // Use the same exchange name
        });
        cfg.Message<AppRequest>(config =>
        {
            config.SetEntityName(ExchangeNames.AppRequestExchange); // Use the same exchange name
        });
        cfg.Publish<IAppRequest>(publishConfig =>
        {
        ...
        cfg.Publish<AppRequest>(publishConfig =>
        {
            publishConfig.AutoDelete = false; // Ensure exchange persists
            publishConfig.Durable = true;     // Make the exchange durable
            publishConfig.ExchangeType = ExchangeTypes.Topic;
