...
{
    public class PGSQLApprovalEngineDL : Base.DL.DBManager.PostgresDBManager, IApprovalEngineDL
    {

        public PGSQLApprovalEngineDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        { }
        public async Task<IEnumerable<Model.Interfaces.IApprovalLog>> GetApprovalLog(string RequestId)
        {
            try
            {
                // Convert string to long for bigint comparison in PostgreSQL
                long requestIdLong = long.TryParse(RequestId, out long parsed) ? parsed : 0;
        ...
        public async Task<IEnumerable<IApprovalHierarchy>> GetApprovalHierarchy(string ruleId)
        {
            try
            {
                // Convert string to int for integer comparison in PostgreSQL
                int ruleIdInt = int.TryParse(ruleId, out int parsed) ? parsed : 0;
        ...
        public async Task<int> UpdateChangesInMainTable(IViewChangeRequestApproval? viewChangeRequestApproval)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"DEBUG ScreenModelName: '{changeRecordDTO.ScreenModelName}'");
        ...
        private async Task<int> UpdateRecord(ChangeRecordDTO changeRecordDTO, string tableName)
        {
            try
            {
                ...
                foreach (var item in changeRecordDTO.ChangeRecords)
                {
                    ...
                    Console.WriteLine($"DEBUG SQL: {updateQuery}");
                    Console.WriteLine($"DEBUG Params: FieldValue={item.NewValue}, UID={changeRecordDTO.UID}");
