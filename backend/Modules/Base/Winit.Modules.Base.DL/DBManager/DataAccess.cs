...
{
    public abstract class DataAccess : DbUtil
    {
        protected sFAModel.BaseModel _basemodel;
        ...
        public DataAccess(bool transaction, int? timeout)
        {
            _withtransaction = transaction;
            _timeout = timeout;
            //_connectionString = $"URI=file:{Path.Combine(_baseFolderPath, SqliteFileName)}";
        }
        public DataAccess(bool transaction) : this(transaction, null) { }
        public DataAccess() : this(false) { }
        public DataAccess(sFAModel.BaseModel model) : this()
        {
            _basemodel = model;
        }
        public DataAccess(sFAModel.BaseModel model, bool transaction) : this(transaction)
        {
            _basemodel = model;
        }
        public DataAccess(sFAModel.BaseModel model, bool transaction, int? timeout) : this(transaction, timeout)
        {
            _basemodel = model;
        }
        ...
        protected DataSet GetDataSet1(string query, DbParam[] param)
        {
            lock (_lock)
            {
        ...
        public DataSet GetDataSet(string query, DbParam[] param)
        {
            lock (_lock)
            {
                var con = GetConnection();
