...
namespace Winit.Modules.Base.DL.DBManager
{
    ...
    {
        public SqlServerDBManager(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config, ConnectionStringName.SqlServer)
        {
        }
        ...
        /// <summary>
        /// To execute procedure by prem
        /// </summary>
        /// <param name="sql"></param>
        /// <param name="parameters"></param>
        /// <returns></returns>
        public async Task<int> ExecuteProcedureAsync(string procedure, object? parameters = null)
        {
            using SqlConnection connection = new(_connectionString);
            return await connection.ExecuteAsync(procedure, parameters, commandType: System.Data.CommandType.StoredProcedure);
        }
        ...
        public async Task<T?> ExecuteScalarAsync<T>(string sql, IDbConnection? connection = null, object? parameters = null)
        {
            if(connection == null)
        ...
        public async Task<bool> CheckIfAnyRecordExistsInDBWithAnyColumn(string tableName, string columnName, string value, IDbConnection? connection = null, IDbTransaction? transaction = null)
        {
            try
            {
                var parameters = new { value = value };
                string sql = $@" SELECT CASE WHEN EXISTS (SELECT 1 FROM {tableName} WHERE  {columnName} = @value ) THEN 1 ELSE 0 END; ";
        ...
        public void AppendFilterCriteria<T>(List<FilterCriteria> filterCriterias, StringBuilder sql, Dictionary<string, object?> parameters) where T : class
        {
            for (int i = 0; i < filterCriterias.Count; i++)
            {
                ...
                if (parsedValue == null || (parsedValue is string && filterCriteria.DataType is not null))
                {
                    try
                    {
                        parsedValue = Convert.ChangeType(filterCriteria.Value, filterCriteria.DataType);
                    }
                    catch (Exception) { }
                }
                ...
                switch (filterCriteria.Type)
                {
                    ...
                    case FilterType.GreaterThanOrEqual or FilterType.LessThanOrEqual:
                        AppendLessOrGreaterThanEqualCondition(sql, paramName, filterCriteria.Name, parsedValue, filterCriteria.Value, parameters, filterCriteria.Type);
                        break;
        ...
        {
            PropertyInfo? propertyInfo = _serviceProvider
        ...
        private void AppendInConditionForArray(StringBuilder sql, string paramName, string columnName, Array rawValue, Type dataType, Dictionary<string, object> parameters)
        {
            ...
            foreach (object? value in rawValue)
            {
                ...
                inParam.Add(Convert.ChangeType(value, dataType));
                //parameters.Add(paramKey, parsedValue);
            }
            //rawValue.
            //sql.Append($"{columnName} IN ({string.Join(", ", inParam)})");
            sql.Append($"{columnName} IN @{columnName + 1}");
            parameters.Add($"{columnName + 1}", inParam);
        }
        ...
        //    }
        //}
        //public async Task<int> ExecuteNonQueryAsync(string sql, SqlConnection connection, SqlTransaction transaction, IDictionary<string, object> parameters = null)
        //{
        ...
        protected async Task Query<T>(Func<IDbConnection, Task> func)
        {
            try
            {
                using (IDbConnection connection = new SqlConnection(_connectionString))
