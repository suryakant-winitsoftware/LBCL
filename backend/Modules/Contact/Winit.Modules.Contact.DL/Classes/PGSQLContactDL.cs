...
{
    public class PGSQLContactDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IContactDL
    {
        public PGSQLContactDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
            ...
            try
            {
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"SELECT COUNT(1) AS Cnt FROM(select id AS Id,
                                                                        ...
                                                                        enabled_for_promo_email AS EnabledForPromoEmail,
                                                                        is_email_cc AS IsEmailCC,
                                                                        mobile2 AS Mobile2 from contact) as SubQuery");
                }
