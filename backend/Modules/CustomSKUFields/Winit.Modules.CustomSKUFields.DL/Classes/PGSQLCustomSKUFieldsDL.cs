...
{
    public class PGSQLCustomSKUFieldsDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ICustomSKUFieldsDL
    {
        Winit.Shared.CommonUtilities.Common.CommonFunctions commonFunctions;
        private readonly ApiSettings _apiSettings;
        public PGSQLCustomSKUFieldsDL(IServiceProvider serviceProvider, IConfiguration config, IOptions<ApiSettings> apiSettings, Winit.Shared.CommonUtilities.Common.CommonFunctions commonFunctions) : base(serviceProvider, config)
        {
            ...
            try
            {
                ...
                var sql = @"
                        ...
                        VALUES (
                            ...
                            @ServerModifiedTime, @SKUUID, @CustomField::json
                        );";
        ...
        public async Task<int> CUDCustomSKUFields(Winit.Modules.CustomSKUField.Model.Classes.CustomSKUField customSKUField)
        {
            try
            {
                var existingRec = await SelectCustomSKUFieldsByUID(customSKUField.SKUUID);

                if (existingRec != null && existingRec.SKUUID == customSKUField.SKUUID)
                {
        ...
        private async Task<Winit.Modules.CustomSKUField.Model.Interfaces.ICustomSKUFields> SelectCustomSKUFieldsByUID(string UID)
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>
            {
                {"UID",  UID}
            ...
            var sql = @"SELECT 
                                ...
                                FROM 
                                    custom_sku_fields
                                 WHERE uid = @UID";
            Type type = _serviceProvider.GetRequiredService<Winit.Modules.CustomSKUField.Model.Interfaces.ICustomSKUFields>().GetType();
            Winit.Modules.CustomSKUField.Model.Interfaces.ICustomSKUFields whStockRequestDetails = await ExecuteSingleAsync<Winit.Modules.CustomSKUField.Model.Interfaces.ICustomSKUFields>(sql, parameters, type);
        ...
        public async Task<IEnumerable<Winit.Modules.CustomSKUField.Model.Classes.CustomField>> SelectAllCustomFieldsDetails(string SKUUID)
        {
            try
            {
                IEnumerable<Model.Classes.CustomField> jsonCustomSKUFieldsList = commonFunctions.ReadJsonFile<Model.Classes.CustomField>(_apiSettings.SKUCustomFields);
                ...
                Dictionary<string, object> parameters = new Dictionary<string, object>()
        {
            {"SKUUID", SKUUID }
                ...
                Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ICustomSKUFields>().GetType();
                IEnumerable<Model.Interfaces.ICustomSKUFields> lstActualCustomFields = await ExecuteQueryAsync<Model.Interfaces.ICustomSKUFields>(sql.ToString(), parameters, type);
