...
public class MSSQLDashBoardDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IDashBoardDL
{
    protected Winit.Shared.CommonUtilities.Common.CommonFunctions _commonFunctions;


    public MSSQLDashBoardDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
    {
    }
            ...
            int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired, bool isForExport)
    {
        ...
        try
        {
            ...
            var count = await ExecuteScalarAsync<int>(sqlCount.ToString(), connection: connection, parameters);
