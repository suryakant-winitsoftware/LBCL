...
{
    public class PGSQLDIstributorDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IDistributorDL
    {
        IStoreBL _storeBL;
        ...
        public PGSQLDIstributorDL(IServiceProvider serviceProvider, IConfiguration config, IStoreBL storeBL, IStoreAdditionalInfoBL additionalInfoBL,
             ...
             int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                DataTable dataTable = await ExecuteQueryDataTableAsync(sql.ToString(), parameters);
                List<Winit.Modules.Distributor.Model.Classes.Distributor> distributors = new();
                if (dataTable != null)
                {
                    if (dataTable.Rows.Count > 0)
                    {
                        ...
                        {
                            distributors.Add(ConvertDataTableToObject<Winit.Modules.Distributor.Model.Classes.Distributor>(row));
                        }
                    }
                }
                //Count
        ...
        public async Task<int> CreateDistributor(Winit.Modules.Distributor.Model.Classes.DistributorMasterView distributorMasterView)
        {
            ...
            //Store
            var store = await _storeBL.SelectStoreByUID(distributorMasterView.Store.UID);
            if(store==null) 
            ...
            var StoreAddinfo =await _additionalInfoBL.SelectStoreAdditionalInfoByUID(distributorMasterView.StoreAdditionalInfo.UID);
            if(StoreAddinfo==null)
            count += await _additionalInfoBL.CreateStoreAdditionalInfo(distributorMasterView.StoreAdditionalInfo);
            else
            count += await _additionalInfoBL.UpdateStoreAdditionalInfo(distributorMasterView.StoreAdditionalInfo);

            //Store Credit
            var storeCredit=await _creditBL.SelectStoreCreditByUID(distributorMasterView.StoreCredit.UID);
            if(storeCredit==null)
            ...
            {
                new FilterCriteria("LinkedItemUID",distributorMasterView.Store.UID,FilterType.Equal),
            };

            var ContactList = await _contactBL.SelectAllContactDetails(null, 0, 0, filterCriterias, false);
            foreach (var contact in distributorMasterView.Contacts)
            {
                count1 += await _contactBL.CreateContactDetails(contact);
            }
            foreach (var document in distributorMasterView.Documents)
            {
                count2 += await _storeDocumentBL.CreateStoreDocumentDetails(document);
            }
