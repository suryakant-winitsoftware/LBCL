...
{
    public class MSSQLEmpInfoDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IEmpInfoDL
    {
        public MSSQLEmpInfoDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                IEnumerable<Winit.Modules.Emp.Model.Interfaces.IEmpInfo> empInfoDetails = await ExecuteQueryAsync<Winit.Modules.Emp.Model.Interfaces.IEmpInfo>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
