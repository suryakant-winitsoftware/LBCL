...
{
    public class MSSQLEmpOrgMappingDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IEmpOrgMappingDL
    {
        public MSSQLEmpOrgMappingDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
         ...
         int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"select * from(select id,uid,created_by as CreatedBy,created_time as CreatedTime,
                                            modified_by as ModifiedBy, modified_time as ModifiedTime,server_add_time as ServerAddTime,
                                            server_modified_time as ServerModifiedTime,emp_uid as EmpUID,org_uid as OrgUID from emp_org_mapping)as subquery");
                var sqlCount = new StringBuilder();
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"select count(1) AS Cnt from (select id,uid,created_by as CreatedBy,created_time as CreatedTime,
                                            modified_by as ModifiedBy, modified_time as ModifiedTime,server_add_time as ServerAddTime,
                                            server_modified_time as ServerModifiedTime,emp_uid as EmpUID,org_uid as OrgUID from emp_org_mapping)as subquery");
                }
                ...
                IEnumerable<Winit.Modules.Emp.Model.Interfaces.IEmpOrgMapping> empOrgMappingDetails = await ExecuteQueryAsync<Winit.Modules.Emp.Model.Interfaces.IEmpOrgMapping>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
        ...
        public async Task<int> CreateEmpOrgMapping(List<Winit.Modules.Emp.Model.Classes.EmpOrgMapping> empOrgMappings)
        {
            ...
            try
            {
                    ...
                    retVal= await ExecuteNonQueryAsync(sql, empOrgMappings);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<IEnumerable<Winit.Modules.Emp.Model.Interfaces.IEmpOrgMapping>> GetEmpOrgMappingDetailsByEmpUID(string empUID)
        {
            try
            {
                var sql = new StringBuilder(@"select id,uid,created_by as CreatedBy,created_time as CreatedTime,
                                            modified_by as ModifiedBy, modified_time as ModifiedTime,server_add_time as ServerAddTime,
                                            server_modified_time as ServerModifiedTime,emp_uid as EmpUID,org_uid as OrgUID from 
                                            emp_org_mapping where emp_uid=@Empuid");
                var parameters = new Dictionary<string, object>() {
                    {"Empuid",empUID }
                };
               IEnumerable< Winit.Modules.Emp.Model.Interfaces.IEmpOrgMapping> empOrgMappings = await ExecuteQueryAsync<Winit.Modules.Emp.Model.Interfaces.IEmpOrgMapping>(sql.ToString(), parameters);
