...
{
    public class MSSQLFileSysDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IFileSysDL
    {
        public MSSQLFileSysDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        ...
        public async Task<int> CUDFileSys(Winit.Modules.FileSys.Model.Interfaces.IFileSys fileSys)
        {
            ...
            try
            {
                ...
                if (existingRec != null)
                {
                    exists = (existingRec.LinkedItemUID == fileSys.LinkedItemUID);
                }
