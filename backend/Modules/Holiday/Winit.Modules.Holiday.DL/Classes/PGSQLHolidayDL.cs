...
{
    public class PGSQLHolidayDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IHolidayDL
    {
        public PGSQLHolidayDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"SELECT COUNT(1) AS Cnt FROM holiday");
                }
...
FROM 
 ...
 WHERE uid = @UID";
        ...
        public async Task<int> CreateHoliday(Winit.Modules.Holiday.Model.Interfaces.IHoliday createHoliday)
        {
            try
            {
                var sql = @"INSERT INTO holiday (uid, holiday_list_uid, holiday_date, type, name, is_optional, year, created_by, created_time, modified_by, modified_time, server_add_time, server_modified_time) 
                            VALUES (@UID, @HolidayListUID, @HolidayDate, @Type, @Name, @IsOptional, @Year, @CreatedBy, @CreatedTime, @ModifiedBy, @ModifiedTime, @ServerAddTime, @ServerModifiedTime);";
                Dictionary<string, object> parameters = new Dictionary<string, object>
                {
        ...
        public async Task<int> UpdateHoliday(Winit.Modules.Holiday.Model.Interfaces.IHoliday updateHoliday)
        {
            try
            {
                var sql = @"UPDATE holiday 
                            SET holiday_date = @HolidayDate, type = @Type, name = @Name, is_optional = @IsOptional, year = @Year, ss = @SS, modified_by = @ModifiedBy,
                            modified_time = @ModifiedTime, server_modified_time = @ServerModifiedTime WHERE uid = @UID;";
                Dictionary<string, object> parameters = new Dictionary<string, object>
                {
