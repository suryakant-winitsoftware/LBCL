...
{
    public class PGSQLHolidayListDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IHolidayListDL
    {
        public PGSQLHolidayListDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"SELECT COUNT(1) AS Cnt FROM holiday_list");
                }
        ...
        public async Task<int> CreateHolidayList(Winit.Modules.Holiday.Model.Interfaces.IHolidayList createHolidayList)
        {
            try
            {
                var sql = @"INSERT INTO holiday_list 
                     (uid, company_uid, org_uid, name, description, location_uid, is_active, year, ss, created_by, created_time, modified_by, modified_time, server_add_time, server_modified_time) 
                     VALUES (@UID, @CompanyUID, @OrgUID, @Name, @Description, @LocationUID, @IsActive, @Year, @SS, @CreatedBy, @CreatedTime, @ModifiedBy, @ModifiedTime, @ServerAddTime, @ServerModifiedTime);";
                Dictionary<string, object> parameters = new Dictionary<string, object>
                {
