...
{
    public class MSSQLLocationTemplateDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, ILocationTemplateDL
    {
        IServiceProvider _serviceProvider;
        public MSSQLLocationTemplateDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        ...
        public async Task<int> UpdateLocationTemplate(ILocationTemplate locationTemplate)
        {
            string sql = @"UPDATE location_template
                   SET 
                       ...
                       is_active = @IsActive,
                       location_template_data = @LocationTemplateData
                   WHERE uid = @Uid";
        ...
        public async Task<int> CUDLocationTemplate(Winit.Modules.Location.Model.Interfaces.ILocationTemplate locationTemplate)
        {
            ...
            try
            {
                string? isExists = await CheckIfUIDExistsInDB(DbTableName.LocationTemplate, locationTemplate.UID);
        ...
        public async Task<int> CUDLocationTemplateLine(List<Winit.Modules.Location.Model.Interfaces.ILocationTemplateLine> locationTemplateLines)
        {
            ...
            try
            {
                List<string>? existingUIDs = await CheckIfUIDExistsInDB(DbTableName.LocationTemplateLine, uidList);
                ...
                if (existingLocationTemplateLines != null && existingLocationTemplateLines.Any())
                {
                    count = await UpdateLoactionTemplateLine(existingLocationTemplateLines);
                }
                if (newLocationTemplateLines!=null && newLocationTemplateLines.Any())
                {
        ...
        public async Task<int> DeleteLocationTemplateLines(List<string> uIDs)
        {
            try
            {
                ...
                var sql = @"DELETE FROM location_template_line WHERE uid IN @UIDs;";
