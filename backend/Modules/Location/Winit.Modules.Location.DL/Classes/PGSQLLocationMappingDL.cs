...
{
    public class PGSQLLocationMappingDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ILocationMappingDL
    {
        public PGSQLLocationMappingDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT * FROM (SELECT 
                                                    ...
                                                    FROM public.location_mapping) as SubQuery");
        ...
        public async Task<Winit.Modules.Location.Model.Interfaces.ILocationMapping> GetLocationMappingByUID(string UID)
        {
            ...
            var sql = @"SELECT 
                            ...
                            location_type_uid AS LocationTypeUid,
                            location_uid AS LocationUid
                        FROM public.location_mapping WHERE UID = @UID";
            Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ILocationMapping>().GetType();
        ...
        public async Task<int> CreateLocationMapping(Winit.Modules.Location.Model.Interfaces.ILocationMapping createLocationMapping)
        {
            try
            {
                var sql = @"INSERT INTO public.location_mapping (
                                                        id, 
                                                        uid, 
                                                        created_by, 
                                                        created_time, 
        ...
        public async Task<int> InsertLocationHierarchy(string type, string uid)
        {
            ...
            try
            {
                /*string settingSql = "SELECT l.uid FROM setting s " +
                    "INNER JOIN location_type lt on s.value = lt.uid " +
                    "INNER JOIN location l on l.location_type_uid = lt.uid AND l.uid = @UID " +
                    "WHERE s.name = 'LastLocationType' AND l;";
                string? primaryUid = await ExecuteScalarAsync<string?>(settingSql);*/

                string hierarchySql = @"
                    WITH RECURSIVE location_hierarchy AS (
                    SELECT l1.id AS Id, l1.uid AS UID, l1.created_by AS CreatedBy, l1.created_time AS CreatedTime, l1.modified_by as ModifiedBy,
                    l1.modified_time AS ModifiedTime, l1.server_add_time AS ServerAddTime, l1.server_modified_time AS ServerModifiedTime,
                    l1.company_uid AS CompanyUID, l1.location_type_uid AS LocationTypeUID, l1.code AS LocationCode, l1.name As LocationName, 
                    l1.parent_uid AS LocationParentUID, l1.item_level AS ItemLevel, lt1.name AS LocationTypeName, lt1.code AS LocationTypeCode, 
                    lt1.parent_uid AS LocationTypeParentUID, lt1.level_no AS LevelNo
                    FROM location l1
                    INNER JOIN location_type lt1 on l1.location_type_uid = lt1.uid
                    WHERE l1.uid = @UID AND lt1.name = @Type
                    UNION ALL
                    SELECT l2.id AS Id, l2.uid AS UID, l2.created_by AS CreatedBy, l2.created_time AS CreatedTime, l2.modified_by AS ModifiedBy,
                    l2.modified_time AS ModifiedTime, l2.server_add_time AS ServerAddTime, l2.server_modified_time AS ServerModifiedTime,
                    l2.company_uid AS CompanyUID, l2.location_type_uid AS LocationTypeUID, l2.code AS LocationCode, l2.name AS LocationName, 
                    l2.parent_uid AS LocationParentUID, l2.item_level AS ItemLevel, lt2.name AS LocationTypeName, lt2.code AS LocationTypeCode, 
                    lt2.parent_uid AS LocationTypeParentUID, lt2.level_no AS LevelNo
                    FROM location l2
                    INNER JOIN location_type lt2 ON lt2.uid = l2.location_type_uid AND lt2.show_in_ui = true
...
Primary_Label as PrimaryLabel
            ...
            Type type = _serviceProvider.GetRequiredService<ILocationData>().GetType();

            List<ILocationData> LocationData = await ExecuteQueryAsync<ILocationData>(sql.ToString(), null, type);
