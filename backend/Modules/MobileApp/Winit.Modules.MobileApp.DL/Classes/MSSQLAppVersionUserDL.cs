...
{
    public class MSSQLAppVersionUserDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IAppVersionUserDL
    {
        public MSSQLAppVersionUserDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
...
");
                ...
                IEnumerable<Winit.Modules.Mobile.Model.Interfaces.IAppVersionUser> appVersionUserList = await ExecuteQueryAsync<Winit.Modules.Mobile.Model.Interfaces.IAppVersionUser>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
        ...
        public async Task<Winit.Modules.Mobile.Model.Interfaces.IAppVersionUser> GetAppVersionDetailsByUID(string UID)
        {
            try
            {
                ...
                Winit.Modules.Mobile.Model.Interfaces.IAppVersionUser? appVersionUser = await ExecuteSingleAsync<Winit.Modules.Mobile.Model.Interfaces.IAppVersionUser>(sql.ToString(), parameters);
        ...
        public async Task<int> InsertAppVersionUser(Winit.Modules.Mobile.Model.Classes.AppVersionUser appVersionUser)
        {
            try
            {
                ...
                var parameters = new Dictionary<string, object>
                {
                    ...
                    { "@DeviceId", appVersionUser.DeviceId },
                    { "@AppVersion", appVersionUser.AppVersion ?? string.Empty },
                    { "@AppVersionNumber", appVersionUser.AppVersionNumber },
                    { "@ApiVersion", appVersionUser.ApiVersion ?? (object)DBNull.Value },
                    { "@DeploymentDateTime", appVersionUser.DeploymentDateTime == default ? (object)DBNull.Value : appVersionUser.DeploymentDateTime },
                    { "@NextAppVersion", appVersionUser.NextAppVersion ?? (object)DBNull.Value },
                    { "@NextAppVersionNumber", appVersionUser.NextAppVersionNumber },
                    { "@PublishDate", appVersionUser.PublishDate == default ? (object)DBNull.Value : appVersionUser.PublishDate },
                    { "@IsTest", appVersionUser.IsTest ? 1 : 0 },
                    { "@IMEINo", appVersionUser.IMEINo ?? (object)DBNull.Value },
                    { "@OrgUID", appVersionUser.OrgUID ?? string.Empty },
                    { "@GcmKey", appVersionUser.GcmKey ?? string.Empty },
                    { "@IMEINo2", string.Empty }, // IMEINo2 field from specification
                    { "@CreatedDate", appVersionUser.CreatedTime },
                    { "@CreatedBy", appVersionUser.CreatedBy ?? "System" },
