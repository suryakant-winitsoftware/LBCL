...
{
    public class MSSQLOrgDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IOrgDL
    {
        public MSSQLOrgDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        public async Task<Winit.Modules.Org.Model.Interfaces.IOrg> GetOrgByUID(string UID)
        {
            ...
            Winit.Modules.Org.Model.Interfaces.IOrg? orgDetails = await ExecuteSingleAsync<Winit.Modules.Org.Model.Interfaces.IOrg>(sql, parameters);
            return orgDetails;
        }
                ...
                string hierarchySql = """
                                       ...
                                       WITH org_hierarchy_cte AS (
                                           ...
                                           SELECT 
                                               ...
                                               po.modified_by
                                           FROM 
                                               org_hierarchy_cte cp
                                           INNER JOIN 
                                               org po ON po.uid = cp.parent_uid 
                                       )

                                       -- Insert into the org_hierarchy table
                                       ...
                                       SELECT 
                                           NEWID(), -- Generate a new UUID
                                           d.uid, 
                                           d.parent_uid, 
                                           d.level,  
                                           d.created_by,
                                           d.modified_by,
                                           GETDATE() AS created_time, -- Current date and time
                                           GETDATE() AS modified_time, 
                                           GETDATE() AS server_add_time, 
                                           GETDATE() AS server_modified_time
                                       FROM 
                                           (SELECT DISTINCT uid, parent_uid, level, created_by, modified_by 
                                            FROM org_hierarchy_cte 
                                            WHERE parent_uid IS NOT NULL) AS d
                                       ORDER BY 
                                           d.uid, d.level;
        ...
        public async Task<List<Winit.Modules.Org.Model.Interfaces.IOrg>> PrepareOrgMaster()
        {
            try
            {
                ...
                List<Winit.Modules.Org.Model.Interfaces.IOrg> orgList = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IOrg>(skuSql.ToString(), skuParameters);
          ...
          int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired, string FranchiseeOrgUID)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT * FROM (
                                            ...
                                            WHERE
                                                o.org_type_uid IN ('FRWH', 'WOWH') AND (o.parent_uid = @FranchiseeOrgUID OR @FranchiseeOrgUID = ''))
                                                 AS SubQuery ");
                ...
                IEnumerable<Winit.Modules.Org.Model.Interfaces.IWarehouseItemView> warehouseDetails = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IWarehouseItemView>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
        ...
        public async Task<Winit.Modules.Org.Model.Interfaces.IEditWareHouseItemView> ViewFranchiseeWarehouseByUID(string UID)
        {
            Dictionary<string, object?> parameters = new Dictionary<string, object?>
            {
                {"UID",  UID},
                                ...
                                '[' + ot.uid + ']' + ot.name AS WarehouseType
                            FROM
                                org o
                            JOIN
...
";
            ...
            Winit.Modules.Org.Model.Interfaces.IEditWareHouseItemView editWareHouseItemDetails = await ExecuteSingleAsync<Winit.Modules.Org.Model.Interfaces.IEditWareHouseItemView>(sql, parameters);
          ...
          int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired, string FranchiseeOrgUID, string WarehouseUID, string StockType)
        {
            try
            {
                var sql = new StringBuilder(@"select * from(SELECT
                                            ...
                                            SUM(wa.qty) OVER (PARTITION BY s.uid) AS TotalEaQty
                                        FROM
                                            WH_Stock_Available wa
                                        INNER JOIN
                                            SKU s ON s.uid = wa.sku_uid
                                        INNER JOIN
                                            sku_uom su ON su.sku_uid = s.uid AND su.is_outer_uom = 1
                                        WHERE
                                            wa.warehouse_uid = @WarehouseUID
                                            AND wa.stock_type = @StockType 
                                        GROUP BY
                                            s.uid,s.id, s.code, s.name, wa.qty, wa.uom, wa.stock_type, su.multiplier
                                        )as subquery");
                var sqlCount = new StringBuilder();
                if (isCountRequired)
                {
                ...
                IEnumerable<Winit.Modules.Org.Model.Interfaces.IWarehouseStockItemView> WarehouseStockItemViewDetails = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IWarehouseStockItemView>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
                    totalCount = await ExecuteScalarAsync<int>(sqlCount.ToString(), parameters);
        ...
        public async Task<int> CreateViewFranchiseeWarehouse(Winit.Modules.Org.Model.Interfaces.IEditWareHouseItemView createWareHouseItemView)
        {
            ...
            try
            {
                var sql = @"insert into org (uid,created_by,created_time,modified_by,modified_time,server_add_time,server_modified_time,code,
                            name,org_type_uid,is_active,parent_uid,country_uid,show_in_ui,show_in_template) values(@uid,@created_by,@created_time,@modified_by,
                             @modified_time,@server_add_time, @server_modified_time,@code,@name,@org_type_uid,@is_active,@parent_uid,@country_uid,@show_in_ui,@show_in_template);";
                Dictionary<string, object?> parameters = new Dictionary<string, object?>
                        {
                            ...
                            {"is_active", true},
                            {"country_uid", createWareHouseItemView.CountryUID},
                            {"show_in_ui", createWareHouseItemView.ShowInUI ?? true},
                            {"show_in_template", createWareHouseItemView.ShowInTemplate ?? true}
                        };
                ...
                using (var connection = CreateConnection())
                {
                    ...
                    try
                    {
                        retVal = await ExecuteNonQueryAsync(sql, connection, transaction, parameters);
                        retVal += await ExecuteNonQueryAsync(Addresssql, connection, transaction, Addressparameters);
                        transaction.Commit();
                    }
                    catch (Exception ex)
                    {
                        ...
                        throw new Exception("An error occurred while creating the warehouse and address information.", ex);
                    }
                    finally
                    {
                        transaction?.Dispose();
        ...
        public async Task<int> UpdateViewFranchiseeWarehouse(Winit.Modules.Org.Model.Interfaces.IEditWareHouseItemView updateWareHouseItemView)
        {
            ...
            try
            {
                ...
                var warehouseParameters = new Dictionary<string, object?>
        {
            ...
            { "region_code", updateWareHouseItemView.RegionCode }
                ...
                using (var connection = CreateConnection())
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            retVal = await ExecuteNonQueryAsync(warehouseSQL, connection, transaction, warehouseParameters);
                            retVal += await ExecuteNonQueryAsync(addressSQL, connection, transaction, addressParameters);
                            transaction.Commit();
                        }
                        catch (Exception ex)
                        {
                            transaction.Rollback();
                            throw new Exception("An error occurred while updating the warehouse and address information.", ex);
                        }
                    }
                }
            }
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"Select * From (SELECT
                    ...
                    show_in_ui AS ShowInUI,
                    show_in_template AS ShowInTemplate,
                    CONCAT('[', uid, ']', name) AS WarehouseType
                FROM
                    Org_Type )AS SubQuery
                  ");
                var sqlCount = new StringBuilder();
                if (isCountRequired)
                {
                ...
                IEnumerable<Winit.Modules.Org.Model.Interfaces.IOrgType> OrgTypeDetails = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IOrgType>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
                    totalCount = await ExecuteScalarAsync<int>(sqlCount.ToString(), parameters);
            ...
            string? parentUID = null, string? branchUID = null)
        {
            ...
            if (!string.IsNullOrEmpty(branchUID) && OrgTypeUID == OrgTypeConst.WH)
            {
...
";
            ...
            IEnumerable<Winit.Modules.Org.Model.Interfaces.IFOCStockItem> orgDetails = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IFOCStockItem>(sql, parameters);
            return orgDetails;
        }
        public async Task<IEnumerable<Modules.SKU.Model.Interfaces.ISKUGroup>> GetSkuGroupBySkuGroupTypeUID(string skuGroupTypeUid)
        {
            Dictionary<string, object?> parameters = new Dictionary<string, object?>
            {
            ...
            var sql = """
                      ...
                      name AS Name 
                FROM sku_group
                Where sku_group_type_uid =@skuGroupTypeUid
                """;
            IEnumerable<Winit.Modules.SKU.Model.Interfaces.ISKUGroup> skuDetails = await ExecuteQueryAsync<Winit.Modules.SKU.Model.Interfaces.ISKUGroup>(sql, parameters);
            return skuDetails;
        }
           ...
           int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder("""
                                        Select * From (SELECT ou_code AS OUCode, warehouse_code AS WarehouseCode, sub_warehouse_code AS SubWarehouseCode,
                                        S.code AS SKUCode, S.name AS SKUName, CS.qty AS Quantity, 
                                        WH.name AS WarehouseName, SWH.name AS SubWarehouseName
                                        FROM [cmi_wh_stock_available] CS
                                        INNER JOIN sku S on S.uid = CS.sku_uid 
                                        INNER JOIN org WH ON WH.code = CS.warehouse_code AND WH.org_type_uid = 'WH'
                                        INNER JOIN org SWH ON SWH.parent_uid = WH.uid AND SWH.code = CS.sub_warehouse_code AND SWH.org_type_uid = 'SWH'
                                        )AS SubQuery
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(
                        """
                        Select Count(*) From (SELECT ou_code AS OUCode, warehouse_code AS WarehouseCode, sub_warehouse_code AS SubWarehouseCode,
                        S.code AS SKUCode, S.name AS SKUName, CS.qty AS Quantity
                        FROM [cmi_wh_stock_available] CS
                        INNER JOIN sku S on S.uid = CS.sku_uid
                        INNER JOIN org WH ON WH.code = CS.warehouse_code AND WH.org_type_uid = 'WH'
                ...
                if (filterCriterias != null && filterCriterias.Count > 0)
                {
                    StringBuilder sbFilterCriteria = new StringBuilder();
                    sbFilterCriteria.Append(" Where ");
                    AppendFilterCriteria<Winit.Modules.Org.Model.Interfaces.IWareHouseStock>(filterCriterias, sbFilterCriteria, parameters);
                    sql.Append(sbFilterCriteria);
                    if (isCountRequired)
                    {
                        sqlCount.Append(sbFilterCriteria);
                ...
                if (pageNumber > 0 && pageSize > 0)
                {
                    ...
                    {
                        sql.Append($" ORDER BY SKUName OFFSET {(pageNumber - 1) * pageSize} ROWS FETCH NEXT {pageSize} ROWS ONLY");
                    }
                }

                IEnumerable<Winit.Modules.Org.Model.Interfaces.IWareHouseStock> OrgTypeDetails = await ExecuteQueryAsync<Winit.Modules.Org.Model.Interfaces.IWareHouseStock>(sql.ToString(), parameters);
                int totalCount = -1;
                if (isCountRequired)
                {
                    totalCount = await ExecuteScalarAsync<int>(sqlCount.ToString(), parameters);
                }
                PagedResponse<Winit.Modules.Org.Model.Interfaces.IWareHouseStock> pagedResponse = new PagedResponse<Winit.Modules.Org.Model.Interfaces.IWareHouseStock>
                {
                    PagedData = OrgTypeDetails,
                    TotalCount = totalCount
                };
        ...
        public Task<IEnumerable<IOrg>> GetOrgByOrgTypeUID(string OrgTypeUID, string? parentUID = null)
        {
            throw new NotImplementedException();
        }
    }
