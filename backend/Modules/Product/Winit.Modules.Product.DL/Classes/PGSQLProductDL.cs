...
{
    public class PGSQLProductDL:Winit.Modules.Base.DL.DBManager.PostgresDBManager,IProductDL
    {
        public PGSQLProductDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
    ...
    int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        ...
        public async Task<int> CreateProduct(Model.Interfaces.IProduct createproduct)
        {
            ...
            Dictionary<string, object> parameters = new Dictionary<string, object>
            {
                   ...
                   {"ProductAliasName", createproduct.ProductAliasName},
        ...
        public async Task<int> UpdateProduct(Model.Interfaces.IProduct updateProduct)
        {
            try
            {
                var sql = @"UPDATE ""Product"" SET ""ModifiedBy"" = @ModifiedBy, ""ModifiedTime"" = @ModifiedTime, ""ServerModifiedTime""= @ServerModifiedTime,
                          ""LongName"" = @LongName,""DisplayName"" = @DisplayName, ""FromDate"" = @FromDate,""ToDate"" = @ToDate, ""IsActive"" = @IsActive, 
                          ""BaseUOM"" = @BaseUOM WHERE ""UID"" = @UID;";
                Dictionary<string, object> parameters = new Dictionary<string, object>
                {
        ...
        public async Task<IEnumerable<Winit.Modules.Product.Model.Interfaces.IProduct>> GetProductsFiltered(string product_code, String product_name, DateTime CreatedTime, DateTime ModifiedTime)
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            ...
            IEnumerable<Winit.Modules.Product.Model.Interfaces.IProduct> productslist = await ExecuteQueryAsync<Winit.Modules.Product.Model.Interfaces.IProduct>(sql, parameters);
            return await Task.FromResult(productslist);
        }
        public async Task<IEnumerable<Winit.Modules.Product.Model.Interfaces.IProduct>> GetProductsPaged(int pageNumber, int pageSize)
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();
        ...
        public async Task<IEnumerable<Winit.Modules.Product.Model.Interfaces.IProduct>> GetProductsSorted(List<SortCriteria> sortCriterias)
        {
            ...
            IEnumerable<Winit.Modules.Product.Model.Interfaces.IProduct> productList = await ExecuteQueryAsync<Winit.Modules.Product.Model.Interfaces.IProduct>(sql.ToString(), sortParameters);
        ...
        public async Task<IEnumerable<IProduct>> SelectAllProduct(List<SortCriteria> sortCriterias, int pageNumber, int pageSize, List<FilterCriteria> filterCriterias)
        {
            try
            {
                ...
                {
                    sql.Append(" WHERE ");
                    AppendFilterCriteria<IProduct>(filterCriterias, sql, parameters);
                }
