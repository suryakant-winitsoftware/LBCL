...
{
    public class MSSQLReturnOrderTaxDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IReturnOrderTaxDL
    {
        public MSSQLReturnOrderTaxDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        public async Task<int> CreateReturnOrderTax(Winit.Modules.ReturnOrder.Model.Interfaces.IReturnOrderTax returnOrderTax)
        {
            try
            {
                ...
                return await ExecuteNonQueryAsync(sql, returnOrderTax);
            }
            catch (Exception)
            {
                throw;
