...
public class PGSQLReturnOrderDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IReturnOrderDL
{
    protected IStoreHistoryDL _storeHistoryDL;
    public PGSQLReturnOrderDL(IServiceProvider serviceProvider, IConfiguration config, IStoreHistoryDL storeHistoryDL) : base(serviceProvider, config)
    {
        ...
        string storeUID = "", List<FilterCriteria>? filterCriterias = null)
    {
        try
        {
            StringBuilder sql = new StringBuilder("""
                        ...
                        S.name AS StoreName, 
                        Ro.route_uid AS routeuid, 
                        RO.return_order_number AS OrderNumber, 
                        COALESCE(RO.return_order_number, RO.draft_order_number) AS OrderNumber, 
                    ...
                    FROM 
                        ...
                        LEFT JOIN currency C ON RO.currency_uid = C.uid 
                        LEFT JOIN address A ON A.linked_item_type = 'Store' 
                        AND A.linked_item_uid = S.uid AND A.is_default = TRUE ) as returnsummarydata
                    WHERE 
            ...
            Type type = _serviceProvider.GetRequiredService<IReturnSummaryItemView>().GetType();
            List<IReturnSummaryItemView> returnOrders = await ExecuteQueryAsync<IReturnSummaryItemView>(sql.ToString(), parameters, type);
            return returnOrders;
        }
        catch (Exception)
        {
        ...
        try
        {
            var sql = @"UPDATE return_order 
                                SET 
                                    status = @Status 
                                WHERE 
                                    uid = ANY(string_to_array(@ReturnOrderUIDs, ','));";
            Dictionary<string, object?> parameters = new Dictionary<string, object?>
            {
                {"Status",Status },
