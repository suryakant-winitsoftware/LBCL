...
{
    public class PGSQLReturnOrderLineDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IReturnOrderLineDL
    {
        public PGSQLReturnOrderLineDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                IEnumerable<Model.Interfaces.IReturnOrderLine> returnOrderLines = await ExecuteQueryAsync<Model.Interfaces.IReturnOrderLine>(sql.ToString(), parameters, type);
                //Count
                int totalCount = 0;
                if (isCountRequired)
                {
