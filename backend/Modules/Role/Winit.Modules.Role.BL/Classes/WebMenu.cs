...
{
    public class WebMenu : Interfaces.IMenu
    {


        public Winit.Modules.Role.Model.Interfaces.IMenuMasterHierarchyView ModulesMasterHierarchy { get; set; }

        public WebMenu(Winit.Modules.Role.Model.Interfaces.IMenuMasterHierarchyView _ModulesMasterHierarchy, ApiService apiService, Winit.Shared.Models.Common.IAppConfig appConfigs, CommonFunctions commonFunctions, NavigationManager navigationManager,
          Winit.Modules.Common.Model.Interfaces.IDataManager dataManager, IAlertService alertService, ILoadingService loadingService, IAppUser appUser,ILocalStorageService localStorageService)
        {
        ...
        public async Task PopulateMenuData()
        {
           ...
           // ModulesMasterHierarchy.ModuleMasterHierarchies = ModuleMasters;
            ...
            await _localStorageService.SetItem(nameof(MenuMasterHierarchyView), ModulesMasterHierarchy);
        }
        ...
        public async Task GetPermissions()
        {
            try
            {
                ApiResponse<string> apiResponse = await _apiService.FetchDataAsync($"{_appConfigs.ApiBaseUrl}Role/SelectAllPermissionsByRoleUID?roleUID={_appUser.Role.UID}&platform=Web&isPrincipalTypePermission={_appUser.Role.IsPrincipalRole}", HttpMethod.Get);
                if (apiResponse != null)
                {
                        ...
                        if (module.UID.Equals(subModule.ModuleUid))
                        {
                            ...
                            foreach (SubSubModules sub_Sub_Modules in ModulesMasterView.SubSubModules)
                            {
                                if (subModule.UID.Equals(sub_Sub_Modules.SubModuleUid))
                                {
                                    ...
                                    if (!_appUser.Role.IsAdmin)
                                    {
                                        permission = Permissions.Find(p => p.SubSubModuleUid.Equals(sub_Sub_Modules.UID));
                                        if (permission == null)
                                        {
