...
{
    public class SQLiteRoleDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IRoleDL
    {
        public SQLiteRoleDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
...
is_for_reports_to=@is_for_reports_to
        ...
        public async Task<int> CreateRoles(IRole role)
        {
            try
            {
                ...
                have_vehicle,mobile_menu_data,is_for_reports_to)
			values(@uid,@org_uid,@code,@role_name_en,@role_name_other,@parent_role_uid,@is_principal_role,@is_distributor_role,@is_admin,@bu_to_dist_access,
                ...
                @have_warehouse,@have_vehicle,@mobile_menu_data,@is_for_reports_to)";
                Dictionary<string, object> parameters = new Dictionary<string, object>
            {
                 {"uid" , role.UID},
                 {"org_uid" , role.OrgUid},
            ...
            int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                StringBuilder sql = new StringBuilder(@"select * from(SELECT 
                                                            ...
                                                            r.mobile_menu_data as MobileMenuData,
        ...
        public async Task<List<ISubModule>> SelectAllSubModules(string platForm)
        {
            try
            {
                var sql = @"select * from sub_modules";
                Type type = _serviceProvider.GetRequiredService<IRole>().GetType();
        ...
        public async Task<List<ISubSubModules>> SelectAllSubSubModules(string platForm)
        {
            try
            {
                var sql = @"select * from sub_sub_modules";
                Type type = _serviceProvider.GetRequiredService<IRole>().GetType();
        ...
        {
            List<IPermission> permissions = await SelectAllPermissions(permissionMaster.RoleUID, permissionMaster.Platform, permissionMaster.IsPrincipalPermission);
            int insCount = 0;
        ...
        public async Task<int> UpdatePermissition(IPermission permission, bool isPrincipalPermission)
        {
            try
            {
                ...
                var sql1 = @"UPDATE public.principal_permissions
                        ...
                        delete_access=@delete_access, download_access=@download_access, approval_access=@approval_access, ss=@ss, created_by=@created_by, modified_by=@modified_by, created_time=@created_time, 
                        modified_time=@modified_time,server_add_time=@server_add_time, server_modified_time=@server_modified_time,platform=@platform
	                    WHERE uid=@uid;";
