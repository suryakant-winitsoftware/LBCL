...
{
    public class PGSQLRouteCustomer : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IRouteCustomerDL
    {
        public PGSQLRouteCustomer(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        ...
        public async Task<int> DeleteRouteCustomerDetails(List<string> UIDs)
        {
            ...
            var sql = @"UPDATE route_customer SET  is_deleted=TRUE WHERE uid = ANY(string_to_array(@UIDs, ','));";
            return await ExecuteNonQueryAsync(sql, parameters);
