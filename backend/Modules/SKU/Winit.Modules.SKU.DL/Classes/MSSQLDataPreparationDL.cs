...
{
    public class MSSQLDataPreparationDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IDataPreparationDL
    {
        public MSSQLDataPreparationDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
            ...
            PrepareSKUMaster(List<String> orgUIDs, List<string> DistributionChannelUIDs, List<string> skuUIDs, List<string> attributeTypes)
        {
            try
            {
                ...
                {
                    skuSql.Append(@" AND (  org_uid IN (SELECT value FROM STRING_SPLIT(@OrgUIDs, ','))) ");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedorgUIDs))
                {
                    skuConfig.Append(@" AND  (org_uid IN (SELECT value FROM STRING_SPLIT(@ORGUIDs, ','))) ");
                }
                if (!string.IsNullOrEmpty(commaSeparatedDistributionChannelUIDs))
                {
                    skuConfig.Append(@" AND  (distribution_channel_org_uid IN (SELECT value FROM STRING_SPLIT(@DistributionChannelOrgUIDs, ','))) ");
                }
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    skuConfig.Append(@" AND  (sku_uid IN (SELECT value FROM STRING_SPLIT(@SKUUIDS, ','))) ");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    skuUomSql.Append(@" WHERE ( sku_uid IN (SELECT value FROM STRING_SPLIT(@SKUUIDS, ','))) ");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    skuAtrributesSql.Append(@" WHERE ( sku_uid IN (SELECT value FROM STRING_SPLIT(@SKUUIDS, ','))) ");
                }

                if (!string.IsNullOrEmpty(commaSeparatedattributeTypes))
                {
                    skuAtrributesSql.Append(@" AND ( type IN (SELECT value FROM STRING_SPLIT(@Types, ','))) ");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    taxSkuMapSql.Append(@" WHERE sku_uid IN (SELECT value FROM STRING_SPLIT(@SKUUIDS, ',')) ");
                }
            ...
            List<Winit.Modules.Contact.Model.Interfaces.IContact>)> PrepareStoreMaster(List<string> storeUIDs)
        {
            try
            {
                ...
                var storeSql = new StringBuilder(@"SELECT id AS Id,uid AS UID,created_by AS CreatedBy,created_time AS CreatedTime,modified_by AS ModifiedBy,
                                                ...
                                                is_vat_qr_capture_mandatory AS IsVATQRCaptureMandatory, tax_type AS TaxType, is_asm_mapped_by_customer as IsAsmMappedByCustomer FROM store");
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    storeSql.Append(@" WHERE uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','))");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    storeAdditionalInfoSql.Append(@" WHERE store_uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','))");
                }
                ...
                List<Winit.Modules.Store.Model.Interfaces.IStoreCredit> storeCreditList = await ExecuteQueryAsync<Winit.Modules.Store.Model.Interfaces.IStoreCredit>(storeCreditSql.ToString(), parameters);
                ...
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    storeAttributesSql.Append(@" WHERE linked_item_uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','))");
                }
                ...
                var addressSql = new StringBuilder(@"SELECT 
                    ...
                    sales_office_uid as SalesOfficeUID,
                    custom_field1 as CustomField1,
                    custom_filed2 as CustomField2
                FROM 
                    address");
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    addressSql.Append(@" WHERE linked_item_uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','))");
                }
                ...
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    contactSql.Append(@" WHERE linked_item_uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','))");
                }
                var contactParameters = new Dictionary<string, object>();
                List<Winit.Modules.Contact.Model.Interfaces.IContact> contactList = await ExecuteQueryAsync<Winit.Modules.Contact.Model.Interfaces.IContact>(contactSql.ToString(), parameters);
