...
{
    public class MSSQLSKUConfigDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager,ISKUConfigDL
    {
        public MSSQLSKUConfigDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                IEnumerable<Model.Interfaces.ISKUConfig> skuConfigList = await ExecuteQueryAsync<Model.Interfaces.ISKUConfig>(sql.ToString(), parameters);
                int totalCount = 0;
                if (isCountRequired)
                {
        ...
        public async Task<Winit.Modules.SKU.Model.Interfaces.ISKUConfig> SelectSKUConfigByUID(string UID)
        {
            ...
            WHERE 
                uid = @UID";
            Winit.Modules.SKU.Model.Interfaces.ISKUConfig skuConfigDetails = await ExecuteSingleAsync<Winit.Modules.SKU.Model.Interfaces.ISKUConfig>(sql, parameters);
        ...
        public async Task<int> CreateSKUConfig(Winit.Modules.SKU.Model.Interfaces.ISKUConfig skuConfig)
        {
            try
            {
                ...
                return await ExecuteNonQueryAsync(sql, skuConfig);
            }
            catch (Exception)
            {
                throw;
