...
{
    public class MSSQLSKUUOMDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, ISKUUOMDL
    {
        private readonly Winit.Modules.SKU.DL.Interfaces.ISKUPriceDL _sKUPriceDL;
        public MSSQLSKUUOMDL(IServiceProvider serviceProvider, IConfiguration config, ISKUPriceDL sKUPriceDL) : base(serviceProvider, config)
        {
            ...
            var parameters = new Dictionary<string, object>
    {
        ...
        public async Task<int> UpdateSKUForBaseAndOuterUOMs(Winit.Modules.SKU.Model.Interfaces.ISKUUOM skuUOM)
        {
            try
            {
                ...
                if (skuUOM.IsBaseUOM == true)
                {
                    sql = "update sku set base_uom = @base_uom, server_modified_time = GETDATE() where uid = @sku_uid";
                }
                else
                {
                    sql = "update sku set outer_uom = @outer_uom, server_modified_time = GETDATE() where uid = @sku_uid";
                }
