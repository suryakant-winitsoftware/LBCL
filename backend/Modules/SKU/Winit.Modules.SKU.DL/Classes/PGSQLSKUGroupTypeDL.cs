...
{
    public class PGSQLSKUGroupTypeDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ISKUGroupTypeDL
    {
        public PGSQLSKUGroupTypeDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                if (filterCriterias != null && filterCriterias.Count > 0)
                {
                    ...
                    if (filterCriterias.Any(e => e.Name == "sku_group_type_hierarchy_level_code_name"))
                    {
                        sbFilterCriteria.Append(" (code = @Code or name = @Name) ");
                        FilterCriteria filter = filterCriterias.Find(e => e.Name == "sku_group_type_hierarchy_level_code_name")!;
                ...
                Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ISKUGroupType>().GetType();
                IEnumerable<Model.Interfaces.ISKUGroupType> skuGroupTypeList = await ExecuteQueryAsync<Model.Interfaces.ISKUGroupType>(sql.ToString(), parameters, type);
                int totalCount = 0;
                if (isCountRequired)
                {
        ...
        public async Task<Winit.Modules.SKU.Model.Interfaces.ISKUGroupType> SelectSKUGroupTypeByUID(string UID)
        {
            ...
            WHERE uid= @UID";
            Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ISKUGroupType>().GetType();
            Winit.Modules.SKU.Model.Interfaces.ISKUGroupType skGroupTypeDetails = await ExecuteSingleAsync<Winit.Modules.SKU.Model.Interfaces.ISKUGroupType>(sql, parameters, type);
        ...
        public async Task<int> CreateSKUGroupType(Winit.Modules.SKU.Model.Interfaces.ISKUGroupType sKUGroupType)
        {
            try
            {
                ...
                Dictionary<string, object> parameters = new Dictionary<string, object>
                {
                   ...
                   {"OrgUID",sKUGroupType.OrgUID},
                   {"Code",sKUGroupType.Code},
                   {"Name",sKUGroupType.Name},
                   {"ParentUID",sKUGroupType.ParentUID},
        ...
        public async Task<IEnumerable<Winit.Modules.SKU.Model.Interfaces.ISKUGroupTypeView>> SelectSKUGroupTypeView()
        {
            ...
            var sql = @"SELECT id AS Id,
                ...
                item_level AS ItemLevel
            FROM sku_group_type;";
            Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ISKUGroupTypeView>().GetType();
            IEnumerable<Winit.Modules.SKU.Model.Interfaces.ISKUGroupTypeView> skGroupTypeDetails = await ExecuteQueryAsync<Winit.Modules.SKU.Model.Interfaces.ISKUGroupTypeView>(sql, parameters, type);
            return skGroupTypeDetails;
        }
