...
{
    public class PGSQLSKUPriceListDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ISKUPriceListDL
    {
        public PGSQLSKUPriceListDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
        ...
        public async Task<IEnumerable<Winit.Modules.SKU.Model.Interfaces.IBuyPrice>> PopulateBuyPrice(string OrgUID)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT spl.selection_uid AS FranchiseeOrgUID,
                    ...
                    AND CURRENT_DATE >= sp.valid_from :: Date
                    AND CURRENT_DATE <= sp.valid_upto :: Date;");
                var parameters = new Dictionary<string, object>()
                {
                    {"OrgUID",OrgUID }
                };
        ...
        public async Task<int> CreateStandardPriceList(string orgUID)
        {
            try
            {
                StringBuilder sql = new StringBuilder(@"INSERT INTO sku_price_list (uid, created_by, created_time, modified_by, modified_time, server_add_time,
                                                            ...
                                                            SELECT o.uid uid, O.created_by, O.created_time, 
                                                            O.modified_by, 
                                                            O.modified_time, O.server_add_time,
                                                            O.server_modified_time, O.company_uid, O.code, 
                                                            O.code || ' Price List' name, O.org_type_uid type, 
                                                            O.parent_uid org_uid, null distribution_channel_uid, 99999 priority,
                                                            'Org' selection_group, 'Org' selection_type, 
                                                            O.uid selection_uid, 
                                                            true is_active, 'Published' status, O.created_time::Date valid_from, '2099-12-31' valid_upto
                                                            FROM Org O
                                                            LEFT JOIN sku_price_list SPL ON SPL.type = 'FR' AND SPL.org_uid = O.parent_uid 
                                                            AND SPL.selection_group = 'Org' AND SPL.selection_type = 'Org' 
                                                            AND SPL.selection_uid = O.uid 
                                                            WHERE O.org_type_uid = 'FR' AND O.uid = @OrgUID
                                                            AND SPL.id is null");
