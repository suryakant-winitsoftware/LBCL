...
{
    public class PGSQLSKUTemplateLineDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ISKUTemplateLineDL
    {
        protected ISKUTemplateDL _skuTemplateDL;
        public PGSQLSKUTemplateLineDL(IServiceProvider serviceProvider, IConfiguration config, ISKUTemplateDL skuTemplateDL) : base(serviceProvider, config)
        {
        ...
        int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ISKUTemplateLine>().GetType();
                IEnumerable<Model.Interfaces.ISKUTemplateLine> skuTemplateLineList = await ExecuteQueryAsync<Model.Interfaces.ISKUTemplateLine>(sql.ToString(), parameters, type);
                int totalCount = 0;
                if (isCountRequired)
                {
        ...
        public async Task<Winit.Modules.SKU.Model.Interfaces.ISKUTemplateLine> SelectSKUTemplateLineByUID(string UID)
        {
            try
            {
                ...
                Type type = _serviceProvider.GetRequiredService<Winit.Modules.SKU.Model.Interfaces.ISKU>().GetType();
                Winit.Modules.SKU.Model.Interfaces.ISKUTemplateLine skuTemplateLineDetails = await ExecuteSingleAsync<Winit.Modules.SKU.Model.Interfaces.ISKUTemplateLine>(sql, parameters, type);
        ...
        public async Task<int> CreateSKUTemplateLine(Winit.Modules.SKU.Model.Interfaces.ISKUTemplateLine sKUTemplateLine)
        {
                                    ...
                                    INSERT INTO sku_template_line (
                                        ...
                                        Dictionary<string, object> parameters = new Dictionary<string, object>
                                {
                                    {"UID", sKUTemplateLine.UID},
                                    {"CreatedBy", sKUTemplateLine.CreatedBy},
                                    {"CreatedTime", sKUTemplateLine.CreatedTime},
                                    {"ModifiedBy", sKUTemplateLine.ModifiedBy},
                                    {"ModifiedTime", sKUTemplateLine.ModifiedTime},
                                    {"ServerAddTime", sKUTemplateLine.ServerAddTime},
                                    {"ServerModifiedTime", sKUTemplateLine.ServerModifiedTime},
                                    {"SKUTemplateUID", sKUTemplateLine.SKUTemplateUID},
                                    {"SKUGroupTypeUID", sKUTemplateLine.SKUGroupTypeUID},
                                    {"SKUGroupUID", sKUTemplateLine.SKUGroupUID},
                                    {"IsExcluded", sKUTemplateLine.IsExcluded},
                                };
        ...
        private async Task<List<string>> CheckSKUTemplateLineByUIDs(List<string> UIDs)
        {
            ...
            var sql = @"SELECT uid FROM  sku_template_line  WHERE  UID = ANY(@UIDs) ";
