...
{
    public class SQLiteDataPreparationDL : Winit.Modules.Base.DL.DBManager.SqliteDBManager,IDataPreparationDL
    {
        public SQLiteDataPreparationDL(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }
            ...
            PrepareSKUMaster(List<String> orgUIDs, List<string> DistributionChannelUIDs, List<string> skuUIDs, List<string> attributeTypes)
        {
            try
            {
                ...
                var skuSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                ...
                                                is_active AS IsActive, is_third_party AS IsThirdParty, supplier_org_uid AS SupplierOrgUID 
                                                FROM sku WHERE is_active = 1");
                if (!string.IsNullOrEmpty(commaSeparatedorgUIDs))
                {
                    //skuSql.Append(@" AND OrgUID IN ('@ORGUIDs');");
                    skuSql.Append($" AND org_uid IN ('{commaSeparatedorgUIDs}');");
                ...
                var skuConfig = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, 
                                                    ...
                                                    sku_uid AS SKUUID, can_buy AS CanBuy, can_sell AS CanSell, buying_uom AS BuyingUOM, 
                                                    selling_uom AS SellingUOM, is_active AS IsActive FROM sku_config WHERE is_active = true;");

                if (!string.IsNullOrEmpty(commaSeparatedorgUIDs))
                {
                    skuConfig.Append($" AND org_uid IN ('{commaSeparatedorgUIDs}');");
                }
                if (!string.IsNullOrEmpty(commaSeparatedDistributionChannelUIDs))
                {
                    
                    skuConfig.Append($" AND distribution_channel_org_uid IN ('{commaSeparatedDistributionChannelUIDs}'));");
                }
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    
                    skuConfig.Append($" AND sku_uid IN ('{commaSeparatedskuUIDs}');");
                ...
                var skuUomSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                    modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, 
                                                    sku_uid AS SKUUID, code AS Code, name AS Name, label AS Label, barcodes AS Barcodes, is_base_uom AS IsBaseUOM, 
                                                    is_outer_uom AS IsOuterUOM, multiplier AS Multiplier,length AS Length, depth AS Depth, width AS Width, height AS Height, 
                ...
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    skuUomSql.Append($" WHERE sku_uid IN ('{commaSeparatedskuUIDs}');");
                }
                ...
                var skuAtrributesSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, 
                                                            modified_by AS ModifiedBy, modified_time AS ModifiedTime, server_add_time AS ServerAddTime,
                                                            server_modified_time AS ServerModifiedTime, sku_uid AS SKUUID, type AS Type, code AS Code, 
                                                            value AS Value, parent_type AS ParentType 
                                                            FROM sku_attributes; ");
                ...
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    skuAtrributesSql.Append($" WHERE sku_uid IN ('{commaSeparatedskuUIDs}');");
                }

                if (!string.IsNullOrEmpty(commaSeparatedattributeTypes))
                {
                    skuAtrributesSql.Append($" AND type IN ('{commaSeparatedattributeTypes}');");
                ...
                var taxSkuMapSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime,
                                                        modified_by AS ModifiedBy, modified_time AS ModifiedTime, server_add_time AS ServerAddTime,
                                                        server_modified_time AS ServerModifiedTime, company_uid AS CompanyUID, sku_uid AS SKUUID,
                                                        tax_uid AS TaxUID FROM tax_sku_map");
                if (!string.IsNullOrEmpty(commaSeparatedskuUIDs))
                {
                    taxSkuMapSql.Append($" WHERE sku_uid IN ('{commaSeparatedskuUIDs}');");
                }
            ...
            List<Winit.Modules.Contact.Model.Interfaces.IContact>)> PrepareStoreMaster(List<string> storeUIDs)
        {
            try
            {
                ...
                var storeSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                    ...
                                                    created_by_job_position_uid AS CreatedByJobPositionUID, country_uid AS CountryUID, region_uid AS RegionUID, 
                                                    city_uid AS CityUID, source AS Source, outlet_name AS OutletName, blocked_by_emp_uid AS BlockedByEmpUID,
                                                    arabic_name AS ArabicName, is_tax_applicable AS IsTaxApplicable, tax_doc_number AS TaxDocNumber, 
                                                    school_warehouse AS SchoolWarehouse, day_type AS DayType, special_day AS SpecialDay, is_tax_doc_verified AS IsTaxDocVerified,
                                                    store_size AS StoreSize, prospect_emp_uid AS ProspectEmpUID, tax_key_field AS TaxKeyField, store_image AS StoreImage, 
                ...
                var storeAdditionalInfoSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                            modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, 
                                                            store_uid AS StoreUID, order_type AS OrderType, is_promotions_block AS IsPromotionsBlock, customer_start_date AS CustomerStartDate, 
                                                            customer_end_date AS CustomerEndDate, purchase_order_number AS PurchaseOrderNumber, 
                                                            ...
                                                            is_capture_signature_required AS IsCaptureSignatureRequired, is_always_printed AS IsAlwaysPrinted, 
                                                            ...
                                                            display_delivery_docket AS DisplayDeliveryDocket, display_price AS DisplayPrice, show_cust_po AS ShowCustPO, 
                                                            invoice_text AS InvoiceText, invoice_frequency AS InvoiceFrequency, stock_credit_is_purchase_order_required AS StockCreditIsPurchaseOrderRequired,
                                                            ...
                                                            is_dummy_customer AS IsDummyCustomer, default_run AS DefaultRun, is_foc_customer AS IsFOCCustomer, rss_show_price AS RSSShowPrice, 
                                                            rss_show_payment AS RSSShowPayment, rss_show_credit AS RSSShowCredit, rss_show_invoice AS RSSShowInvoice, 
                                                            rss_is_active AS RSSIsActive, rss_delivery_instruction_status AS RSSDeliveryInstructionStatus, 
                                                            ...
                                                            is_manual_edit_allowed AS IsManaualEditAllowed, can_update_lat_long AS CanUpdateLatLong, allow_good_return AS AllowGoodReturn, 
                                                            ...
                                                            is_asset_enabled AS IsAssetEnabled, is_survey_enabled AS IsSurveyEnabled,  allow_return_against_invoice AS AllowReturnAgainstInvoice, 
                                                            allow_return_with_sales_order AS AllowReturnWithSalesOrder, week_off_sun AS WeekOffSun, week_off_mon AS WeekOffMon, 
                                                            week_off_tue AS WeekOffTue, week_off_wed AS WeekOffWed, week_off_thu AS WeekOffThu, week_off_fri AS WeekOffFri, 
                ...
                var storeCreditSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                        modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, 
                                                        store_uid AS StoreUID, payment_term_uid AS PaymentTermUID, credit_type AS CreditType, credit_limit AS CreditLimit,  
                                                        temporary_credit AS TemporaryCredit, org_uid AS OrgUID, distribution_channel_uid AS DistributionChannelUID, 
                                                        preferred_payment_mode AS PreferredPaymentMode, is_active AS IsActive, is_blocked AS IsBlocked, 
                                                        blocking_reason_code AS BlockingReasonCode, blocking_reason_description AS BlockingReasonDescription 
                                                        FROM store_credit;");
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    storeCreditSql.Append(@" WHERE store_uid IN (SELECT value FROM STRING_SPLIT(@UIDs, ','));");

                    storeCreditSql.Append($" WHERE store_uid IN ('{commaSeparatedstoreUIDs}');");
                ...
                var storeAttributesSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                            modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime,
                                                            company_uid AS CompanyUID, org_uid AS OrgUID, distribution_channel_uid AS DistributionChannelUID, store_uid AS StoreUID, 
                                                            name AS Name, code AS Code, value AS Value, parent_name AS ParentName 
                ...
                var addressSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                    modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, 
                                                    ...
                                                    phone AS Phone, phone_extension AS PhoneExtension, mobile1 AS Mobile1, mobile2 AS Mobile2, email AS Email, 
                                                    fax AS Fax, latitude AS Latitude, longitude AS Longitude, altitude AS Altitude, linked_item_uid AS LinkedItemUID, 
                                                    linked_item_type AS LinkedItemType, status AS Status, state_code AS StateCode, territory_code AS TerritoryCode, 
                                                    pan AS PAN, aadhar AS AADHAR, ssn AS SSN, is_editable AS IsEditable, is_default AS IsDefault, line4 AS Line4, 
                                                    info AS Info, depot AS Depot FROM address;");
                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    addressSql.Append($" WHERE store_uid IN ('{commaSeparatedstoreUIDs}');");
                }
                ...
                var contactSql = new StringBuilder(@"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                                                    modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, 
                                                    ...
                                                    invoice_for_email1 AS InvoiceForEmail1, invoice_for_email2 AS InvoiceForEmail2, invoice_for_email3 AS InvoiceForEmail3, 
                                                    fax AS Fax, linked_item_uid AS LinkedItemUID, linked_item_type AS LinkedItemType, is_default AS IsDefault, 
                                                    is_editable AS IsEditable, enabled_for_invoice_email AS EnabledForInvoiceEmail, enabled_for_docket_email AS EnabledForDocketEmail, 
                                                    enabled_for_promo_email AS EnabledForPromoEmail, is_email_cc AS IsEmailCC, mobile2 AS Mobile2 FROM contact;");

                if (!string.IsNullOrEmpty(commaSeparatedstoreUIDs))
                {
                    contactSql.Append($" WHERE store_uid IN ('{commaSeparatedstoreUIDs}');");
                }
        ...
        public async Task<List<IOrgCurrency>> PrepareOrgCurrencyMaster()
        {
            try
            {
                var orgCurrencySql = new StringBuilder(@"SELECT id AS Id, org_uid AS OrgUID, currency_uid AS CurrencyUID, is_primary AS IsPrimary, ss AS SS, created_time AS CreatedTime, 
                                                        modified_time AS ModifiedTime, server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime 
                                                        FROM org_currency;");
