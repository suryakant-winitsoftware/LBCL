...
namespace Winit.Modules.SKU.DL.Interfaces
{
    public interface ISKUPriceDL
    {
        ...
        Task<int> UpdateSKUPriceList(List<Winit.Modules.SKU.Model.Classes.SKUPrice> sKUPriceList);
        Task<int> DeleteSKUPrice(string UID);
        Task<(List<ISKUPriceList>, List<ISKUPrice>, int)> SelectSKUPriceViewByUID(List<SortCriteria> sortCriterias, int pageNumber, int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired, string skuPriceUID);
