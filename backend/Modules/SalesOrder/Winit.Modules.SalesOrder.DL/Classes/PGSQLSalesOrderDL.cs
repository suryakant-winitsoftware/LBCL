...
public class PGSQLSalesOrderDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, ISalesOrderDL
{
    private readonly ILogger<PGSQLSalesOrderDL> _logger;
    ...
    public PGSQLSalesOrderDL(IServiceProvider serviceProvider, IConfiguration config, ILogger<PGSQLSalesOrderDL> logger, IStoreHistoryDL storeHistoryDL,
    ...
    int pageSize, List<FilterCriteria> filterCriterias)
    {
        try
        {
            StringBuilder sql = new(@"SELECT so.id AS SalesOrderId, so.uid AS SalesOrderUID, 
                            ...
                            so.uid AS SalesOrderUID, so.job_position_uid AS JobPositionUID, so.emp_uid AS EmpUID, 
            ...
            {
                // Use PostgreSQL syntax for pagination: LIMIT ... OFFSET ...
            ...
            // Create mappings for SalesOrder properties - map all columns from SQL to object properties
            Dictionary<string, string> salesOrderColumnMappings = new()
            {
                ...
                { "Remarks", "Remarks" },
                { "IsTemperatureCheckEnabled", "IsTemperatureCheckEnabled" },
                { "AlwaysPrintedFlag", "AlwaysPrintedFlag" },
                { "PurchaseOrderNoRequiredFlag", "PurchaseOrderNoRequiredFlag" },
                { "IsWithPrintedInvoicesFlag", "IsWithPrintedInvoicesFlag" }
            };
            List<ISalesOrderViewModel>? salesOrderViewModelList = null;
            if (dt != null && dt.Rows.Count > 0)
            {
                salesOrderViewModelList = [];
                foreach (DataRow row in dt.Rows)
                {
                    // Create sales order object manually instead of using ConvertDataTableToObject1
                    // Use the service provider to get the concrete implementation
                    ISalesOrder salesOrder = _serviceProvider.GetRequiredService<Model.Interfaces.ISalesOrder>();
                    
                    // Manually map all the important fields
                    salesOrder.Id = Convert.ToInt32(row["SalesOrderId"] ?? 0);
                    salesOrder.UID = row["SalesOrderUID"]?.ToString() ?? "";
                    salesOrder.SalesOrderNumber = row["SalesOrderNumber"]?.ToString() ?? "";
                    salesOrder.StoreUID = row["StoreUID"]?.ToString() ?? "";
                    salesOrder.Status = row["Status"]?.ToString() ?? "";
                    salesOrder.OrderType = row["OrderType"]?.ToString() ?? "";
                    salesOrder.EmpUID = row["EmpUID"]?.ToString() ?? "";
                    salesOrder.CreatedBy = row["SalesOrderCreatedBy"]?.ToString() ?? "";
                    salesOrder.ModifiedBy = row["SalesOrderModifiedBy"]?.ToString() ?? "";
                    
                    // Handle dates safely
                    if (DateTime.TryParse(row["OrderDate"]?.ToString(), out DateTime orderDate))
                        salesOrder.OrderDate = orderDate;
                    if (DateTime.TryParse(row["ExpectedDeliveryDate"]?.ToString(), out DateTime expectedDeliveryDate))
                        salesOrder.ExpectedDeliveryDate = expectedDeliveryDate;
                    if (DateTime.TryParse(row["DeliveredDateTime"]?.ToString(), out DateTime deliveredDateTime))
                        salesOrder.DeliveredDateTime = deliveredDateTime;
                    if (DateTime.TryParse(row["SalesOrderCreatedTime"]?.ToString(), out DateTime createdTime))
                        salesOrder.CreatedTime = createdTime;
                    if (DateTime.TryParse(row["SalesOrderModifiedTime"]?.ToString(), out DateTime modifiedTime))
                        salesOrder.ModifiedTime = modifiedTime;
                    
                    // Handle numeric values safely
                    if (decimal.TryParse(row["TotalAmount"]?.ToString(), out decimal totalAmount))
                        salesOrder.TotalAmount = totalAmount;
                    if (decimal.TryParse(row["TotalDiscount"]?.ToString(), out decimal totalDiscount))
                        salesOrder.TotalDiscount = totalDiscount;
                    if (decimal.TryParse(row["TotalTax"]?.ToString(), out decimal totalTax))
                        salesOrder.TotalTax = totalTax;
                    if (decimal.TryParse(row["NetAmount"]?.ToString(), out decimal netAmount))
                        salesOrder.NetAmount = netAmount;
                    if (int.TryParse(row["LineCount"]?.ToString(), out int lineCount))
                        salesOrder.LineCount = lineCount;
                    if (int.TryParse(row["QtyCount"]?.ToString(), out int qtyCount))
                        salesOrder.QtyCount = qtyCount;
                    if (int.TryParse(row["CreditDays"]?.ToString(), out int creditDays))
                        salesOrder.CreditDays = creditDays;
                    
                    // Map other important fields
                    salesOrder.Source = row["Source"]?.ToString() ?? "";
                    salesOrder.CurrencyUID = row["CurrencyUID"]?.ToString() ?? "";
                    salesOrder.OrgUID = row["OrgUID"]?.ToString() ?? "";
                    salesOrder.RouteUID = row["RouteUID"]?.ToString() ?? "";
                    salesOrder.BeatHistoryUID = row["BeatHistoryUID"]?.ToString() ?? "";
                    salesOrder.Notes = row["Notes"]?.ToString() ?? "";
                    salesOrder.DeliveryInstructions = row["DeliveryInstructions"]?.ToString() ?? "";
                    salesOrder.Remarks = row["Remarks"]?.ToString() ?? "";
                    salesOrder.VehicleUID = row["VehicleUID"]?.ToString() ?? "";
    ...
    public async Task<IEnumerable<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrderViewModel>> SelectSalesOrderByUID(string SalesOrderUID)
    {
        try
        {
            ...
            string sql = @"SELECT so.id AS SalesOrderId, so.uid AS SalesOrderUID, so.created_by AS SalesOrderCreatedBy, 
                        ...
                        so.server_modified_time AS SalesOrderInfoServerModifiedTime, so.uid AS SalesOrderUID, 
    ...
    public async Task<List<Winit.Modules.SalesOrder.Model.UIInterfaces.ISalesSummaryItemView>>
        GetISalesSummaryItemViews(DateTime startDate, DateTime endDate, string storeUID = "")
    {
        StringBuilder sql = new(@"SELECT s.code AS StoreCode, s.name AS StoreName, ISNULL(so.sales_order_number, 
                so.draft_order_number) AS OrderNumber, a.line1 + ' ' + a.line2 AS Address, so.order_type AS OrderType, 
                so.status AS OrderStatus, so.order_date AS OrderDate, so.expected_delivery_date AS DeliveryDate,
                so.net_amount AS OrderAmount, C.name AS CurrencyLabel, CASE WHEN so.ss = 0 THEN 1 ELSE 0 END AS IsPosted 
                FROM sales_order so
                ...
                WHERE so.expected_delivery_date  BETWEEN @startDate AND @endDate");
        ...
        Type type = _serviceProvider.GetRequiredService<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrder>().GetType();
    ...
    public async Task<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrderPrintView> GetSalesOrderPrintView(string SalesOrderUID)
    {
        try
        {
            ...
            string sql = @"SELECT COALESCE(so.sales_order_number, so.draft_order_number) AS SalesOrderNumber,
                           so.status AS Status, so.order_type AS OrderType, s.code AS StoreCode, s.number AS StoreNumber, 
                           s.name AS StoreName, so.customer_po AS CustomerPO, so.order_date AS OrderDate,
                           so.expected_delivery_date AS ExpectedDeliveryDate, so.delivered_date_time AS DeliveredDateTime, 
                           c.symbol AS CurrencySymbol, so.total_amount AS TotalAmount, so.total_discount AS TotalDiscount,
                           so.total_tax AS TotalTax, so.net_amount AS NetAmount, so.line_count AS LineCount, so.qty_count AS QtyCount,
                            so.total_line_discount AS TotalLineDiscount, so.total_cash_discount AS TotalCashDiscount, 
                            so.total_header_discount AS TotalHeaderDiscount, so.total_excise_duty AS TotalExciseDuty, 
                           ...
                           INNER JOIN store s ON s.uid = so.store_uid
                           INNER JOIN currency c ON c.uid = so.currency_uid
                           INNER JOIN address a ON a.linked_item_uid = s.uid AND a.is_default = true
    ...
    public async Task<IEnumerable<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrderLinePrintView>>
        GetSalesOrderLinePrintView(string SalesOrderUID)
    {
        try
        {
            Dictionary<string, object?> parameters = new()
        ...
        DateTime endDate, string Status)
    {
        try
        {
            StringBuilder sql = new(@"SELECT * From (SELECT
                                            ...
                                            so.net_amount AS NetAmount, so.total_amount AS  TotalAmount,
                                            so.order_type AS OrderType,
                                            so.status AS Status,
                                            '[' || R.code || '] ' || R.name AS RouteName,
                                            --TO_CHAR(so.order_date, 'DD Mon YYYY') AS OrderDate,
                                            --TO_CHAR(so.expected_delivery_date, 'DD Mon YYYY') AS ""DeliveryDate"",
            ...
            string sql = @"SELECT DISTINCT
                        ...
                        WHERE
                            ...
                            SELECT
                            	...
                            	sol.uom AS UOM,
                            	CASE WHEN so.order_type IN ('PreSales', 'Draft') THEN sol.qty ELSE sol.reco_qty END,
                            ...
                            WHERE sol.sales_order_uid = @SalesOrderUID Order BY sol.line_number ASC;";
...
//{ "in_iswithprintedinvoicesflag", true },
            ...
            int count = await ExecuteStoredProcedureAsync(query, Parameters);
        ...
        IDbConnection? connection = null, IDbTransaction? transaction = null)
    {
        ...
        try
        {
            //ISalesOrder? existingRec = await SelectSalesOrder_ByUID(salesOrder.UID, connection, transaction);
    ...
    private async Task<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrder?> SelectSalesOrder_ByUID(string UID,
        IDbConnection? connection = null, IDbTransaction? transaction = null)
    {
        Dictionary<string, object?> parameters = new()
        {
        ...
        try
        {
            if (salesOrder != null)
            {
                ...
                Dictionary<string, object?> salesOrderParameters = new()
                {
                    ...
                    { "@ServerAddTime", salesOrder.ServerAddTime },
                    { "@ServerModifiedTime", salesOrder.ServerModifiedTime },
    ...
    private async Task<int> CUDSalesOrderLine_New(List<Winit.Modules.SalesOrder.Model.Interfaces.ISalesOrderLine> salesOrderLines,
        IDbConnection? connection = null, IDbTransaction? transaction = null)
    {
        int count = 0;
        try
        {
            ...
            //List<string> existingUIDs = await SelectSalesOrderLineByUID(uidList, connection, transaction);
            ...
            {
                _ = await UpdateSalesOrderLine(existingSalesOrderLines, connection, transaction);
            }
            if (newSalesOrderLines.Any())
            {
                _ = await CreateSalesOrderLine(newSalesOrderLines, connection, transaction);
            }
        }
        catch
        {
        ...
        IDbConnection? connection = null, IDbTransaction? transaction = null)
    {
        ...
        string sql = @"SELECT  uid FROM sales_order_line WHERE uid = ANY(@UIDs)";
        ...
        try
        {
            ...
            string salesOrderLineQuery = @"INSERT INTO sales_order_line (uid, ss,created_by, created_time,
                                     ...
                                     total_cash_discount, total_excise_duty, sku_uid,approved_qty,tax_data)
                                      ...
                                      @TotalCashDiscount, @TotalExciseDuty, @SKUUID,@ApprovedQty,@TaxData::json);";
    ...
    public async Task<int> InsertorUpdate_SalesOrders(Winit.Modules.SalesOrder.Model.Classes.SalesOrderViewModelDCO salesOrderView)
    {
        ...
        try
        {
            using Npgsql.NpgsqlConnection connection = PostgreConnection();
            ...
            try
            {
                ...
                {
                    count += await _collectionModuleDL.CUDAccPayable([salesOrderView.AccPayable], connection, transaction);
                }
    ...
    public async Task<List<ISalesOrderLineInvoice>> GetSalesOrderLineInvoiceItems(string salesOrderUID)
    {
        ...
        {
            string sql = """
                         SELECT SOL.sales_order_uid AS SalesOrderUID, SOL.uid AS SalesOrderLineUID, 
                         S.uid AS SKUUID, S.code AS SKUCode, 
                         S.Name AS SKUName,
