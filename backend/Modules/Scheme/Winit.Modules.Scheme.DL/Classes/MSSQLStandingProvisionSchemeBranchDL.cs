...
{
    public class MSSQLStandingProvisionSchemeBranchDL : SqlServerDBManager, IStandingProvisionSchemeBranchDL
    {
        public MSSQLStandingProvisionSchemeBranchDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
       ...
       int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT * FROM (SELECT 
                                    e.name as CreatedByEmpName, spsb.id, spsb.uid, spsb.created_by, spsb.created_time, spsb.modified_by, spsb.modified_time, spsb.server_add_time, 
									spsb.server_modified_time, spsb.ss, spsb.standing_provision_scheme_uid, spsb.branch_uid
                                FROM 
                                    standing_provision_scheme_branch spsb
									inner join emp e on e.uid=spsb.created_by
                                ) AS SubQuery");
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"SELECT COUNT(1) AS Cnt FROM 
                                ...
                                FROM 
                                    standing_provision_scheme_branch
                                ) AS SubQuery");
                }
        ...
        {
            var sql = @"
            UPDATE standing_provision_scheme_branch
                SET
                    id = @Id,
                    uid = @Uid,
            ...
            Dictionary<string, object> parameters = new Dictionary<string, object>
        {
            ...
            var sql = @"DELETE FROM standing_provision_scheme_branch WHERE uid = @UID";
