...
{
    public class SQLiteSettingDL:Winit.Modules.Base.DL.DBManager.SqliteDBManager, ISettingDL
    {
        public SQLiteSettingDL(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }
        ...
        int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT id AS Id, uid AS UID, type AS Type, name AS Name, value AS Value, data_type AS DataType,
                    is_editable AS IsEditable, ss AS SS, created_time AS CreatedTime, modified_time AS ModifiedTime, server_add_time AS ServerAddTime,
                    server_modified_time AS ServerModifiedTime FROM setting");
                var sqlCount = new StringBuilder();
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder("SELECT COUNT(1) AS Cnt FROM setting");
                }
        ...
        public async Task<Winit.Modules.Setting.Model.Interfaces.ISetting> GetSettingByUID(string UID)
        {
            ...
            var sql = @"SELECT id AS Id, uid AS UID, type AS Type, name AS Name, value AS Value, data_type AS DataType, 
                        is_editable AS IsEditable, ss AS SS, created_time AS CreatedTime, modified_time AS ModifiedTime, server_add_time AS ServerAddTime,
                        server_modified_time AS ServerModifiedTime FROM SETTING WHERE uid = @UID";
            Type type = _serviceProvider.GetRequiredService<Model.Interfaces.ISetting>().GetType();
        ...
        public async Task<int> CreateSetting(Winit.Modules.Setting.Model.Interfaces.ISetting createSetting)
        {
            try
            {
                var sql = "INSERT INTO setting ([uid],[type], [name],  [value],[date_type],[is_editable],[ss],[created_time],[modified_time],[server_add_time],[server_modified_time])" +
