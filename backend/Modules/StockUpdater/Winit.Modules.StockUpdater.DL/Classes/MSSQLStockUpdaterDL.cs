...
{
    public class MSSQLStockUpdaterDL : Winit.Modules.Base.DL.DBManager.SqlServerDBManager, IStockUpdaterDL
    {
        public MSSQLStockUpdaterDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
            ...
            object connection = null, object transaction = null)
        {
            ...
            try
            {
                returnValue = await ExecuteNonQueryAsync(query, (SqlConnection)connection, (SqlTransaction)transaction, parameters);
            }
            catch (Exception ex)
            {
                throw;
