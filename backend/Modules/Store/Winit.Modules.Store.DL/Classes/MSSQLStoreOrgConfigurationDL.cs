...
{
    public class MSSQLStoreOrgConfigurationDL : Base.DL.DBManager.SqlServerDBManager, Interfaces.IStoreOrgConfigurationDL
    {
        public MSSQLStoreOrgConfigurationDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
	...
	WHERE uid = @UID";
       ...
       server_add_time AS ServerAddTime, server_modified_time AS ServerModifiedTime, company_uid AS CompanyUID, org_uid AS OrgUID, 
       distribution_channel_uid AS DistributionChannelUID, store_uid AS StoreUID, name AS Name, code AS Code, value AS Value, parent_name AS ParentName
        FROM store_attributes WHERE uid =@UID";
