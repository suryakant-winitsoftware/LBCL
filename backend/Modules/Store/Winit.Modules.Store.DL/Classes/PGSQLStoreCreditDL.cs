...
{
    public class PGSQLStoreCreditDL : Base.DL.DBManager.PostgresDBManager, Interfaces.IStoreCreditDL
    {
        public PGSQLStoreCreditDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        }
            ...
            try
            {
                var sql = @"INSERT INTO store_credit (id, uid, created_by, created_time, modified_by, modified_time,
                          ...
                          is_negative_invoice_allowed) VALUES (@Id, @UID, @CreatedBy, @CreatedTime, @ModifiedBy, @ModifiedTime, @ServerAddTime, 
                            ...
                            @PreferredPaymentMethod, @PaymentType, @InvoiceAdminFeePerBillingCycle, @InvoiceAdminFeePerDelivery, @InvoiceLatePaymentFee,
        ...
        public async Task<PagedResponse<Model.Interfaces.IStoreCredit>> SelectAllStoreCredit(List<SortCriteria> sortCriterias,
            int pageNumber, int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                var sql = new StringBuilder(@"SELECT id AS Id,
                                                    ...
                                                    payment_term_uid AS PaymentTermUID,
                                                    credit_type AS CreditType,
                                                    credit_limit AS CreditLimit,
                                                    temporary_credit AS TemporaryCredit,
                                                    org_uid AS OrgUID,
                                                    distribution_channel_uid AS DistributionChannelUID,
                                                    preferred_payment_mode AS PreferredPaymentMode,
                                                    is_active AS IsActive,
                                                    is_blocked AS IsBlocked,
                                                    blocking_reason_code AS BlockingReasonCode,
                                                    blocking_reason_description AS BlockingReasonDescription,
                                                    price_list AS PriceList,
                                                    authorized_item_grp_key AS AuthorizedItemGrpKey,
                                                    message_key AS MessageKey,
                                                    tax_key_field AS TaxKeyField,
                                                    promotion_key AS PromotionKey,
                                                    disabled AS Disabled,
                                                    bill_to_address_uid AS BillToAddressUID,
                                                    ship_to_address_uid AS ShipToAddressUID,
                                                    outstanding_invoices AS OutstandingInvoices,
                                                    preferred_payment_method AS PreferredPaymentMethod,
                                                    payment_type AS PaymentType,
                                                    invoice_admin_fee_per_billing_cycle AS InvoiceAdminFeePerBillingCycle,
                                                    invoice_admin_fee_per_delivery AS InvoiceAdminFeePerDelivery,
                                                    invoice_late_payment_fee AS InvoiceLatePaymentFee,
                                                    is_cancellation_of_invoice_allowed AS IsCancellationOfInvoiceAllowed,
                                                    is_allow_cash_on_credit_exceed AS IsAllowCashOnCreditExceed,
                                                    is_outstanding_bill_control AS IsOutstandingBillControl,
                                                    is_negative_invoice_allowed AS IsNegativeInvoiceAllowed
                                                FROM public.store_credit ");
                var sqlCount = new StringBuilder();
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"SELECT COUNT(1) AS Cnt FROM public.store_credit");
                }
        ...
        public async Task<Model.Interfaces.IStoreCredit> SelectStoreCreditByStoreUID(string StoreUID)
        {
            ...
            var sql = @"SELECT id AS Id, 
                       ...
                       is_outstanding_bill_control AS IsOutstandingBillControl, 
                       is_negative_invoice_allowed AS IsNegativeInvoiceAllowed
                        FROM public.store_credit WHERE store_uid = @StoreUID";
            Type type = _serviceProvider.GetRequiredService<Model.Interfaces.IStoreCredit>().GetType();
            Model.Interfaces.IStoreCredit StoreCreditList = await ExecuteSingleAsync<Model.Interfaces.IStoreCredit>(sql, parameters, type);
            return StoreCreditList;
        }
        ...
        public async Task<List<IStoreCreditLimit>> GetCurrentLimitByStoreAndDivision(List<string> storeUIDs, string divisionUID)
        {
            try
            {
                var sql = @"
                      ...
                      FROM acc_payable
                      WHERE store_uid = ANY(@StoreUID)
                      AND (@DivisionUID = '' OR division_org_uid = @DivisionUID)
                      GROUP BY division_org_uid, store_uid
                      )
                      SELECT SC.store_uid AS StoreUID, SC.division_org_uid, O.Code AS Division,
                      COALESCE(SUM(SC.credit_limit), 0) AS CreditLimit,
                      COALESCE(SUM(SC.temporary_credit), 0) AS TemporaryCreditLimit,
                      COALESCE(SUM(SB.balance_amount), 0) AS CurrentOutstanding,
                      COALESCE(SUM(SC.credit_days), 0) AS CreditDays,
                      COALESCE(SUM(SC.temporary_credit_days), 0) AS TemporaryCreditDays,
                      MIN(SB.DueDate) AS DueDate,
                      COALESCE((CURRENT_DATE - MIN(SB.DueDate)), 0) AS MaxAgingDays,
                      ...
                      WHERE SC.store_uid = ANY(@StoreUID)
                      AND (@DivisionUID = '' OR SC.division_org_uid = @DivisionUID)
                      GROUP BY SC.store_uid, SC.division_org_uid, O.Code";

                var parameters = new Dictionary<string, object>
                {
                    { "@DivisionUID", divisionUID ?? string.Empty },
                    { "@StoreUID", storeUIDs.ToArray() }
                };

                Type type = _serviceProvider.GetRequiredService<IStoreCreditLimit>().GetType();
                var result = await ExecuteQueryAsync<IStoreCreditLimit>(sql, parameters, type);
                return result?.ToList() ?? new List<IStoreCreditLimit>();
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<List<IPurchaseOrderCreditLimitBufferRange>> GetPurchaseOrderCreditLimitBufferRanges()
        {
            try
            {
                ...
                Type type = _serviceProvider.GetRequiredService<IPurchaseOrderCreditLimitBufferRange>().GetType();
                var result = await ExecuteQueryAsync<IPurchaseOrderCreditLimitBufferRange>(sql, null, type);
                return result?.ToList() ?? new List<IPurchaseOrderCreditLimitBufferRange>();
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<int> UpdateStoreCreditStatus(List<IStoreCredit> storeCredits)
        {
            try
            {
                int totalUpdated = 0;
                foreach (var storeCredit in storeCredits)
                {
                    ...
                    totalUpdated += await ExecuteNonQueryAsync(sql, parameters);
                }
                return totalUpdated;
            }
            catch (Exception)
