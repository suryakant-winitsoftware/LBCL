...
public class PGSQLStoreGroupDL : Base.DL.DBManager.PostgresDBManager, Interfaces.IStoreGroupDL
{
    public PGSQLStoreGroupDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
    {
    }
       ...
       (List<SortCriteria> sortCriterias, int pageNumber, int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
    {
        try
        {
            StringBuilder sql = new(@"SELECT * FROM ( SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, 
                       ...
                       code AS Code, name AS Name, parent_uid AS ParentUID, item_level AS ItemLevel FROM public.store_group) AS SUBQUERY ");
            ...
            if (filterCriterias != null && filterCriterias.Count > 0)
            {
                ...
                if (filterCriterias.Any(e => e.Name == "store_group_hierarchy_level_code_name"))
                {
                    sbFilterCriteria.Append(" (code = @Code or name = @Name) ");
                    FilterCriteria filter = filterCriterias.Find(e => e.Name == "store_group_hierarchy_level_code_name")!;
            ...
            IEnumerable<Model.Interfaces.IStoreGroup> StoreGroups = await ExecuteQueryAsync<Model.Interfaces.IStoreGroup>(sql.ToString(),
                parameters, type);
            //Count
            int totalCount = 0;
            if (isCountRequired)
    ...
    public async Task<int> InsertStoreGroupHierarchy(string type, string uid)
    {
        ...
        {
            string hierarchySql = @"
                    WITH RECURSIVE store_group_hierarchy AS (
                    ...
                    sg1.store_group_type_uid AS GroupTypeUID, sg1.code AS GroupCode, sg1.name As GroupName, sg1.parent_uid AS GroupParentUID, 
                    sg1.item_level AS ItemLevel, sgt1.name AS GroupTypeName, sgt1.code AS GroupTypeCode, 
                    sgt1.parent_uid AS GroupTypeParentUID, sgt1.level_no AS LevelNo
                    FROM store_group sg1
                    INNER JOIN store_group_type sgt1 on sg1.store_group_type_uid = sgt1.uid
                    ...
                    sg2.store_group_type_uid AS GroupTypeUID, sg2.code AS GroupCode, sg2.name AS GroupName, sg2.parent_uid AS GroupParentUID, 
                    sg2.item_level AS ItemLevel, sgt2.name AS GroupTypeName, sgt2.code AS GroupTypeCode, 
                    sgt2.parent_uid AS GroupTypeParentUID, sgt2.level_no AS LevelNo
                    FROM store_group sg2
                    INNER JOIN store_group_type sgt2 ON sgt2.uid = sg2.store_group_type_uid
                    INNER JOIN store_group_hierarchy sgh ON sg2.uid = sgh.GroupParentUID
                    )
            ...
            var customStoreGroupDataList = new List<Dictionary<string, object>>();
            var labelBuilder = new StringBuilder();

            foreach (var storeGroup in storeGroupHierarchy)
            {
                if (storeGroup.LevelNo >= 1 && storeGroup.LevelNo <= 20)
                {
                ...
                labelBuilder.Insert(0, storeGroup.GroupName);
            ...
            var jsonData = Newtonsoft.Json.JsonConvert.SerializeObject(customStoreGroupDataList);
    ...
    public async Task<Model.Interfaces.IStoreGroup?> SelectStoreGroupByUID(string UID)
    {
        ...
        string sql = @"SELECT id AS Id, uid AS UID, created_by AS CreatedBy, created_time AS CreatedTime, modified_by AS ModifiedBy, 
                    ...
                    code AS Code, name AS Name, parent_uid AS ParentUID, item_level AS ItemLevel
	                FROM public.store_group WHERE uid = @UID";
        Type type = _serviceProvider.GetRequiredService<Model.Interfaces.IStoreGroup>().GetType();
        return await ExecuteSingleAsync<Model.Interfaces.IStoreGroup>(sql, parameters, type);
    }
