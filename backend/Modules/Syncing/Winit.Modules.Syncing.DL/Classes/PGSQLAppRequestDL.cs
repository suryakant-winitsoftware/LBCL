...
{
    public class PGSQLAppRequestDL : Base.DL.DBManager.PostgresDBManager, Interfaces.IAppRequestDL
    {
        public PGSQLAppRequestDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        ...
        private async Task<int> CreateAppRequest_Old(IAppRequest appRequest)
        {
            ...
            try
            {
                var Query = @"INSERT INTO app_request (
                    ...
                    @RequestReceivedByAPITime, @NextUID, @RequestBody::json, @RequestUIDs::json);";
                ...
                var Parameters = new Dictionary<string, object>
                {
                    ...
                    { "LinkedItemUID", appRequest.LinkedItemUID },
                    { "EmpUID", appRequest.EmpUID },
                    { "JobPositionUID", appRequest.JobPositionUID },
                    { "RequestCreatedTime", appRequest.RequestCreatedTime },
                    { "RequestPostedToAPITime", appRequest.RequestPostedToAPITime },
                    { "RequestReceivedByAPITime", appRequest.RequestReceivedByAPITime },
                    { "NextUID", appRequest.NextUID },
                    { "RequestBody", requestBodyJson },
                    { "RequestUIDs", requestUIDsJson },
                   };
                return await ExecuteNonQueryAsync(Query, Parameters);
            }
            catch (Exception)
