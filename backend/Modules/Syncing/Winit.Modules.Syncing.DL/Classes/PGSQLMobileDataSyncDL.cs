...
{
    public class PGSQLMobileDataSyncDL : Base.DL.DBManager.PostgresDBManager, Interfaces.IMobileDataSyncDL
    {
        public PGSQLMobileDataSyncDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
        ...
        public async Task<int> CreateDynamicTable(bool action, string jobPositionUID, string tableName)
        {
            try
            {
                if (action)
                {
                    ...
                    string createTableSql = $@"CREATE TABLE IF NOT EXISTS {tableName} ( store_uid VARCHAR(250) );";

                    string insertSql = $@"INSERT INTO {tableName} (store_uid)
                                    SELECT DISTINCT RC.store_uid 
                                    FROM route_user RU
                                    INNER JOIN route_customer RC ON RC.route_uid = RU.route_uid
                                    WHERE RU.job_position_uid = @JobPositionUID;";
                    try
                    {
                        // Execute table creation (DDL)
                        await ExecuteNonQueryAsync(createTableSql, null);  // Assume this handles DDL

                        // Insert data into the table (DML)
                        return await ExecuteNonQueryAsync(insertSql, parameters); // Returns affected rows
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("Error in CreateDynamicTable", ex);
                    }
                }
                else
                {
                    ...
                    try
                    {
                        await ExecuteNonQueryAsync(dropTableSql, null);
                        return -1; // No rows affected since it's DDL
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("Error in CreateDynamicTable - DROP block", ex);
                    }
                }
            }
            catch (Exception ex)
