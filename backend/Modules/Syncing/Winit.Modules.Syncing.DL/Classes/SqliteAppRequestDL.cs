...
{
    public class SqliteAppRequestDL : Base.DL.DBManager.SqliteDBManager, Interfaces.IAppRequestDL
    {
        public SqliteAppRequestDL(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        ...
        public async Task<int> UpdateAppRequest_IsNotificationReceived(List<string> uIDs, bool isNotificationReceived)
        {
            try
            {
                var query = $@"UPDATE app_request SET is_notification_received = '{CommonFunctions.GetBooleanInBeat(isNotificationReceived)}' 
                                WHERE UID IN @UIDs;";
        ...
        public async Task<IEnumerable<IAppRequest>> SelectAppRequestByUID(List<string> UIDs)
        {
            ...
            var sql = @"SELECT 
                            ...
                            ap.is_notification_received AS IsNotificationReceived
                        FROM 
                            app_request ap
                            WHERE
                            UID IN @UIDs;
        ...
        private async Task<int> CreateAppRequest(List<IAppRequest> appRequests)
        {
            try
            {
                var Query = @"INSERT INTO app_request (
                    ...
                    request_received_by_api_time, next_uid, request_body, request_uids, is_notification_received
                        ) VALUES (
                     @UID, @OrgUID, @LinkedItemType, @YearMonth, @LinkedItemUID, @EmpUID, 
                    @JobPositionUID, @RequestCreatedTime, @RequestPostedToAPITime, 
                    @RequestReceivedByAPITime, @NextUID, @RequestBody, @RequestUIDs, @IsNotificationReceived);";
