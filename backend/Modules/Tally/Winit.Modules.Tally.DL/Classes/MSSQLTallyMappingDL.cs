...
{
    public class MSSQLTallyMappingDL : Base.DL.DBManager.SqlServerDBManager, Interfaces.ITallyMappingDL
    {
        public MSSQLTallyMappingDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
                    ...
                    {                        
                        foreach (var data in inventoryFromTally)
                        {
                            ...
                            WHEN NOT MATCHED THEN
                                ...
                                gst_details, remote_alt_guid, distributor_code, created_time, modified_time, distributor_org_uid, qty)
                                VALUES (@Name, @Code, @Units, @LastRecordDate, TRIM(@OpeningBalance), TRIM(@OpeningRate), @StockGroup, TRIM(REPLACE(@Parent, CHAR(4), '')), 
                                @GSTDetails, @RemoteAltGUID, @DistributorCode, GETDATE(), GETDATE(), @DistributorOrgUID, @Qty);";
                            ...
                            Dictionary<string, object> parameters = new Dictionary<string, object>
                            {
                                ...
                                {"DistributorCode", data.DistributorCode ?? "" },
                                {"DistributorOrgUID", data.DistributorOrgUID ?? "" },
                                {"Qty", Shared.CommonUtilities.Common.CommonFunctions.ExtractNumericQuantity(data.OpeningBalance ?? "0")},
                            };
