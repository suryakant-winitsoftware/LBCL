...
{
    public class PGSQLUOMTypeDL : Winit.Modules.Base.DL.DBManager.PostgresDBManager, IUOMTypeDL
    {
        public PGSQLUOMTypeDL(IServiceProvider serviceProvider, IConfiguration config) : base(serviceProvider, config)
        {
       ...
       int pageSize, List<FilterCriteria> filterCriterias, bool isCountRequired)
        {
            try
            {
                ...
                if (isCountRequired)
                {
                    sqlCount = new StringBuilder(@"select count(1) as ""Cnt"" from uom_type;");
                }
                ...
                Type type = _serviceProvider.GetRequiredService<Model.Interfaces.IUOMType>().GetType();
                IEnumerable<Model.Interfaces.IUOMType> uOMTypes = await ExecuteQueryAsync<Model.Interfaces.IUOMType>(sql.ToString(), parameters, type);
                int totalCount = 0;
                if (isCountRequired)
                {
