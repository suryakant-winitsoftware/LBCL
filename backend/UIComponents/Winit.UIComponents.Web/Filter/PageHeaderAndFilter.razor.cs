...
{
    public partial class PageHeaderAndFilter
    {
        [Parameter]
        public RenderFragment ChildContent { get; set; }
        [Parameter]
        public Winit.UIModels.Web.Breadcrum.Interfaces.IDataService? IDataBreadcrumbService { get; set; }
        [Parameter]
            ...
            foreach (var column in ColumnsForFilter)
            {
                ...
                switch (column.FilterType)
                {
                    case FilterConst.DropDown:
                        ...
                        {
                            selectedValue = string.Join(",", column.IsCodeOnDDLSelect ? column.SelectedValues.Select(e => e.Code) : column.SelectedValues.Select(e => e.UID));
                        }
                        break;
                    ...
                    case FilterConst.CheckBox:
                        selectedValue = column.SelectedBoolValue.ToString();
                        break;
                }
        ...
        private async Task ResetFilter()
        {
            foreach (var column in ColumnsForFilter)
            {
                switch (column.FilterType)
                {
                    ...
                    case FilterConst.CheckBox:
                        column.SelectedBoolValue = false; // Reset checkbox to unchecked
                        break;
                }
            }
            await ApplyFilter();
        ...
        private string GetSearchCriteriaDisplay()
        {
            ...
            foreach (var column in ColumnsForFilter)
            {
                ...
                switch (column.FilterType)
                {
                    ...
                    case FilterConst.CheckBox:
                        filterValue = column.SelectedBoolValue ? "Checked" : "Unchecked";
                        break;
                }
