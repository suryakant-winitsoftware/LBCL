...
{
    public class PostgreSQLCommissionRepository : Interfaces.Commission.ICommissionRepository
    {
        private readonly string _connectionString;
        public PostgreSQLCommissionRepository(IConfiguration config)
        {
        ...
        public async Task<int> ProcessCommission()
        {
            ...
            //    List<CommissionKPI> commissionKPIList = await new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionKPI>(_connectionString)
            //.ExecuteQueryAsync(Commission_Query.CommissionKPI, emptyParameters);


            Task<List<WINITSharedObjects.Models.CommissionKPI>> commissionKPIListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionKPI>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CommissionKPI, emptyParameters);
            await Task.WhenAll(commissionKPIListTask);
            IEnumerable<WINITSharedObjects.Models.CommissionKPI> commissionKPIListTest = commissionKPIListTask.Result;
            if (1 == 1) {
                
            }

            Task<List<WINITSharedObjects.Models.CommissionKPICustomerMapping>> commissionKPICustomerMappingListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionKPICustomerMapping>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CommissionKPICustomerMapping, emptyParameters);

            Task<List<WINITSharedObjects.Models.CommissionKPIProductMapping>> CommissionKPIProductMappingListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionKPIProductMapping>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CommissionKPIProductMapping, emptyParameters);

            Task<List<WINITSharedObjects.Models.CommissionKPISlab>> CommissionKPISlabListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionKPISlab>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CommissionKPISlab, emptyParameters);

            Task<List<WINITSharedObjects.Models.CommissionUserMapping>> CommissionUserMappingListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionUserMapping>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CommissionUserMapping, emptyParameters);

            Task<List<WINITSharedObjects.Models.FactSales>> FactSalesListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.FactSales>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.FactSales, emptyParameters);

            Task<List<WINITSharedObjects.Models.FactSalestarget>> FactSalestargetListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.FactSalestarget>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.FactSalestarget, emptyParameters);

            Task<List<WINITSharedObjects.Models.CustomerAttributes>> CustomerAttributesListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.CustomerAttributes>(_connectionString)
                .ExecuteQueryAsync(Commission_Query.CustomerAttributes, emptyParameters);


            Task<List<WINITSharedObjects.Models.ProductAttributes>> ProductAttributesListTask = new DBManager.PostgresDBManager<WINITSharedObjects.Models.ProductAttributes>(_connectionString)
            ...
            foreach (WINITSharedObjects.Models.CommissionKPI objCommission in commissionKPIList)
            {
                ...
                List<CommissionUserKPIPerformance> CommissionUserKPIPerformanceList = await new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionUserKPIPerformance>(_connectionString)
                                        .ExecuteQueryAsync(string.Format(Commission_Query.CommissionUserKPIPerformance, comm_kpi_id), emptyParameters);
                List<CommissionUserPayout> CommissionUserPayoutList = await new DBManager.PostgresDBManager<WINITSharedObjects.Models.CommissionUserPayout>(_connectionString)
                ...
                {
                    await new DBManager.PostgresDBManager<int>(_connectionString).ExecuteNonQueryAsync(sbInsUserKPI.ToString(), emptyParameters);
                }
                if (sbUpdUserKPI.ToString() != "")
                {
                    await new DBManager.PostgresDBManager<int>(_connectionString).ExecuteNonQueryAsync(sbUpdUserKPI.ToString(), emptyParameters);
                }
            ...
            foreach (WINITSharedObjects.Models.CommissionUserPayout objCommission in CommissionUserPayoutReProcess)
            {
                ...
                {
                    await new DBManager.PostgresDBManager<int>(_connectionString).ExecuteNonQueryAsync(sbInsUserPayout.ToString(), emptyParameters);
                }
                if (sbUpdUserPayout.ToString() != "")
                {
                    await new DBManager.PostgresDBManager<int>(_connectionString).ExecuteNonQueryAsync(sbUpdUserPayout.ToString(), emptyParameters);
                }
            }
        ...
        {
            public const string CommissionKPI = @"select C.commission_id,C.org_code,C.commission_name,CP.kpi_type_id, CP.kpi_name,CP.kpi_structure_type,CP.kpi_weight  from winit.commission C inner join winit.commissionkpi CP on CP.commission_id=C.commission_id" +
                " where  CURRENT_DATE Between C.commission_start_date and C.commission_end_date";
        ...
        public class Commission_QueryOld
        {
            public const string CommissionKPI = @"select C.commission_id,C.org_code,C.commission_name,CP.kpi_type_id, CP.kpi_name,CP.kpi_structure_type,CP.kpi_weight  from ""Commission"" C inner join ""CommissionKPI"" CP on CP.commission_id=C.commission_id" +
                " where  CURRENT_DATE Between C.commission_start_date and C.commission_end_date";
