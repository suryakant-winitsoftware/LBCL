...
namespace SyncManagerDL.Classes
{
    ...
    {
        private readonly Int_ApiService _apiService;
        ...
        public async Task<IEntityData> GetEntityData(string EntityName)
        {
            try
            {
                ...
                return await ExecuteSingleAsync<IEntityData>(sql.ToString(), parameters);
            }
            catch
            {
                throw;
        ...
        public async Task<int> InsertTemporaryCreditLimitsIntoMainTable(string TableName, string TablePrefix, long SyncLogDetailId)
        {
            try
            {
                ...
                var sql = new StringBuilder($@" exec usp_CMI_temporary_credit_Limit_Process 
                              @strSyncLogDetailId = @SyncLogDetailId, 
                              @strTableName = @TableName, 
                              @strTablePrefix = @TablePrefix");
                return await ExecuteNonQueryAsync(sql.ToString(), Parameters);
        ...
        public async Task<int> InsertPayThroughAPMastersIntoMainTable(string TableName, string TablePrefix, long SyncLogDetailId)
        {
            try
            {
                ...
                var sql = new StringBuilder($@" exec usp_PayThroughAPMaster_Process 
                              @strSyncLogDetailId = @SyncLogDetailId, 
                              @strTableName = @TableName, 
                              @strTablePrefix = @TablePrefix");
                //return await ExecuteNonQueryAsync(sql.ToString(), Parameters);
                var uids = await ExecuteQueryAsync<string>(sql.ToString(), Parameters);
                bool prepareSKU = false;
                if (uids.Count > 0)
                    prepareSKU = await InvokePrepareSKUMasterAPI(uids);
                return prepareSKU ? 1 : -1;
            }
        ...
        public async Task<int> InsertCustomerReferenceIntoMainTable(string TableName, string TablePrefix, long SyncLogDetailId)
        {
            try
            {
                ...
                var sql = new StringBuilder($@" exec usp_customer_Reference_Process 
                              @strSyncLogDetailId = @SyncLogDetailId, 
                              @strTableName = @TableName, 
                              @strTablePrefix = @TablePrefix");
                return await ExecuteNonQueryAsync(sql.ToString(), Parameters);
        ...
        public async Task<int> SaveOrUpdateInvoiceBlobFilePath(SyncManagerModel.Interfaces.IInt_InvoiceHeader invoiceHeader, string path)
        {
            try
            {
                ...
                ,modified_time=getdate(),server_modified_time=getdate() where linked_item_uid=@DeliveryId");
        ...
        public async Task<int> CheckisJobEnable(string Entity)
        {
            try
            {
                var sql = new StringBuilder($@"select is_job_enable from int_entity where [name]=@Entity ");
                var parameters = new Dictionary<string, object?>()
                {
                    {"Entity",Entity }
                };
                int isERPposting = await ExecuteScalarAsync<int>(sql.ToString(), parameters);
                return isERPposting;
