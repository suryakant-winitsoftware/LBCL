...
{
    public partial class AddCustomerDetails : BaseComponentBase
    {
        ...
        public async Task OnBoardingSetup()
        {
            try
            {
                ...
                if (IsEditOnBoardDetails)
                {
                    ...
                    BranchUID = _customerDetailsViewModel.EditOnBoardingDetails.Address.Where(p => p.IsDefault).FirstOrDefault().BranchUID;
        ...
        public async Task AddFetchDetails()
        {
            try
            {
                ...
                {
                    await _alertService.ShowErrorAlert("GST", "Please enter GST Number");
                    return;
                }
                else
                {
                    if (await _customerDetailsViewModel.IsGstUnique(gstNumber))
                    {
                        ...
                        {
                            await _alertService.ShowErrorAlert("GST", "Please enter valid GST number");
                        }
                    }
                    else
                    {
                        await _alertService.ShowErrorAlert("GST", "GST number already exists");
                    }
            ...
            {
                await _alertService.ShowErrorAlert("GST", "GST Server is down. Please try again later.");
            }
            finally
            {
                _loadingService.HideLoading();
        ...
        protected async Task SaveOrUpdateCustomerInformation(IOnBoardCustomerDTO? onBoardCustomerDTO)
        {
            ...
            if (!IsEditOnBoardDetails && string.IsNullOrEmpty(_customerDetailsViewModel.CustomerCreationStoreUID))
            {
                if (await _customerDetailsViewModel.SaveUpdateCustomerInformation(onBoardCustomerDTO, true))
                {
                    ...
                    await _alertService.ShowSuccessAlert("Customer", "Customer details Saved Successfully");
                    IsSuccessCustomerInformation = true;
                    _customerDetailsViewModel.CustomerCreationStoreUID = onBoardCustomerDTO.Store.UID;
                }
                else
                {
                    await _alertService.ShowErrorAlert("Error", "Failed to save...");
                }
            }
            else
            {
                ...
                {
                    await _alertService.ShowSuccessAlert("Customer", "Customer details Updated Successfully");
                }
                else
                {
                    await _alertService.ShowErrorAlert("Error", "Failed to Update...");
                }
            }
            _loadingService.HideLoading();
        ...
        protected async Task SaveOrUpdateContact(IContact? contact)
        {
            ...
            {
                await _alertService.ShowErrorAlert("Error", "A contact with the same mobile number or email already exists.");
                _loadingService.HideLoading();
                return;
            }
            if (contact.IsDefault)
            {
                ...
                if (primaryContact != null)
                {
                    ...
                    await _alertService.ShowErrorAlert("Error", "Kindly check there is already one primary contact saved if you want to make it as primary then make the existing contact as not primary.");
                    _loadingService.HideLoading();
                    return;
                }
            }
            if (!IsEdit)
            {
                if (_customerDetailsViewModel.StoreUID != null)
                {
                    if (await _customerDetailsViewModel.SaveUpdateContact(contact, true))
                    {
                        if (IsBackBtnClicked)
                        {
                            await _alertService.ShowSuccessAlert("", "Data Saved Successfully");

                        }
                        else
                        {
                            await _alertService.ShowSuccessAlert("Contact", "Contact details Saved Successfully");
                ...
                {
                    await _alertService.ShowErrorAlert("Error", "First fill the Customer Information...");
                }
            }
            else
            {
                ...
                {
                    await _alertService.ShowSuccessAlert("Contact", "Contact details Updated Successfully");
                    IsContactSuccess = true;
                    await _contactDetails.OnClean();
                }
                else
        ...
        protected async Task SaveOrUpdateBillToAddress(IAddress? address)
        {
            ...
            //{
            //    await _alertService.ShowErrorAlert("Error", "A address with the same mobile number or email already exists.");
            //    _loadingService.HideLoading();
            //    return;
            //}
            if (address.IsDefault)
            {
                ...
                {
                    // Show an error if a primary contact already exists
                    await _alertService.ShowErrorAlert("Error", "Kindly check there is already one primary address saved if you want to make it as primary then make the existing address as not primary.");
                    _loadingService.HideLoading();
                    return;
                }
            }
            if (!IsEdit)
            {
                if (_customerDetailsViewModel.StoreUID != null)
                {
                    if (await _customerDetailsViewModel.SaveUpdateBillToAddress(address, true))
                    {
                        ...
                        {
                            await _alertService.ShowSuccessAlert("Bill To Address", "Bill To Address details Saved Successfully");
            ...
            else
            {
                if (await _customerDetailsViewModel.SaveUpdateBillToAddress(address, false))
                {
                    ...
                    {
                        await _alertService.ShowSuccessAlert("Bill To Address", "Bill To Address details Updated Successfully");
                        IsBillToAddressSuccess = true;
                    }
                }
                else
        ...
        protected async Task SaveOrUpdateShipToAddress(IAddress? address)
        {
            ...
            if (!IsEdit)
            {
                if (_customerDetailsViewModel.StoreUID != null)
                {
                    if (await _customerDetailsViewModel.SaveUpdateShipToAddress(address, true))
                    {
                        ...
                        {
                            await _alertService.ShowSuccessAlert("Ship To Address", "Ship To Address details Saved Successfully");
                        }
                    }
                    else
                    {
            ...
            else
            {
                if (await _customerDetailsViewModel.SaveUpdateShipToAddress(address, false))
                    ...
                    {
                        await _alertService.ShowSuccessAlert("Ship To Address", "Ship To Address Details Updated Successfully");
                    }
                else
                {
                    await _alertService.ShowErrorAlert("Error", "Failed to Update...");
        ...
        protected async Task SaveOrUpdateeEmployeeDetails(IStoreAdditionalInfoCMI? storeAdditionalInfoCMI)
        {
            ...
            if (_customerDetailsViewModel.StoreUID != null)
            {
                if (await _customerDetailsViewModel.SaveUpdateEmployeeDetails(storeAdditionalInfoCMI))
                {
                    ...
                    {
                        await _alertService.ShowSuccessAlert($"{storeAdditionalInfoCMI.SectionName}", $"{storeAdditionalInfoCMI.SectionName} details Delete Successfully");
                    }
                    else
                    {
                        if (!IsEditOnBoardDetails)
                        {
                            await _alertService.ShowSuccessAlert($"{storeAdditionalInfoCMI.SectionName}", $"{storeAdditionalInfoCMI.SectionName} details Saved Successfully");
                        }
                        else
                        {
                            await _alertService.ShowSuccessAlert($"{storeAdditionalInfoCMI.SectionName}", $"{storeAdditionalInfoCMI.SectionName} details Updated Successfully");
                        }
                    }
                }
                else
                {
                    if (storeAdditionalInfoCMI.Action == "Delete")
                    {
                        await _alertService.ShowErrorAlert("Error", "Failed to Delete...");
                    }
                    else
                    {
                        await _alertService.ShowErrorAlert("Error", "Failed to save...");
            ...
            try
            {
                if (_customerDetailsViewModel.StoreUID != null)
                {
                    if (fileSysList.Count > 0)
                    {
                        ...
                        _ = _alertService.ShowSuccessAlert("File", "File Uploaded Successfully");
                    }
                    else
                    {
                        await _alertService.ShowErrorAlert("File", "File Uploaded Failed");
                    }
                }
                else
                {
        ...
        protected async Task OnDeleteBill_ShiptoAddress(string addressUID)
        {
            ...
            if (await _AlertMessgae.ShowConfirmationReturnType("Delete", "Are you sure want to Delete this?"))
            {
                try
                {
                    ...
                    {
                        await _alertService.ShowSuccessAlert("Ship To Address", "Bill To Address Deleted Successfully");
        ...
        public async Task CreateUpdateDocument(List<IFileSys> fileSys)
        {
            if (fileSys.Count > 0)
            {
                ...
                StateHasChanged();
                 await _alertService.ShowSuccessAlert("File", "File Uploaded Successfully");
            }
            else
            {
                await _alertService.ShowErrorAlert("File", "File Uploaded Failed");
        ...
        public async Task CreateUpdateDocumentAppendix(List<List<IFileSys>> fileSys)
        {
            ...
            if (_customerDetailsViewModel.StoreUID != null)
            {
                if (fileSys.Count > 0)
                {
                    ...
                    if (result)
                    {
                         await _alertService.ShowSuccessAlert("File", "Files Uploaded Successfully");
                    }
                    else
                    {
                        await _alertService.ShowErrorAlert("File", "File Upload Failed");
                    }
                    StateHasChanged();
                }
                else
                {
                    await _alertService.ShowErrorAlert("File", "Pllease Upload Files");
                }
            }
            else
            {
        ...
        public async Task CreateUpdateDocumentAppendix1(List<List<IFileSys>> fileSys)
        {
            if (_customerDetailsViewModel.StoreUID != null)
            {
                ...
                {
                    //_ = await _alertService.ShowErrorAlert("File", "Pllease Upload Files", Winit.UIComponents.SnackBar.Enum.Severity.Error);
                }
            }
            else
            {
        ...
        public async Task AsmConfirm()
        {
            ...
            {
                await _alertService.ShowSuccessAlert("Data", "Data Saved Successfully");
                // await executeRule();
                _navigationManager.NavigateTo("CustomerDetails");
            }
            else
        ...
        public async Task CheckForBM()
        {
            try
            {
                if (BranchBM.LoginId == _appUser.Emp.LoginId && _customerDetailsViewModel.EditOnBoardingDetails.Store.Status == SalesOrderStatus.PENDING_FROM_BM)
                {
        ...
        public bool CheckReportingEmpUID()
        {
            try
            {
                if (_appUser != null)
                {
                    return _appUser.Emp.UID == _customerDetailsViewModel.EditOnBoardingDetails.Store.ReportingEmpUID;
                }
                else
                {
                    return false;
        ...
        private async Task<ApprovalActionResponse> HandleApprovalAction(ApprovalStatusUpdate approvalStatusUpdate)
        {
            ...
            try
            {
                ...
                if (await _customerDetailsViewModel.SaveUpdateDistDetails(storeApprovalDTO))
                {
                    _customerDetailsViewModel._onBoardCustomerDTO.Store.FranchiseeOrgUID = _appUser.SelectedJobPosition.OrgUID ?? "";
                    ...
                    if (isSuccess)
                    {
                        await _alertService.ShowSuccessAlert("Data", "Customer OnBoarded Successfully");
                        //_loadingService.HideLoading();
                    }
                    //await _alertService.ShowErrorAlert("Data", "Data Saved Successfully", Winit.UIComponents.SnackBar.Enum.Severity.Success);
                    // await executeRule();
                    _navigationManager.NavigateTo("CustomerDetails");
                    StateHasChanged();
                }
                ...
                {
                    await _alertService.ShowErrorAlert("Error", "Failed to save store status...");
                }
                approvalActionResponse.IsSuccess = await _customerDetailsViewModel.SaveUpdateDistDetails(storeApprovalDTO);
                return approvalActionResponse;
            }
        ...
        //            if (isSuccess)
        //            {
        //                await _alertService.ShowErrorAlert("Data", "Customer OnBoarded Successfully", Winit.UIComponents.SnackBar.Enum.Severity.Success);
        //                //_loadingService.HideLoading();
        //            }
        //            //await _alertService.ShowErrorAlert("Data", "Data Saved Successfully", Winit.UIComponents.SnackBar.Enum.Severity.Success);
        //            // await executeRule();
        ...
        //    ErrorMessage = error;
        //    await await _alertService.ShowErrorAlert("Approval Engine", ErrorMessage);
        //}
        ...
        //    else
        //    {
        //        await await _alertService.ShowErrorAlert("Error", ErrorMessage);
        //    }
        //}
        ...
        //    }
        //    else
        //        await await _alertService.ShowErrorAlert("Error", res.Message);
        //}
        ...
        public bool CheckApprovalTracker()
        {
            try
            {
                if (ApprovalTracker != null && ApprovalTracker.Count > 0)
                {
                    ...
                    {
                        if (_appUser?.Role?.Code == item.ApproverId && item.ApproverLevel == highestApprovedLevel + 1)
                        {
                            return true;
                        }
                    }
        ...
        //        //    //_loadingService.HideLoading();
        //        //}
        //        await _alertService.ShowErrorAlert("Reject", "Successfully rejected", Winit.UIComponents.SnackBar.Enum.Severity.Success);
        //        _navigationManager.NavigateTo("CustomerDetails");
        ...
        public async Task<bool> ValidateBilltoAddressDetails()
        {
            try
            {
                ...
                if (hasErrors)
                {
                    ...
                    {
                        await _alertService.ShowErrorAlert("Alert", "Atleast One Primary address is needed in BillTo Address");
                    }
                }
                else
                {
        ...
        public async Task<bool> ValidateShiptoAddressDetails()
        {
            try
            {
                ...
                if (hasErrors)
                {
                    ...
                    if (!_customerDetailsViewModel.ShipToAddresses.Any(p => p.IsDefault))
                    {
                        await _alertService.ShowErrorAlert("Alert", "Atleast One Primary address is needed in ShipTo Address");
                    }
                }
                else
                {
        ...
        public async Task AssignToAsm()
        {
            try
            {
                ...
                {
                    await _alertService.ShowErrorAlert("Alert", "Please select Asm");
                }
            }
            catch (Exception ex)
            {
                await _alertService.ShowErrorAlert("Alert", "Exception");
            }
        }
