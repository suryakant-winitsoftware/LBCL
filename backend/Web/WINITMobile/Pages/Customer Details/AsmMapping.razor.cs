...
{
    public partial class AsmMapping : BaseComponentBase
    {
            ...
            try
            {
                if (AsmDivisionDetails.Any())
                {
                    ...
                    if (IsAsmMappingSuccess)
                        await _alertService.ShowSuccessAlert("", "ASM mapped for Divisions Successfully");
                    else
                        await _alertService.ShowErrorAlert("", "ASM mapping Failed...");
                }
                else
                {
                    await _alertService.ShowErrorAlert("", "Please add Division and ASM");
                }
            }
            catch (Exception ex)
            {
