...
{
    public partial class StoreAsmDivisionMapping : BaseComponentBase
    {
        ...
        public async Task InsertIntoDB(List<StoreAsmMapping> storeAsmDetails)
        {
            try
            {
                ...
                {
                    await _alertService.ShowSuccessAlert("Success", "Data saved successfully");
                }
                else
                {
            ...
            {
                await _alertService.ShowErrorAlert("Error", "Exception");
            }
            finally
            {
                await _storeAsmMappingViewModel.PopulateViewModel();
