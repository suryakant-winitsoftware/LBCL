...
public partial class PurchaseOrderBasePage : BaseComponentBase
{
    [Parameter]
    public string PurchaseOrderUID { get; set; } = string.Empty;
    private WINITMobile.Pages.DialogBoxes.AddProductDialogBoxV1<ISKUV1>? AddProductDialogBox;
    ...
    private bool IsForApproval => /*_appUser.Role.IsPrincipalRole &&*/ !_viewModel.IsNewOrder;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            ...
            if (/*!_appUser.Role.IsDistributorRole &&*/ string.IsNullOrEmpty(PurchaseOrderUID))
            {
    ...
    private async Task OnTemlateSelect(DropDownEvent dropDownEvent)
    {
        ...
        if (string.IsNullOrEmpty(_viewModel.PurchaseOrderHeader.ShippingAddressUID))
        {
            ...
            ShowAlert("Alert", "Please Select ShipTo Address.");
        }
        try
        {
    ...
    private async Task OnSaveAsDraftClick()
    {
        ...
        catch (CustomException ex)
        {
            if (ex.Status == Winit.Shared.Models.Enums.ExceptionStatus.Success)
            {
                ShowAlert("Purchase Order", ex.Message);
    ...
    private async Task OnDistributorSelect()
    {
        try
        {
            ...
            {
                ShowAlert("Failed", "Please Select Distributor...");
                return;
            }
            ...
            {
                await _alertService.ShowErrorAlert("Alert", $"This customer is blocked due to reason:  {selectedStore.BlockedReasonDescription ?? "N/A"} ");
                return;
            }
    ...
    private bool OnItemSelect(ISKUV1 item)
    {
        ...
        {
            ShowAlert("Alert", "This model is not part of AP master please contact CMI team ");
        }
