...
namespace WinIt.Pages.Maintain_Warehouse
{
    public partial class MaintainWareHouseStock :BaseComponentBase
    {
        ...
        {
            HeaderText = "Maintain Warehouse Stock",
            BreadcrumList = new List<IBreadCrum>()
            {
                new BreadCrumModel(){SlNo=1,Text="Maintain Warehouse Stock"},
            }
        };
        ...
        private async Task HandleWarehouseSelection(DropDownEvent eventArgs)
        {
            ...
            await _warehousestockViewModel.PopulateViewModelForSubWareHouseDD(OrgTypeConst.SWH, uID, _iAppUser.SelectedJobPosition.BranchUID);
            _loadingService.HideLoading();
        ...
        private bool SelectionsValidated()
        {
            ...
            {
                ShowErrorSnackBar("Error", "Warehouse cannot be null or empty.");
                return false;
            }
            ...
            {
                ShowErrorSnackBar("Error", "Please select atleast one organisation unit.");
                return false;
            }
            else
            {
