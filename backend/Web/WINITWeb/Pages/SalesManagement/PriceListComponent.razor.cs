...
namespace WinIt.Pages.SalesManagement
{
    public partial class PriceListComponent : ComponentBase
    {
        [Parameter]
        ...
        WinIt.Pages.DialogBoxes.AddProductDialogBoxV1<Winit.Modules.SKU.Model.Interfaces.ISKUV1>? AddProductDialogBox { get; set; }
        private int count = 0;
        ...
        bool initializeProductComponent = false;
        protected override async Task OnInitializedAsync()
        {
            LoadResources(null, _languageService.SelectedCulture);
            DropDownDataSource.Clear();
            var ddds = await _addProductPopUpDataHelper.GetSKUAttributeData();
            if (ddds != null)
                DropDownDataSource.AddRange(ddds);

            await _skuPriceViewModel.GetAllProducts();
            initializeProductComponent = true;
        }

        [Parameter] public List<Winit.Modules.SKU.Model.Interfaces.ISKUV1> SKUList { get; set; } = [];

        private List<SKUAttributeDropdownModel> DropDownDataSource { get; set; } = [];
        ...
        public void GetValidated(out bool isValidated, out string errorMessage)
        {
            isValidated = true;
        ...
        public List<Winit.Modules.SKU.Model.Interfaces.ISKUPrice> GetModifiedSKUPricesList(string status)
        {
            ...
            Parallel.ForEach(SKUPriceList, sKUPrice =>
            {
        ...
        public async Task SaveOrUpdateIndividual(Winit.Modules.SKU.Model.Interfaces.ISKUPrice sKUPrice, bool isSave)
        {
            ...
            if (isSave)
            {
                ...
                List<Winit.Modules.SKU.Model.Interfaces.ISKUPrice> sKUPriceList = sKUPrice.IsLatest == 1 ?
                    SKUPriceList.FindAll(p => p.SKUUID == sKUPrice.SKUUID) : new List<Winit.Modules.SKU.Model.Interfaces.ISKUPrice>() { sKUPrice };
