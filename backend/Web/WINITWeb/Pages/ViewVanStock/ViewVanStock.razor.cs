...
namespace WinIt.Pages.ViewVanStock
{
    public partial class ViewVanStock
    {
        [CascadingParameter]
        ...
        {
            HeaderText = "View Van Stock",
            BreadcrumList = new List<IBreadCrum>()
            {
                new BreadCrumModel(){SlNo=1,Text="View Van Stock"},
            }
        };
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                ...
                {
                 new SelectionItem{ Label=@Localizer["salable"], Code="Salable", UID="1"},
                ...
                _warehousestockViewModel.OrgTypeUID = "VWH";
                await _warehousestockViewModel.PopulateViewModelForVanDD();
                _warehousestockViewModel.selectedwarehouseUID = _warehousestockViewModel.VanItemViewListFrmORG.First().UID;
                //_warehousestockViewModel.selectedwarehouseUID = _iAppUser.SelectedRoute.WHOrgUID;
                _warehousestockViewModel.FranchiseeOrgUID = _iAppUser.SelectedJobPosition.OrgUID;
                await _warehousestockViewModel.PopulateViewModelForSalableVan();
                _warehousestockViewModel.VanSelectionItems[0].IsSelected = true;
                StateHasChanged();
        ...
        {
            new FilterModel { FilterType = Winit.Shared.Models.Constants.FilterConst.TextBox, Label =@Localizer["skucode/name"] ,ColumnName = "SKUCode"},
        };
        }
        ...
        public async void ApplyFilter(Dictionary<string, string> filterCriterias)
        {
            ...
            await _warehousestockViewModel.ApplyFilterVan(criteriaList);
            _loadingService.HideLoading();
            StateHasChanged();
        ...
        private async Task HandleVanSelection(DropDownEvent eventArgs)
        {
            _loadingService.ShowLoading();
            if (eventArgs != null && eventArgs.SelectionItems != null && eventArgs.SelectionItems.Count > 0)
            {
                ...
                await _warehousestockViewModel.PopulateViewModelForSalableVan();
                await _warehousestockViewModel.PopulateViewModelForFocVan();
