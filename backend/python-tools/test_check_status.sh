#!/bin/bash
TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..r-qSi8dcoPooe-EUeI-MwMdYc0ESE0Tac6kRWbLVFpg"

echo "=== Check change request status ===" curl -s -X GET "https://multiplex-promotions-api.winitsoftware.com/api/ApprovalEngine/GetChangeRequestDataByUid?requestUid=SUCCESS_TEST_1758196090" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" | python3 -m json.tool | grep -E "(Status|UID)"
