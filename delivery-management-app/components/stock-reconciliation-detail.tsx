...
export function StockReconciliationDetail() {
  ...
  const [stockCounts, setStockCounts] = useState<Record<string, number>>(
    mockStockData.reduce((acc, item) => ({ ...acc, [item.id]: item.totalStockCount }), {}),
  )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 sticky top-0 z-10">
        <div className="flex items-center justify-between max-w-[1600px] mx-auto">
          <NavigationMenu />
          <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 flex-1 text-center px-4">
            Perform Warehouse Stock Take Reconciliation
          </h1>
        ...
        <div className="max-w-[1600px] mx-auto flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-6 flex-1">
            ...
            <div>
              ...
              <p className="text-base font-bold text-gray-900">[5844] LBCL WAREHOUSE</p>
            </div>
            <div>
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={handleSubmit} className="bg-[#A08B5C] hover:bg-[#8A7549] text-white px-8">
              Submit
            </Button>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#FFF8E7] border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[100px]">Deliveries</th>
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[120px]">
                  Credits
                  <br />
                  <span className="text-xs font-normal">(Warehouse & Customer)</span>
                </th>
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[120px]">Adjustments</th>
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[120px]">
                  Closing Balance
                </th>
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[120px]">
                ...
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900 min-w-[100px]">Variance</th>
              </tr>
            </thead>
            <tbody>
              {mockStockData.map((item) => {
                const currentStockCount = stockCounts[item.id] || 0
                const variance = calculateVariance(item.closingBalance, currentStockCount)
                ...
                return (
                  <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.openBalance}</td>
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.stockReceipt}</td>
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.deliveries}</td>
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.credits}</td>
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.adjustments}</td>
                    <td className="px-4 py-4 text-center text-sm text-gray-900">{item.closingBalance}</td>
                    <td className="px-4 py-4">
                      <div className="flex justify-center">
                        <input
                          ...
                          onChange={(e) => handleStockCountChange(item.id, e.target.value)}
                    ...
                    <td className="px-4 py-4 text-center text-sm font-semibold text-gray-900">{variance}</td>
                  </tr>
                )
              })}
            </tbody>
