...
export function TruckLoadingList() {
  ...
  const [activeTab, setActiveTab] = useState<"ALL" | "PENDING" | "APPROVED" | "SHIPPED">("ALL")
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 max-w-4xl mx-auto space-y-4">
        {filteredRequests.map((request) => (
          <div
            key={request.id}
            onClick={() => router.push(`/truck-loading/${request.id}`)}
          ...
          >
            ...
            <div className="p-4 flex items-center justify-between">
              <div className="flex-1 grid grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="font-bold text-gray-900">{request.loadRequestNo}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">Distributor</div>
                  <div className="font-bold text-gray-900">{request.distributor}</div>
                </div>
                <div>
            ...
            <div className="bg-[#FFF8E7] px-4 py-3 flex items-center justify-between border-t border-gray-100">
              <div>
                ...
                <div className="text-lg font-bold text-gray-900">{request.collectedQty}</div>
              </div>
              <div className="h-12 w-px bg-gray-300" />
              <div className="text-right">
                <div className="text-xs text-gray-600 mb-1">Approval Status</div>
                <div className={`text-lg font-bold ${getStatusColor(request.approvalStatus)}`}>
      ...
      <button
        onClick={() => router.push("/truck-loading/new")}
