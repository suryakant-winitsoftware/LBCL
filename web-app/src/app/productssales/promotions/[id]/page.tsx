...
export default function PromotionDetailPage() {
  ...
  const [promotionV3Data, setPromotionV3Data] =
    useState<Partial<PromotionV3FormData> | null>(null);
  ...
  const fetchPromotionDetails = useCallback(async () => {
    try {
      ...
      try {
        ...
        if (v3Response.success && v3Response.data) {
          ...
          const legacyFormat: any = {
            ...
            ReconstructedFootprintData: {
              SelectionType:
                v3Data.OrganizationSelections?.length > 0 ? "specific" : "all",
              // Separate organizations and stores from V3 response
              OrganizationSelections:
                v3Data.OrganizationSelections?.filter(
                  (sel: any) =>
                    sel.FootprintType === "SPECIFIC_ORG" ||
                    sel.FootprintType === "ALL_ORG"
                ).map(
                  (org: any) => org.OrganizationName || org.OrganizationId
                ) || [],
              StoreSelections:
                v3Data.OrganizationSelections?.filter(
                  (sel: any) => sel.FootprintType === "SPECIFIC_STORE"
                ).map(
                  (store: any) => store.OrganizationName || store.OrganizationId
                ) || [],
          ...
          console.log(
            "Successfully loaded V3 data with organizations:",
            v3Data.OrganizationSelections?.length || 0,
            "entries"
          );
          return;
        }
        ...
        console.log(
          "V3 API not available, falling back to legacy API:",
          v3Error
        );
      }
        ...
        const transformedData = {
          ...
          ReconstructedProductSelections: response.data.ItemPromotionMapViewList
            ? {
                ...
                SelectionType: response.data.ItemPromotionMapViewList.some(
                  (item: any) =>
                    item.SKUType !== "SKU" && item.SKUType !== "EXCLUDED_SKU"
                )
                  ? "hierarchy"
                  : "specific",

                // Count actual products (SKUs)
                TotalProducts: response.data.ItemPromotionMapViewList.filter(
                  (item: any) => item.SKUType === "SKU"
                ).length,
                NetProducts: response.data.ItemPromotionMapViewList.filter(
                  (item: any) => item.SKUType === "SKU"
                ).length,
                ExcludedProducts: response.data.ItemPromotionMapViewList.filter(
                  (item: any) =>
                    item.SKUType === "EXCLUDED_SKU" ||
                    item.SKUType?.includes("EXCLUDED")
                ).length,

                // Specific products list
                SpecificProducts: response.data.ItemPromotionMapViewList.filter(
                  (item: any) => item.SKUType === "SKU"
                ).map((item: any) => ({
                ...
                HierarchySelections:
                  response.data.ItemPromotionMapViewList.filter(
                    (item: any) =>
          ...
          ReconstructedFootprintData: {
            SelectionType:
              response.data.SchemeOrgs?.length > 0 ||
              response.data.SchemeBranches?.length > 0
                ? "specific"
                : "all",
            OrganizationSelections:
              response.data.SchemeOrgs?.map(
                (org: any) => org.OrgUID || org.UID
              ) || [],
            BranchSelections:
              response.data.SchemeBranches?.map(
                (branch: any) =>
                  branch.BranchCode || branch.BranchUID || branch.UID
              ) || [],
            CoverageByType: {
            ...
            TotalCoverage:
              (response.data.SchemeOrgs?.length || 0) +
                (response.data.SchemeBranches?.length || 0) || "All"
          }
        };
  ...
  const handleEdit = () => {
    router.push(`/productssales/promotions/manage?edit=${id}`);
  };
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this promotion?")) {
      try {
        ...
        if (response.success) {
          router.push("/productssales/promotions/manage");
        } else {
  ...
  const promotionData =
    promotion?.PromotionView || promotion?.promotionView || promotion;
          ...
          <div>
            <div className="flex items-center space-x-3">
              <h1 className="text-3xl font-bold text-gray-900">
                {promotionData?.Name || "Unnamed Promotion"}
              </h1>
              {getStatusBadge()}
            </div>
          ...
          <CardContent className="py-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-lg font-semibold">
                  {getPromotionDisplayType(promotionData)}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="p-6">
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <dd className="mt-1 text-3xl font-semibold text-gray-900">
                        {promotionData?.Type === "Invoice"
                          ? "INVOICE"
                          : promotionData?.Type === "Instant"
                          ? "ITEM_LEVEL"
                          : promotionData?.Level ||
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        {promotionData?.Type === "Invoice" ||
                        promotionData?.Level === "INVOICE_LEVEL" ||
                        promotionData?.OrderType === "INVOICE"
                    ...
                    <CardContent className="p-6">
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        {(() => {
                          ...
                          switch (format) {
                            case "IQFD":
                              return "Item Quantity Fixed Discount - Fixed discount amount per item";
                            case "IQPD":
                              return "Item Quantity Percentage Discount - Percentage discount on item price";
                            case "IQXF":
                              return "Item Quantity X Free - Buy X get Y free promotion";
                            case "BQXF":
                              return "Buy Quantity X Free - Buy quantity X get free items (FOC)";
                            case "BYVALUE":
                              return "By Value - Discount based on invoice value";
                            case "BYQTY":
                              return "By Quantity - Discount based on total quantity";
                            case "LINECOUNT":
                              return "Line Count - Discount based on number of line items";
                            case "BRANDCOUNT":
                              return "Brand Count - Discount based on number of brands";
                            case "ANYVALUE":
                              return "Any Value - Fixed discount on any invoice";
                            case "PERCENTAGE":
                              return "Percentage-based discount promotion";
                            case "FIXED":
                              return "Fixed amount discount promotion";
                            default:
                              return "Standard promotion format";
                          }
                        })()}
                      </p>
                    </CardContent>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <dd className="mt-1 flex items-center">
                        <Badge
                          variant={
                            promotionData?.IsActive ? "default" : "destructive"
                          }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {promotion?.PromotionView?.Code || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {promotion?.PromotionView?.Name || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.Description ||
                              promotion?.PromotionView?.Remarks ||
                              "N/A"}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.Category || "N/A"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="text-base text-gray-700">
                            {formatDate(promotion?.PromotionView?.ValidFrom)}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {formatDate(promotion?.PromotionView?.ValidUpto)}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.Priority || 1}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                    ...
                    <div className="mt-6 border-t border-gray-200 pt-6">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.PromoTitle || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.PromoMessage || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {promotion?.PromotionView?.CreatedByEmpUID || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {formatDate(promotion?.PromotionView?.CreatedTime)}
                          </p>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-500">
                          ...
                          <p className="text-base text-gray-700">
                            {formatDate(promotion?.PromotionView?.ModifiedTime)}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <TabsContent value="configuration" className="space-y-6">
                  ...
                  <CardContent>
                    {promotion.ReconstructedProductSelections ? (
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">
                          This may be a legacy promotion or the data is still
                          processing
                        </p>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {(promotionData?.PromoFormat === "IQXF" ||
                    promotionData?.PromoFormat === "BQXF") &&
                    promotion.PromoConditionViewList &&
                    promotion.PromoConditionViewList.length > 0 && (
                      <Card>
                        <CardHeader>
                          <CardTitle className="flex items-center">
                        ...
                        <CardContent>
                          <div className="space-y-3">
                            {promotion.PromoConditionViewList.map(
                              (condition, index) => (
                                <div
                                  key={condition.UID || index}
                                ...
                                >
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                      ...
                                      {condition.Min || "Not set"}
                                    </div>
                                    <div>
                                      <span className="text-gray-500">
                                        Get Free:
                                      </span>{" "}
                                      {(condition as any).FreeQuantity ||
                                        "Not set"}
                                    </div>
                                    {(condition as any).DiscountType && (
                                      <div>
                                        <span className="text-gray-500">
                                          Discount Type:
                                        </span>{" "}
                                    ...
                                    {(condition as any).DiscountPercentage > 0 && (
                                      <div>
                                        <span className="text-gray-500">
                                          Discount Percentage:
                                        </span>{" "}
                                        {(condition as any).DiscountPercentage}%
                                      </div>
                                    )}
                                    {(condition as any).DiscountAmount > 0 && (
                                      <div>
                                        <span className="text-gray-500">
                                          Discount Amount:
                                        </span>{" "}
                                        {(condition as any).DiscountAmount}
                                      </div>
                                    )}
                                    {promotion.PromoOrderViewList?.[0]
                                      ?.MaxDealCount && (
                                      <div className="col-span-2">
                                        <span className="text-gray-500">
                                          Max Applications per Product per
                                          Invoice:
                                        </span>{" "}
                                        {
                                          promotion.PromoOrderViewList[0]
                  ...
                  {/* Promotion Conditions - For other formats */}
                  {promotionData?.PromoFormat !== "IQXF" &&
                    promotionData?.PromoFormat !== "BQXF" &&
                    promotion.PromoConditionViewList &&
                    promotion.PromoConditionViewList.length > 0 && (
                          ...
                          <div className="space-y-3">
                            {promotion.PromoConditionViewList.map(
                              (condition, index) => (
                                <div
                                  key={condition.UID || index}
                                  className="p-3 border rounded-lg bg-indigo-50"
                                >
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    {(condition as any).Min && (
                                      <div>
                                        <span className="text-gray-500">
                                          Min Value:
                                        </span>{" "}
                                    ...
                                    {(condition as any).ConditionType && (
                                      <div>
                                        <span className="text-gray-500">
                                          Condition Type:
                                        </span>{" "}
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {promotion.PromoOrderViewList &&
                      promotion.PromoOrderViewList.length > 0 ? (
                        <div className="space-y-3">
                            ...
                            >
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                  ...
                                  {order.MinDealCount || 0}
                                </div>
                                <div>
                                  <span className="text-gray-500">
                                    Max Deal Count:
                                  </span>{" "}
                                  {order.MaxDealCount || 0}
                                </div>
                                <div>
                                  <span className="text-gray-500">
                                    Min Purchase Qty:
                                  </span>{" "}
                                  {order.MinPurchaseQty || 0}
                                </div>
                                <div>
                                  <span className="text-gray-500">
                                    Min Purchase Value:
                                  </span>{" "}
                                  {order.MinPurchaseValue || 0}
                                </div>
                                <div className="col-span-2">
                                  <span className="text-gray-500">
                                    Selection Model:
                                  </span>{" "}
                                  {order.SelectionModel || "any"}
                                </div>
                              </div>
                            </div>
                          ))}
                        ...
                        <p className="text-gray-500">
                          No order configuration defined
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {promotion.PromoOfferViewList &&
                      promotion.PromoOfferViewList.length > 0 ? (
                        <div className="space-y-3">
                            ...
                            >
                              <div className="text-sm space-y-2">
                                ...
                                {promotion.PromoOfferItemViewList &&
                                  promotion.PromoOfferItemViewList.filter(
                                    (item) => item.PromoOfferUID === offer.UID
                                  ).length > 0 && (
                                    <div className="mt-3 pt-3 border-t">
                                      ...
                                      {promotion.PromoOfferItemViewList.filter(
                                        (item) =>
                                          item.PromoOfferUID === offer.UID
                                      ).map((item, itemIndex) => (
                                        <div
                                          key={item.UID || itemIndex}
                                          className="ml-3 text-xs space-y-1 bg-gray-50 p-2 rounded mb-1"
                                        >
                                          <div>
                                            <span className="text-gray-500">
                                              Product:
                        ...
                        <p className="text-gray-500">
                          No offer configuration defined
                        </p>
                      )}
                    </CardContent>
                  </Card>
              ...
              <TabsContent value="footprint" className="space-y-6">
                <Card>
                  ...
                  <CardContent>
                    {promotion.ReconstructedFootprintData ||
                    !promotion.SchemeOrgs ||
                    promotion.SchemeOrgs.length === 0 ? (
                      <div className="space-y-6">
                          ...
                          promotion.ReconstructedFootprintData
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {promotion.ReconstructedFootprintData.OrganizationSelections.map(
                                  (org, index) => (
                                    <div
                                      key={index}
                        ...
                        {/* Store Groups */}
                        {promotion.ReconstructedFootprintData
                          ?.StoreGroupSelections &&
                          promotion.ReconstructedFootprintData
                            .StoreGroupSelections.length > 0 && (
                            <div>
                              <h4 className="text-md font-medium text-gray-900 mb-2">
                                Selected Store Groups
                              </h4>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {promotion.ReconstructedFootprintData.StoreGroupSelections.map(
                                  (group, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center bg-gray-50 p-3 rounded"
                        ...
                        {promotion.ReconstructedFootprintData
                          ...
                          promotion.ReconstructedFootprintData.BranchSelections
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {promotion.ReconstructedFootprintData.BranchSelections.map(
                                  (branch, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center bg-gray-50 p-3 rounded"
                          ...
                          promotion.ReconstructedFootprintData.StoreSelections
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {promotion.ReconstructedFootprintData.StoreSelections.map(
                                  (store, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center bg-gray-50 p-3 rounded"
                        ...
                        {promotion.ReconstructedFootprintData
                          ...
                          promotion.ReconstructedFootprintData
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {promotion.ReconstructedFootprintData.CustomerSelections.map(
                                  (customer, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center bg-gray-50 p-3 rounded"
                        ...
                        {promotion.ReconstructedFootprintData
                          ?.SalesmanSelections &&
                          promotion.ReconstructedFootprintData
                            .SalesmanSelections.length > 0 && (
                            <div>
                              <h4 className="text-md font-medium text-gray-900 mb-2">
                                Selected Salesmen
                              </h4>
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {promotion.ReconstructedFootprintData.SalesmanSelections.map(
                                  (salesman, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center bg-gray-50 p-3 rounded"
                            ...
                            <div className="flex items-center">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-green-700 mt-1">
                                  This promotion applies to all organizations,
                                  stores, and customers without restrictions.
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {(!promotion.ReconstructedFootprintData
                          ...
                          (!promotion.ReconstructedFootprintData
                            ?.BranchSelections ||
                            promotion.ReconstructedFootprintData
                              ?.BranchSelections.length === 0) &&
                          (promotion.SchemeBranches ||
                            promotion.SchemeOrgs) && (
                            <div>
                              <h4 className="text-md font-medium text-gray-900 mb-3">
                                Legacy Coverage Data
                              </h4>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-2">
                                        {promotion.SchemeOrgs.map((org) => (
                                          <div
                                            key={org.UID}
                                            className="flex items-center bg-gray-50 p-2 rounded text-sm"
                                          >
                                ...
                                {/* Legacy Branches */}
                                {promotion.SchemeBranches &&
                                  promotion.SchemeBranches.length > 0 && (
                                    <div>
                                      <h5 className="text-sm font-medium text-gray-700 mb-2">
                                        Branches
                                      </h5>
                                      <div className="space-y-2">
                                        {promotion.SchemeBranches.map(
                                          (branch) => (
                                            <div
                                              key={branch.UID}
                                              className="flex items-center bg-gray-50 p-2 rounded text-sm"
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">
                          This promotion may apply to all locations by default
                        </p>
                      </div>
                    )}
                  </CardContent>
              ...
              <TabsContent value="volumecaps" className="space-y-6">
                {promotion.PromotionVolumeCap ? (
                  <div className="space-y-6">
                    {/* Overall Cap */}
                    {(promotion.PromotionVolumeCap.OverallCapValue || 0) >
                      0 && (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-semibold text-blue-900">
                                {promotion.PromotionVolumeCap.OverallCapType ||
                                  "VALUE"}
                              </p>
                            </div>
                            <div>
                    ...
                    {((promotion.PromotionVolumeCap.InvoiceMaxDiscountValue ||
                      0) > 0 ||
                      (promotion.PromotionVolumeCap.InvoiceMaxQuantity || 0) >
                        0 ||
                            ...
                            {promotion.PromotionVolumeCap.TotalCapEnabled && (
                              <div className="bg-white p-3 rounded border border-green-300">
                                ...
                                <p className="text-lg font-bold text-green-900">
                                  {promotion.PromotionVolumeCap.TotalCapValue}{" "}
                                  {promotion.PromotionVolumeCap.TotalCapUnit ||
                                    "VALUE"}
                                </p>
                              </div>
                            )}
                  ...
                  <Card>
                    <CardContent className="py-12 text-center">
                      ...
                      <p className="text-gray-500">
                        This promotion does not have any volume caps configured.
                      </p>
                    </CardContent>
                  </Card>
                )}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-base">
                          {promotionData?.Level || "N/A"} -{" "}
                          {promotionData?.PromoFormat || "N/A"}
                        </p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-500">
                        ...
                        <p className="text-base">
                          {formatDate(promotionData?.ValidFrom)} to{" "}
                          {formatDate(promotionData?.ValidUpto)}
                        </p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-500">
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <p className="text-base">
                          {promotion.ReconstructedProductSelections
                            ?.TotalProducts ||
                            promotion.ItemPromotionMapViewList?.length ||
                            0}{" "}
                          products selected
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-base">
                          {promotion.PromoOrderViewList?.length || 0} order
                          configurations
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-base">
                          {promotion.PromoOfferViewList?.length || 0} offer
                          configurations
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-base">
                          {promotion.SchemeBranches?.length
                            ? `${promotion.SchemeBranches.length} specific branches`
                            : "All branches"}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-base">
                          {promotion.SchemeOrgs?.length
                            ? `${promotion.SchemeOrgs.length} specific organizations`
                            : "All organizations"}
                        </p>
                      </div>
                    </CardContent>
                  ...
                  <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-base">
                          {formatDate(promotionData?.CreatedTime)}
                        </p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-500">
                        ...
                        <p className="text-base">
                          {promotionData?.CreatedByEmpUID || "N/A"}
                        </p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-base">
                          {formatDate(promotionData?.ModifiedTime)}
                        </p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-500">
                        ...
                        <p className="text-base">
                          {formatDate(promotionData?.ServerModifiedTime)}
                        </p>
                      </div>
                    </div>
                  </CardContent>
