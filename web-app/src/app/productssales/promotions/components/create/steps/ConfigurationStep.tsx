...
import DynamicProductAttributes, {
  ProductAttribute
} from "../../product-selection/DynamicProductAttributes";
...
export default function ConfigurationStep({
  ...
  const [focProducts, setFocProducts] = useState<any[]>(
    formData.focProducts || []
  );
                ...
                >
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const newSlabs = [...(formData.slabs || [])];
                          if (isPercentage) {
                            newSlabs[index].discountPercent = e.target.value
                              ? parseFloat(e.target.value)
                              : null;
                          } else {
                            newSlabs[index].discountAmount = e.target.value
                              ? parseFloat(e.target.value)
                              : null;
                          }
                          onFormDataChange("slabs", newSlabs);
                  ...
                  {hasErrors && (
                    <div className="mt-2 text-xs text-red-500">
                      {!slab.minQty && (
                        <div>• Minimum quantity is required</div>
                      )}
                      {!slab.maxQty && (
                        <div>• Maximum quantity is required</div>
                      )}
                      {slab.minQty &&
                        slab.maxQty &&
        ...
        <>
          <Alert className="border-blue-200 mb-4">
            ...
            <AlertDescription className="text-blue-700">
              <strong>Buy X Get Y Configuration:</strong>
              <br />
              • The "Buy X" quantity is set in the product selection above (Min.
              Purchase Qty)
              <br />• Configure the "Get Y Free" quantity below
            </AlertDescription>
          </Alert>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              How many times this offer can be applied per product in a single
              invoice
            </p>
          </div>
          ...
          <Alert className="border-green-200">
            ...
            <AlertDescription className="text-green-700">
              <strong>Single Product IQXF:</strong>
              <br />
              • Customer buys minimum quantity set in product selection
              <br />• Gets {formData.freeQuantity ||
                formData.getQty ||
                "Y"}{" "}
              units of the same product FREE
              <br />• Can be applied {formData.maxDealCount || "unlimited"}{" "}
              times per product in a single invoice
            </AlertDescription>
          </Alert>
        </>
      )}
      ...
      {selectedFormat === "BQXF" && (
        <>
          <Alert className="border-blue-200">
            <Info className="h-4 w-4 text-blue-600" />
            <AlertDescription className="text-blue-700">
              <strong>Buy Quantity X Free (FOC) Configuration:</strong>
              <br />
              • Step 1: Select products to buy from the product selection above
              <br />
              • Step 2: Set buy quantities (X) for each selected product
              <br />• Step 3: Select free products (Y) below with their
              quantities
            </AlertDescription>
          </Alert>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-500">
              Select products that customers will receive free when they meet
              the buy requirements.
            </p>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label
                htmlFor="maxApplicationsPerInvoice"
                className="text-sm font-medium text-gray-700 mb-1.5 block"
              >
                Maximum Applications Per Invoice
              </Label>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                How many times this promotion can be applied within a single
                invoice
              </p>
            </div>
            ...
            <div>
              <Label
                htmlFor="maxDealCount2"
                className="text-sm font-medium text-gray-700 mb-1.5 block"
              >
                Maximum Total Invoices
              </Label>
              <Input
                id="maxDealCount2"
                type="number"
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Total number of invoices that can use this promotion (overall
                limit)
              </p>
            </div>
          </div>
    ...
    return (
      <div className="space-y-4">
        ...
        <Alert className="border-blue-200">
          <Info className="h-4 w-4 text-blue-600" />
          <AlertDescription className="text-blue-700">
            <strong>Invoice Level Promotion:</strong>
            <br />
            This promotion will apply to the entire invoice when the configured
            threshold is met.
            {selectedFormat === "ANYVALUE" &&
              " No minimum threshold required - applies to all invoices."}
          </AlertDescription>
        </Alert>
        {selectedFormat !== "ANYVALUE" && (
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {selectedFormat === "BYVALUE" &&
                "Invoice total must be at least this amount"}
              {selectedFormat === "BYQTY" &&
                "Total quantity of all items must be at least this amount"}
              {selectedFormat === "LINECOUNT" &&
                "Invoice must have at least this many line items"}
              {selectedFormat === "BRANDCOUNT" &&
                "Invoice must include products from at least this many brands"}
            </p>
          </div>
        )}
        ...
        {formData.offerType === "foc" && (
          <div className="space-y-4">
            <div className="border border-gray-200 rounded-lg p-5 space-y-4">
              ...
              <p className="text-xs text-gray-500">
                Select products that customers will receive free when the
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            How many times this promotion can be applied in a single invoice
            (usually 1 for invoice level)
          </p>
        </div>
      </div>
    );
        ...
        <>
          ...
          {selectedFormat === "IQXF" && (
            <Alert className="mb-4 border-amber-200">
              ...
              <AlertDescription className="text-amber-700">
                <strong>Important: IQXF Single Product Requirement</strong>
                <br />
                For Item Quantity X Free promotions, you must select exactly ONE
                product. The customer will buy X units of this product and get Y
                units free of the same product.
              </AlertDescription>
            </Alert>
          )}
          ...
          <DynamicProductAttributes
            ...
            onFinalProductsChange={(products) => {
              ...
              if (
                formData.productSelectionMode === "specific" ||
                selectedFormat === "IQXF" ||
                selectedFormat === "BQXF"
              ) {
          ...
          {selectedFormat === "IQXF" && (
            <div className="border border-blue-200 bg-blue-50 rounded-lg p-4 mt-4">
              <p className="text-sm text-blue-800">
                <strong>Note:</strong> Configure the buy quantity directly in
                the product selection above. The free quantity will be
      ...
      {selectedLevel === "instant" && selectedFormat === "MPROD" && (
        <div className="border border-gray-200 rounded-lg p-5">
          <MultiProductConfiguration
            ...
            onChange={(promotions) =>
              onFormDataChange("productPromotions", promotions)
            }
            orgUid={orgUid || ""}
