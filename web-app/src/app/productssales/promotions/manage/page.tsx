...
export default function PromotionManagePage() {
  ...
  const handleViewDetails = (promotionId: string) => {
    ...
    router.push(`/productssales/promotions/${promotionId}`);
  };
  ...
  const handleEdit = (promotionId: string) => {
    ...
    router.push(`/productssales/promotions/manage?edit=${promotionId}`);
  };
  ...
  const handleToggleStatus = async (
    ...
    try {
      ...
      if (response.IsSuccess) {
        ...
        throw new Error(
          response.ErrorMessage || "Failed to update promotion status"
        );
      }
    } catch (err: any) {
  ...
  const handleCreateNew = () => {
    router.push("/productssales/promotions/create");
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${selectedIds.size} promotions?`
      )
    ) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(
        new Set(paginatedPromotions.map((p) => p.UID || p.uid || p.code))
      );
    } else {
      setSelectedIds(new Set());
    }
  ...
  const getStatusBadge = (promotion: IPromotionView) => {
    ...
    return (
      <div className="flex items-center gap-2">
        <Badge variant={variants[status.color]}>{status.label}</Badge>
        <div
        ...
        <div className="flex justify-between items-center">
            ...
            <div className="flex items-center mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Manage all your promotional campaigns and marketing
                  initiatives
                </p>
              </div>
            </div>
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={async () => {
                try {
                  const blob = await promotionService.exportPromotions("csv", {
                  ...
                  toast.error("Failed to export promotions. Please try again.");
                }
              }}
            >
                ...
                <TableHead className="w-12 pl-6">
                  <Checkbox
                    checked={
                      ...
                      selectedIds.size === paginatedPromotions.length
                    }
              ...
              {paginatedPromotions.map((promotion, index) => (
                ...
                >
                  <TableCell className="pl-6">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleSelectItem(promotion.UID, checked as boolean)
                      }
                      className="border-gray-300"
                    />
                  </TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <DropdownMenu
                      key={`${promotion.UID}-${promotion.IsActive}-${promotion.isActive}`}
                    >
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem
                          onClick={() =>
                            handleToggleStatus(
                              promotion.UID,
                              Boolean(promotion.IsActive || promotion.isActive)
                            )
                          }
                          className="cursor-pointer"
