...
export class DirectProductQueryService {
  ...
  /**
   * Get products with pagination for lazy loading
   */
  async getProductsPaginated(page: number = 1, pageSize: number = 10): Promise<{
    ...
    try {
      ...
      const totalCount = actualResponse.TotalCount || actualResponse.TotalRecords || 0;
      
      // Convert to FinalProduct format
      const products: FinalProduct[] = pageData.map((sku: any) => ({
