...
class PromotionV3Service {
  ...
  private buildPromoOrderItemList(formData: any, promotionUID: string, promoOrderUID: string, employeeUID: string): any[] {
    ...
    if (format === 'IQFD' || format === 'IQPD') {
      ...
      // Check if hierarchy data exists (categories, brands, subcategories)
      if (formData.productAttributes && formData.productAttributes.length > 0) {
        actualSelectionMode = 'hierarchy';
      }
      // Check if specific products are selected directly
      else if (formData.specificProducts && formData.specificProducts.length > 0) {
        actualSelectionMode = 'specific';
      }
      // Check if final attribute products exist WITHOUT hierarchy attributes (specific product selection)
      else if (formData.finalAttributeProducts && formData.finalAttributeProducts.length > 0) {
        // Only treat as hierarchy if there are actual productAttributes, otherwise it's specific selection
        if (formData.productAttributes && formData.productAttributes.length > 0) {
          actualSelectionMode = 'hierarchy'; // Products selected within hierarchy
        } else {
          actualSelectionMode = 'specific'; // Direct product selection stored in finalAttributeProducts
        }
      }
      
      console.log('[DEBUG] Selection mode determination:');
      console.log('  - productAttributes count:', formData.productAttributes?.length || 0);
      console.log('  - specificProducts count:', formData.specificProducts?.length || 0);
      console.log('  - finalAttributeProducts count:', formData.finalAttributeProducts?.length || 0);
      console.log('  - Determined actualSelectionMode:', actualSelectionMode);
  ...
  private extractCompanyUID(formData: any): string {
    const companyUID = formData.companyUID || 
           ...
           this.extractPrimaryOrgUID(formData);  // Use organization ID instead of hardcoded WINIT
