...
export function CreateLocation() {
  ...
  const fetchInitialData = async () => {
    setLoading(true)
    try {
      // Fetch location types and filter by ShowInUI
      const types = await locationService.getLocationTypes()
      const filteredTypes = types.filter(type => type.ShowInUI === true)
      setLocationTypes(filteredTypes.sort((a, b) => a.LevelNo - b.LevelNo))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      // Clean up the form data before submission and add audit fields
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push("/administration/location-management/locations")}
        >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleInputChange("Code", upperCaseCode)
                    if (!isEditMode) {
