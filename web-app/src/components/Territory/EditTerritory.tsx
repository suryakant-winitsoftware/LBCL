...
export function EditTerritory({ territoryId }: EditTerritoryProps) {
  ...
  useEffect(() => {
    if (formData.ItemLevel && formData.ItemLevel > 1 && territories.length > 0) {
      ...
      const parents = territories.filter(ter => ter.ItemLevel === parentLevel && ter.UID !== territoryId)
      setParentTerritories(parents)
    } else {
      setParentTerritories([])
    }
  }, [formData.ItemLevel, territories, territoryId])
  ...
  const fetchTerritoryDetails = async () => {
    setLoading(true)
    try {
      const territory = await territoryService.getTerritoryByUID(territoryId)
      setFormData(territory)
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to fetch territory details",
        variant: "destructive"
      })
    } finally {
      setLoading(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await territoryService.updateTerritory(submitData)
      toast({
        title: "Success",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Edit Territory</h1>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the territory information below
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              {/* Territory Code - Read Only */}
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Territory code cannot be changed
                </p>
              </div>
