...
export function PermissionMatrix({ matrix, loading = false, onChange, onSave, onDiscard, hasUnsavedChanges = false }: PermissionMatrixProps) {
  ...
  // Check if this is mobile platform
  const isMobilePlatform = matrix?.platform === "Mobile"
      ...
      modules: matrix.modules.map(module => ({
        ...
        subModules: module.subModules.map(subModule => ({
          ...
          pages: subModule.pages.filter(page => {
            ...
            // Permission filter - only check actual permission fields
            let matchesPermission = true
            if (permissionFilter === 'granted') {
              matchesPermission = page.permissions.fullAccess || 
                                  page.permissions.viewAccess || 
                                  page.permissions.addAccess || 
                                  page.permissions.editAccess || 
                                  page.permissions.deleteAccess || 
                                  page.permissions.downloadAccess || 
                                  page.permissions.approvalAccess
            } else if (permissionFilter === 'denied') {
              matchesPermission = !page.permissions.fullAccess && 
                                  !page.permissions.viewAccess && 
                                  !page.permissions.addAccess && 
                                  !page.permissions.editAccess && 
  ...
  // Calculate column header checkbox states
  const getColumnCheckState = useCallback((permissionType: PermissionType) => {
    if (!filteredMatrix) return { checked: false, indeterminate: false }

    const allPages = filteredMatrix.modules.flatMap(module =>
      module.subModules.flatMap(subModule =>
        subModule.pages
      )
    )

    if (allPages.length === 0) return { checked: false, indeterminate: false }

    const checkedCount = allPages.filter(page => 
      !!page.permissions[permissionType]
    ).length

    if (checkedCount === 0) {
      return { checked: false, indeterminate: false }
    } else if (checkedCount === allPages.length) {
  ...
  // Calculate module checkbox states
  const getModuleCheckState = useCallback((moduleUid: string, permissionType: PermissionType) => {
    if (!matrix) return { checked: false, indeterminate: false }

    const moduleData = matrix.modules.find(m => m.module.UID === moduleUid)
    if (!moduleData) return { checked: false, indeterminate: false }

    const allPages = moduleData.subModules.flatMap(subModule => subModule.pages)
    if (allPages.length === 0) return { checked: false, indeterminate: false }

    const checkedCount = allPages.filter(page => !!page.permissions[permissionType]).length
  ...
  // Calculate submodule checkbox states
  const getSubModuleCheckState = useCallback((moduleUid: string, subModuleUid: string, permissionType: PermissionType) => {
    if (!matrix) return { checked: false, indeterminate: false }

    const moduleData = matrix.modules.find(m => m.module.UID === moduleUid)
    if (!moduleData) return { checked: false, indeterminate: false }

    const subModuleData = moduleData.subModules.find(sm => sm.subModule.UID === subModuleUid)
    if (!subModuleData) return { checked: false, indeterminate: false }

    const allPages = subModuleData.pages
    if (allPages.length === 0) return { checked: false, indeterminate: false }
    ...
    matrix.modules.forEach(module => {
      module.subModules.forEach(subModule => {
        subModule.pages.forEach(page => {
          total++
          if (page.permissions.fullAccess || 
              page.permissions.viewAccess || 
              page.permissions.addAccess || 
              page.permissions.editAccess || 
              page.permissions.deleteAccess || 
          ...
          subModules: module.subModules.map(subModule => {
            ...
            return {
              ...
              pages: subModule.pages.map(page => {
                ...
                // Mobile platform uses simple YES/NO logic like Blazor app
                if (isMobilePlatform) {
                  if (permissionType === 'fullAccess') {
                    // For mobile, it's simple - either full access or no access
                    newPermissions.fullAccess = value
                    newPermissions.viewAccess = value
                    newPermissions.addAccess = value
                    newPermissions.editAccess = value
                    newPermissions.deleteAccess = value
                    newPermissions.downloadAccess = value
                    newPermissions.approvalAccess = value
                    newPermissions.showInMenu = value
                  } else {
                    // For mobile, only allow toggling fullAccess (ignore other permission types)
                    return page
                  }
                } else {
                  // Web platform uses complex permission logic
  ...
  }, [matrix, onChange, isMobilePlatform])

  const handleGrantAllAccess = useCallback((grantAccess: boolean) => {
    if (!matrix) return
      ...
      modules: matrix.modules.map(module => ({
        ...
        subModules: module.subModules.map(subModule => ({
          ...
          pages: subModule.pages.map(page => {
            ...
            if (isMobilePlatform) {
              // For mobile, set all permission fields based on grantAccess
              newPermissions.fullAccess = grantAccess
              newPermissions.viewAccess = grantAccess
              newPermissions.addAccess = grantAccess
              newPermissions.editAccess = grantAccess
              newPermissions.deleteAccess = grantAccess
              newPermissions.downloadAccess = grantAccess
              newPermissions.approvalAccess = grantAccess
              newPermissions.showInMenu = grantAccess
            } else {
              // For web, handle individual permissions or full access
              if (grantAccess) {
                // Grant all permissions
                newPermissions.fullAccess = true
                newPermissions.viewAccess = true
                newPermissions.addAccess = true
                newPermissions.editAccess = true
                newPermissions.deleteAccess = true
                newPermissions.downloadAccess = true
                newPermissions.approvalAccess = true
                newPermissions.showInMenu = true
              } else {
                // Revoke all permissions
                newPermissions.fullAccess = false
                newPermissions.viewAccess = false
                newPermissions.addAccess = false
                newPermissions.editAccess = false
                newPermissions.deleteAccess = false
                newPermissions.downloadAccess = false
                newPermissions.approvalAccess = false
                newPermissions.showInMenu = false
              }
            }
  ...
  const handleColumnSelectAll = useCallback((permissionType: PermissionType, grantAccess: boolean) => {
    if (!matrix) return
      ...
      modules: matrix.modules.map(module => ({
        ...
        subModules: module.subModules.map(subModule => ({
          ...
          pages: subModule.pages.map(page => {
            ...
            if (isMobilePlatform) {
              // For mobile, only fullAccess matters
              if (permissionType === 'fullAccess') {
                newPermissions.fullAccess = grantAccess
                newPermissions.viewAccess = grantAccess
                newPermissions.addAccess = grantAccess
              ...
              // For web, set the specific permission type
              newPermissions[permissionType] = grantAccess
              
              // Handle dependencies for web permissions
              if (permissionType === 'fullAccess') {
                if (grantAccess) {
                  // Full access grants all permissions
                  newPermissions.viewAccess = true
                ...
                if (grantAccess && permissionType !== 'viewAccess') {
                  // Granting any permission also grants view access
                  newPermissions.viewAccess = true
                } else if (!grantAccess && permissionType === 'viewAccess') {
                  // Removing view access removes all other permissions
                  newPermissions.addAccess = false
                  newPermissions.editAccess = false
                  newPermissions.deleteAccess = false
                  newPermissions.downloadAccess = false
    ...
    onChange(updatedMatrix)
  }, [matrix, onChange, isMobilePlatform, permissionColumns])

  const handleModuleSelectAll = useCallback((moduleUid: string, permissionType: PermissionType, grantAccess: boolean) => {
    if (!matrix) return
        ...
        return {
          ...
          subModules: module.subModules.map(subModule => ({
            ...
            pages: subModule.pages.map(page => {
              ...
              // Preserve backend fields
              if (page.permissions.roleUID) newPermissions.roleUID = page.permissions.roleUID
              if (page.permissions.subSubModuleUID) newPermissions.subSubModuleUID = page.permissions.subSubModuleUID
              if (page.permissions.platform) newPermissions.platform = page.permissions.platform
              if (page.permissions.uid) newPermissions.uid = page.permissions.uid
              if (page.permissions.id) newPermissions.id = page.permissions.id
              if (page.permissions.createdBy) newPermissions.createdBy = page.permissions.createdBy
              if (page.permissions.createdTime) newPermissions.createdTime = page.permissions.createdTime
  ...
  const handleSubModuleSelectAll = useCallback((moduleUid: string, subModuleUid: string, permissionType: PermissionType, grantAccess: boolean) => {
    if (!matrix) return
  ...
  // For mobile platform, show only FullAccess column (like Blazor app)
  const columnsToShow = isMobilePlatform 
    ? permissionColumns.filter(col => col.key === 'fullAccess')
    : permissionColumns
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-medium text-blue-900">Mobile Permission Management</h3>
                <p className="text-xs text-blue-700 mt-1">
                  Mobile permissions use a simple YES/NO access model. Full Access grants all permissions for mobile activities.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleGrantAllAccess(false)}
              ...
              <Button
                size="sm"
                onClick={() => handleGrantAllAccess(true)}
            ...
            <table className="w-full">
              <thead className="bg-gray-100 border-b">
                <tr>
                  ...
                  {columnsToShow.map(column => {
                    const Icon = column.icon
                    const columnState = getColumnCheckState(column.key)
                    return (
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex flex-col items-center gap-1 cursor-help">
                                ...
                                <Checkbox
                                  ...
                                  ref={(el) => {
                                    if (el) {
                                      el.indeterminate = columnState.indeterminate
                                    }
                                  }}
                                  onCheckedChange={(checked) => {
                                    handleColumnSelectAll(column.key, checked === true)
                                  }}
                            ...
                            <TooltipContent>
                              ...
                              <p className="text-xs text-gray-400 mt-1">Click checkbox to select/deselect all</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </th>
                    ...
                    <tr className="bg-gray-50 border-b hover:bg-gray-100 transition-colors">
                      <td className="p-3">
                        <Button
                          ...
                          className="flex items-center gap-2 font-medium text-gray-800 hover:text-gray-900 justify-start"
                        >
                      ...
                      {columnsToShow.map(column => {
                        const moduleState = getModuleCheckState(module.module.UID, column.key)
                        return (
                          <td key={column.key} className="text-center p-3">
                            <Checkbox
                              ...
                              ref={(el) => {
                                if (el) {
                                  el.indeterminate = moduleState.indeterminate
                                }
                              }}
                              onCheckedChange={(checked) => {
                                handleModuleSelectAll(module.module.UID, column.key, checked === true)
                              }}
                    ...
                    {expandedModules.has(module.module.UID) && module.subModules.map(subModule => (
                      <React.Fragment key={subModule.subModule.UID}>
                        ...
                        <tr className="bg-gray-25 border-b">
                          ...
                          {columnsToShow.map(column => {
                            const subModuleState = getSubModuleCheckState(module.module.UID, subModule.subModule.UID, column.key)
                            return (
                              <td key={column.key} className="text-center p-3">
                                <Checkbox
                                  ...
                                  ref={(el) => {
                                    if (el) {
                                      el.indeterminate = subModuleState.indeterminate
                                    }
                                  }}
                                  onCheckedChange={(checked) => {
                                    handleSubModuleSelectAll(module.module.UID, subModule.subModule.UID, column.key, checked === true)
                                  }}
                                  className="border-gray-400"
                                />
                              </td>
                        ...
                        {expandedSubModules.has(subModule.subModule.UID) && subModule.pages.map(page => (
                          <tr key={page.page.UID} className="border-b hover:bg-gray-50 transition-colors">
                            ...
                            {columnsToShow.map(column => (
                              <td key={column.key} className="text-center p-3">
                                <Checkbox
                                  checked={!!page.permissions[column.key]}
                                  onCheckedChange={(checked) => 
