...
export function ProfessionalSidebar({ className }: ProfessionalSidebarProps) {
          ...
          moduleChildren.forEach((subModule) => {
            ...
            if (subModuleChildren) {
              const hasActivePage = subModuleChildren.some((page) => {
                ...
                return pathname === hierarchicalPath ||
  ...
  // Get searchable items from menu hierarchy
        ...
        moduleChildren.forEach(subModule => {
          ...
          if (!subModuleChildren || subModuleChildren.length === 0) {
            ...
            subModuleChildren.forEach(page => {
              const hierarchicalPath = buildHierarchicalPath(menuModule, subModule, page)
              items.push({
                type: 'page',
                name: `${subModule.submoduleNameEn} > ${page.subSubModuleNameEn}`,
                path: page.relativePath ? `/${page.relativePath}` : hierarchicalPath,
                icon: getSubModuleIcon(subModule.submoduleNameEn, menuModule.moduleNameEn)
              })
            })
          }
                            ...
                            <CollapsibleContent className="mt-1">
                              ...
                              {moduleChildren.map((subModule) => {
                                ...
                                if (!subModuleChildren || subModuleChildren.length === 0) {
                                  ...
                                  const isActive = pathname === `/${subModulePath}` || pathname.startsWith(`/${subModulePath}/`)
                                  return (
                                      ...
                                      <CollapsibleContent className="mt-1">
                                        <SidebarMenuSub className="ml-3 pl-3 relative space-y-0.5 before:absolute before:left-0 before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-200/30 dark:before:bg-gray-700/30">
                                          {subModuleChildren.map((page) => {
                                            ...
                                            const isActive = pathname === hierarchicalPath ||
                                                            pathname.startsWith(hierarchicalPath + '/')
      ...
      <CommandList className="max-h-96 overflow-y-auto">
        <CommandEmpty className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs text-gray-400">Try searching for different keywords</p>
          </div>
        </CommandEmpty>
          ...
          <CommandGroup className="p-2">
            {filteredItems.map((item, index) => (
              <CommandItem
                ...
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
