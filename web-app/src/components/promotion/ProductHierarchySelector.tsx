...
export default function ProductHierarchySelector({
  ...
  const loadSKUProducts = async () => {
    ...
    try {
      ...
      // Load all SKUs - backend now handles database queries directly for better performance
      const requestBody = {
        PageNumber: 1,
        PageSize: 50000, // Request large page size to get all products
        IsCountRequired: true,
        FilterCriterias: [], // Backend will now query database directly when no OrgUID filter
        SortCriterias: [
          {
            SortParameter: "Code",
            Direction: "Asc",
      ...
      // Use SelectAllSKUDetails which now returns products with FilterKeys populated
      const response = await fetch(`${baseURL}/SKU/SelectAllSKUDetails`, {
        method: "POST",
        headers: {
          ...getAuthHeaders(),
      ...
      if (result.IsSuccess && result.Data) {
        ...
        // Handle PagedData structure
        if (data.PagedData && Array.isArray(data.PagedData)) {
          // Log first item to see structure (SelectAllSKUDetails API returns FilterKeys)
          if (data.PagedData.length > 0) {
            console.log("SKU structure:", data.PagedData[0]);
            console.log("FilterKeys available:", data.PagedData[0].FilterKeys);
          }

          // SelectAllSKUDetails returns FilterKeys array for each SKU
          // FilterKeys format: ["D001_Divi", "S001_SubDivi", "C001_Category", "S925_Sub Category", "B283_Brand Name", "EPIC01_ORG", "Supplier_Division"]
          const filteredData = data.PagedData.filter((sku: SKUProduct) => {
            // If no filters selected, return all
            if (filterKeys.length === 0) return true;

            // Check if SKU has FilterKeys array
            if (!sku.FilterKeys || !Array.isArray(sku.FilterKeys)) {
              console.warn(`SKU ${sku.Code} has no FilterKeys`);
              return false;
            }
            ...
            if (matches) {
              console.log(
                `SKU ${sku.Code} matches with FilterKeys:`,
                sku.FilterKeys
              );
            }
          ...
          console.log(
            `Filtered ${data.PagedData.length} SKUs to ${filteredData.length} matching selection`
          );

          // Map SelectAllSKUDetails fields to our expected format
          skuData = filteredData.map((sku: SKUProduct) => ({
            Id: sku.Id,
            UID: sku.UID,
            Code: sku.Code,
            Name: sku.Name,
  ...
  }, [selectedItems, enableSKULoading]);
  ...
  const loadChildOptions = async (
    ...
    console.log(
      `Loading child options for level ${childLevel}, parents:`,
      Array.from(parentCodes)
    );
    ...
    try {
      ...
      for (const parentCode of parentCodes) {
        ...
        if (optionsCache[cacheKey]) {
          console.log(`Using cached options for parent ${parentCode}`);
          allChildOptions.push(...optionsCache[cacheKey]);
        } else {
          console.log(`Fetching children for parent ${parentCode}`);
          const childGroups = await hierarchyService.getChildSKUGroups(
            parentCode
          );
          console.log(
            `Got ${childGroups.length} children for parent ${parentCode}`
          );
      ...
      console.log(
        `Setting ${uniqueOptions.length} unique options for ${childType.Name}_combined`
      );
  ...
  const handleItemToggle = async (
    ...
    if (checked) {
      ...
      // Clear child selections and options
      for (
        ...
        // Clear the child options when no parent is selected
        if (childLevel - 1 < hierarchyTypes.length) {
          const childType = hierarchyTypes[childLevel - 1];
          if (childType) {
            setHierarchyOptions((prev) => ({
              ...prev,
              [`${childType.Name}_combined`]: [],
  ...
  const shouldShowLevel = (levelIndex: number): boolean => {
    ...
    // Show next level if previous level has selections
    const previousLevel = levelIndex; // This refers to the actual level number (1-based)
    return (
      levelSelections[previousLevel] && levelSelections[previousLevel].size > 0
    );
  };
        ...
        >
          <PopoverTrigger asChild>
            <Button
              ...
              disabled={
                disabled || (!isFirstLevel && !shouldShowLevel(levelIndex - 1))
              }
            >
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select product hierarchy for promotion configuration
        </CardDescription>
      </CardHeader>
      <CardContent>
        {/* Hierarchy Dropdowns */}
