        ...
        <Table className={className}>
          <TableHeader>
            <TableRow className="border-b bg-gray-50/50">
              {columns.map((column, index) => (
                <TableHead key={column.accessorKey || column.id || index} className="py-3">
                  {renderHeader(column)}
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, rowIndex) => (
                <TableRow 
                  key={rowIndex}
                  className={onRowClick ? 'cursor-pointer hover:bg-gray-50/50 transition-colors' : 'hover:bg-gray-50/50 transition-colors'}
                  onClick={() => onRowClick?.(row)}
                >
                  {columns.map((column, colIndex) => (
                    <TableCell key={column.accessorKey || column.id || colIndex} className="py-2">
