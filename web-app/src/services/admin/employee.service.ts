...
class EmployeeService {
  ...
  async resetPassword(empUID: string, newPassword: string): Promise<boolean> {
    try {
      ...
      const response = await this.apiCall<string>("/Auth/UpdateNewPassword", {
        method: "POST",
        body: JSON.stringify({
          EmpUID: empUID,
  ...
  async getReportsToEmployeesByRoleUID(roleUID: string): Promise<any[]> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/Emp/GetReportsToEmployeesByRoleUID?roleUID=${roleUID}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authService.getToken()}`
  ...
  async getEmployeesSelectionItemByRoleUID(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/Emp/GetEmployeesSelectionItemByRoleUID?orgUID=${orgUID}&roleUID=${roleUID}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authService.getToken()}`
  ...
  async updateJobPositionLocation(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/JobPosition/UpdateJobPositionLocationTypeAndValue`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async exportEmployees(
    ...
    const response = await this.getEmployees(pagingRequest);
    const employees = response.pagedData || [];

    // Enhance employee data with role information (same as employee grid)
    const enhancedEmployees = await Promise.all(
      employees.map(async (emp: any) => {
        try {
          // Get detailed employee data that includes JobPosition
          const detailedEmp = await this.getEmployeeById(emp.UID);
          let roleName = null;
          ...
          if (detailedEmp?.JobPosition?.UserRoleUID) {
            try {
              ...
              const roleInfo = await roleService.getRoleById(
                detailedEmp.JobPosition.UserRoleUID
              );
              roleName =
                roleInfo?.Name || roleInfo?.RoleNameEn || roleInfo?.Code;
            } catch (roleError) {
              console.warn(
                `Failed to load role for employee ${emp.UID}:`,
                roleError
              );
            }
          }
    ...
    if (format === "csv") {
      return this.exportToCSV(enhancedEmployees);
    } else {
      return this.exportToExcel(enhancedEmployees);
    }
  }

  private exportToCSV(employees: any[]): Blob {
    const headers = ["Code", "Name", "Login ID", "Role", "Status", "Email"];
