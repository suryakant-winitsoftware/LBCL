...
class PermissionService {
  ...
  async updatePermissionMatrix(matrix: PermissionMatrix): Promise<boolean> {
    ...
    // Handle mobile permissions differently from web permissions
    if (matrix.platform === "Mobile") {
      return await this.updateMobilePermissions(matrix, role.IsPrincipalRole);
    }

    // Extract all permissions from the matrix for Web
  ...
  // Mobile Permission Handling - Special logic like Blazor app
  private async updateMobilePermissions(
    matrix: PermissionMatrix,
    isPrincipalRole: boolean
  ): Promise<boolean> {
    const permissions: Permission[] = [];

    // For mobile, include ALL permissions (backend needs complete data)
    // Backend requires all permissions to generate proper mobile menu
    matrix.modules.forEach((moduleGroup) => {
      moduleGroup.subModules.forEach((subModuleGroup) => {
        subModuleGroup.pages.forEach((pageGroup) => {
          const permission = pageGroup.permissions;

          // Include ALL permissions for mobile
          // For mobile, simplify to FullAccess only (like Blazor app)
          const mobilePermission: Permission = {
            roleUID: permission.roleUID,
            subSubModuleUID: permission.subSubModuleUID,
            platform: "Mobile",
            // Mobile permissions are simple - only FullAccess matters
            fullAccess: permission.fullAccess,
            viewAccess: permission.fullAccess, // Set view access same as full access for mobile
            addAccess: permission.fullAccess,
            editAccess: permission.fullAccess,
            deleteAccess: permission.fullAccess,
            downloadAccess: permission.fullAccess,
            approvalAccess: permission.fullAccess,
            // Preserve backend fields
            uid: permission.uid,
            id: permission.id,
            isModified: permission.isModified || false,
            showInMenu: permission.fullAccess, // Show in menu if has full access
            createdBy: permission.createdBy || "ADMIN",
            createdTime: permission.createdTime || new Date().toISOString(),
            modifiedBy: permission.modifiedBy || "ADMIN",
            modifiedTime: new Date().toISOString(),
    ...
    // After saving mobile permissions, trigger menu regeneration
    if (result) {
      try {
        await this.regenerateMobileMenu();
      } catch (error) {
        console.warn(
          "Failed to regenerate mobile menu after permission update:",
          error
        );
        // Don't fail the whole operation if menu regeneration fails
      }
    }
  ...
  // Regenerate mobile menu to work around backend bug
  private async regenerateMobileMenu(): Promise<void> {
    ...
    if (response.success) {
      console.log("Mobile menu regeneration successful:", response.data);
    } else {
      console.error("Mobile menu regeneration failed:", response.message);
    }
  }
