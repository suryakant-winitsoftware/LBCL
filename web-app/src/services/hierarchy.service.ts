...
class HierarchyService {
  ...
  async getChildSKUGroups(parentUID: string): Promise<SKUGroup[]> {
    try {
      ...
      console.log("🔍 getChildSKUGroups - Looking for children of:", parentUID);
      console.log(
        "📋 All groups sample:",
      ...
      // In most cases, Code and UID are the same in the database
      // ParentUID field stores the parent's Code/UID (they're usually the same)
      console.log("🔎 Looking for groups where ParentUID =", parentUID);

      // Simple direct filter - ParentUID should match the passed value
      const childGroups: SKUGroup[] = allGroups
        .filter((group) => {
          const matches = group.ParentUID === parentUID;
