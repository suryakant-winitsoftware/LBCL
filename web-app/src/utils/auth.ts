 ...
 */
export function decodeToken(token: string | null | undefined): UserInfo | null {
  try {
    // Check if token exists and is valid
    if (!token || typeof token !== 'string' || token.trim() === '') {
      console.error("Token is null, undefined, or empty");
      return null;
    }
 ...
 */
export function getCurrentUser(token: string | null | undefined): string {
 ...
 */
export function hasPermission(token: string | null | undefined, permission: string): boolean {
  const userInfo = decodeToken(token);
