...
export function handleOrganizationSelection(
  ...
  // Ensure currentSelectedOrgs is an array
  const safeSelectedOrgs = Array.isArray(currentSelectedOrgs) ? currentSelectedOrgs : [];
  const updatedSelectedOrgs = [...safeSelectedOrgs.slice(0, levelIndex), selectedValue];
